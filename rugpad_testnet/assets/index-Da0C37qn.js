...
`)[0]||String(e),a=typeof t?.message==`string`&&t.message||String(e),o=n||typeof t?.name==`string`&&t.name!==`Error`&&t.name||`ERROR`;window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:i,full:a,name:o}}))}var Cp=class extends Y{constructor(...e){super(...e),this.open=!1,this.hash=null,this.action=``,this.transfers=null,this.errorMsg=``,this.errorFull=``,this.errorName=``,this._onError=e=>{this.errorMsg=e.detail.short,this.errorFull=e.detail.full,this.errorName=e.detail.name,this.hash=null,this.open=!0},this._onShow=e=>{this.hash=e.detail.hash,this.action=e.detail.action,this.transfers=null,this.open=!0,this._fetchTransfers(e.detail.hash)}}connectedCallback(){super.connectedCallback(),window.addEventListener(`tx-confirm-show`,this._onShow),window.addEventListener(`app-error-show`,this._onError)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`tx-confirm-show`,this._onShow),window.removeEventListener(`app-error-show`,this._onError)}async _fetchTransfers(e){let{connectedWallet:t}=await Zt(async()=>{let{connectedWallet:e}=await Promise.resolve().then(()=>Bu);return{connectedWallet:e}},void 0,import.meta.url);if(!t){this.transfers=[];return}let n=F[P],r=$u(P),i=t.address.slice(2).toLowerCase(),a;for(let t=0;t<20;t++){try{if(a=await r.getTransactionReceipt({hash:e}),a)break}catch{}await new Promise(e=>setTimeout(e,800))}if(!a){this.transfers=[];return}G.refresh(),G.refreshUser(),setTimeout(()=>{G.refresh(),G.refreshUser()},1500);let o=a.logs.filter(e=>e.topics[0]?.toLowerCase()===yp&&e.topics[2]?.slice(-40).toLowerCase()===i);if(o.length===0){this.transfers=[];return}let s=[];for(let e of o){let t=e.address,i=`0x`+(e.topics[1]??`0x`).slice(-40),a=BigInt(e.data),o;if(t.toLowerCase()===n.fomon.toLowerCase())o=`foMON`;else if(t.toLowerCase()===n.shmon.toLowerCase())o=`shMON`;else{let e=G.vibecoins.find(e=>e.address.toLowerCase()===t.toLowerCase());if(e)o=e.symbol;else try{o=await r.readContract({address:t,abi:V,functionName:`symbol`})}catch{o=t.slice(0,6)+`...`}}let c=i.toLowerCase()===bp&&t.toLowerCase()===n.fomon.toLowerCase();s.push({symbol:o,amount:a,isAirdrop:c})}this.transfers=s}_close(){this.open=!1}_breakPascal(e){return e.replace(/([a-z])([A-Z])/g,`$1​$2`)}async _copy(){this.hash&&await navigator.clipboard.writeText(this.hash)}static{this.styles=K`
  ...
  `}render(){if(!this.open)return q``;if(!this.hash)return q`
    ...
    `;let e=F[P],t=this.hash.slice(0,10)+`…`+this.hash.slice(-6),n=e.explorer?`${e.explorer}/tx/${this.hash}`:null;return q`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="dialog">
          <button class="close" @click=${this._close}>×</button>
    ...
    `}};$([Q()],Cp.prototype,`open`,void 0),$([Q()],Cp.prototype,`hash`,void 0),$([Q()],Cp.prototype,`action`,void 0),$([Q()],Cp.prototype,`transfers`,void 0),$([Q()],Cp.prototype,`errorMsg`,void 0),$([Q()],Cp.prototype,`errorFull`,void 0),$([Q()],Cp.prototype,`errorName`,void 0),Cp=$([X(`tx-confirm-dialog`)],Cp);var wp=10n**18n,Tp=class extends Y{constructor(...e){super(...e),this._subscribedKeys=new Set,this._onLiveUpdate=e=>{[...this._subscribedKeys].some(t=>e.detail.has(t))&&this.requestUpdate()},this._onDenomChanged=()=>this.requestUpdate()}static{this.styles=K`
  ...
  `}get spin(){return q`<span class="spin"></span>`}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),window.addEventListener(`denom-changed`,this._onDenomChanged),this._syncSubscriptions()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate),window.removeEventListener(`denom-changed`,this._onDenomChanged);for(let e of this._subscribedKeys)U(e);this._subscribedKeys.clear()}updated(e){super.updated(e),this._syncSubscriptions()}_syncSubscriptions(){let e=new Set(this.liveKeys().filter(Boolean));for(let t of this._subscribedKeys)e.has(t)||(U(t),this._subscribedKeys.delete(t));for(let t of e)this._subscribedKeys.has(t)||(od(t),this._subscribedKeys.add(t))}val(e){return H.get(e)}loaded(...e){return e.every(e=>H.has(e))}},Ep=class extends Tp{constructor(...e){super(...e),this.key=``,this.numerator=1,this.denominator=1}_tokenPrefix(){let e=this.key.match(/^token:(0x[a-fA-F0-9]+|native)/);return e?`token:${e[1]}`:null}liveKeys(){let e=this._tokenPrefix(),t=`token:${F[P].shmon.toLowerCase()}`,n=e??t,r=[this.key,`meta:shmonRate`];return e&&r.push(`${e}:symbol`),n!==t&&n!==`token:native`&&r.push(`${n}:K`,`${n}:P0`,`${n}:totalSupply`),r}render(){if(!this.key)return q`${this.spin}`;let e=`token:${F[P].shmon.toLowerCase()}`,t=[`:poolBalance`,`:totalVolume`,`:volume24h`].some(e=>this.key.endsWith(e))?e:this._tokenPrefix()??e,n=Cd(),r=n===`mon`?`token:native`:e,i=this.val(`${r}:symbol`)??(n===`mon`?`MON`:`shMON`);if(!this.loaded(this.key))return q`${this.spin} ${i}`;let a=e=>this.numerator!==1||this.denominator!==1?e*BigInt(this.numerator)/BigInt(this.denominator):e,o;if(t===`token:native`){let e=Dd();o=e>0n?a(this.val(this.key)??0n)*wp/e:0n}else if(t===e)o=a(this.val(this.key)??0n);else{let e=this.val(`${t}:K`)??0n;o=((this.val(`${t}:P0`)??0n)+e*(this.val(`${t}:totalSupply`)??0n)/wp)*a(this.val(this.key)??0n)/wp}let s=t===r?``:`≈`;return n===`mon`?q`${s}${Nd(o*Dd()/wp,i)}`:q`${s}${Nd(o,i)}`}};$([Z()],Ep.prototype,`key`,void 0),$([Z({type:Number})],Ep.prototype,`numerator`,void 0),$([Z({type:Number})],Ep.prototype,`denominator`,void 0),Ep=$([X(`live-valuation`)],Ep);var Dp=class extends Tp{constructor(...e){super(...e),this.valueKey=``,this.long=!1}_symbolKey(){return this.valueKey.replace(/:(?!0x|native).*$/,`:symbol`)}liveKeys(){return[this.valueKey,this._symbolKey()]}render(){let e=this._symbolKey(),t=this.val(e);if(!this.valueKey||!this.loaded(this.valueKey,e))return q`${this.spin}${t?q` $${t}`:``}`;let n=this.val(this.valueKey)??0n;if(this.long){let e=Number(n/10n**15n)/1e3;return q`${Intl.NumberFormat().format(e)} \$${t??`???`}`}return q`${Nd(n,t??`???`)}`}};$([Z({attribute:`value-key`})],Dp.prototype,`valueKey`,void 0),$([Z({type:Boolean})],Dp.prototype,`long`,void 0),Dp=$([X(`live-token-amount`)],Dp);var Op=class extends Tp{constructor(...e){super(...e),this.valueKey=``}_symbolKey(){return this.valueKey.replace(/:(?!0x|native).*$/,`:symbol`)}liveKeys(){return[this.valueKey,this._symbolKey(),`meta:shmonRate`]}render(){let e=this._symbolKey(),t=this.val(e),n=Cd()===`mon`?`MON`:`shMON`;return!this.valueKey||!this.loaded(this.valueKey,e)?q`${this.spin} ${n}${t?q`/$${t}`:``}`:q`${Md(this.val(this.valueKey)??0n,t??`???`)}`}};$([Z({attribute:`value-key`})],Op.prototype,`valueKey`,void 0),Op=$([X(`live-token-price`)],Op);var kp=class extends Tp{constructor(...e){super(...e),this.numKey=``,this.demKey=``}liveKeys(){return[this.numKey,this.demKey]}render(){return!this.numKey||!this.demKey||!this.loaded(this.numKey,this.demKey)?q`${this.spin}%`:q`${Rd(this.val(this.numKey)??0n,this.val(this.demKey)??0n)}`}};$([Z({attribute:`num-key`})],kp.prototype,`numKey`,void 0),$([Z({attribute:`dem-key`})],kp.prototype,`demKey`,void 0),kp=$([X(`live-percent`)],kp);var Ap=class extends Tp{constructor(...e){super(...e),this.key=``,this.full=!1}liveKeys(){return[this.key]}render(){if(!this.key||!this.loaded(this.key))return q`0x${this.spin}…${this.spin}`;let e=this.val(this.key)??``,t=this.full?e:Fd(e),n=F[P].explorer;return n?q`<a href="${n}/address/${e}" target="_blank" rel="noopener noreferrer">${t}</a>`:q`${t}`}};$([Z()],Ap.prototype,`key`,void 0),$([Z({type:Boolean})],Ap.prototype,`full`,void 0),Ap=$([X(`live-address`)],Ap);var jp=class extends Tp{constructor(...e){super(...e),this.key=``,this.fallback=`--`}liveKeys(){return[this.key]}render(){return!this.key||!this.loaded(this.key)?q`${this.spin}`:q`${this.val(this.key)??this.fallback}`}};$([Z()],jp.prototype,`key`,void 0),$([Z()],jp.prototype,`fallback`,void 0),jp=$([X(`live-string`)],jp);var Mp=class extends Tp{constructor(...e){super(...e),this.key=``,this.milliseconds=!1,this.warningMs=300*1e3,this._interval=null}static{this.styles=[Tp.styles,K`
      :host(.warning) {
        color: #ff2d6b;
    ...
    `]}liveKeys(){return[this.key]}connectedCallback(){super.connectedCallback();let e=this.milliseconds?50:1e3;this._interval=setInterval(()=>this.requestUpdate(),e)}disconnectedCallback(){super.disconnectedCallback(),this._interval!==null&&(clearInterval(this._interval),this._interval=null)}render(){if(!this.key||!this.loaded(this.key)){this.classList.remove(`warning`);let e=this.spin;return this.milliseconds?q`${e}:${e}:${e}.${e}`:q`${e}:${e}:${e}`}let e=Number(this.val(this.key)??0n)*1e3-Date.now();return this.classList.toggle(`warning`,e>0&&e<=this.warningMs),q`${(this.milliseconds?Ld:Id)(e)}`}};$([Z()],Mp.prototype,`key`,void 0),$([Z({type:Boolean})],Mp.prototype,`milliseconds`,void 0),$([Z({type:Number,attribute:`warning-ms`})],Mp.prototype,`warningMs`,void 0),Mp=$([X(`live-countdown`)],Mp);var Np=class extends Tp{constructor(...e){super(...e),this.prefix=``}liveKeys(){return this.prefix?[`${this.prefix}:K`,`${this.prefix}:P0`,`${this.prefix}:totalSupply`,`meta:shmonRate`]:[]}render(){let e=Cd(),t=e===`mon`?`MON`:`shMON`;if(!this.prefix||!this.loaded(`${this.prefix}:K`,`${this.prefix}:P0`,`${this.prefix}:totalSupply`))return q`${this.spin} ${t}`;let n=this.val(`${this.prefix}:K`)??0n,r=this.val(`${this.prefix}:P0`)??0n,i=this.val(`${this.prefix}:totalSupply`)??0n,a=(r+n*i/wp)*i/wp;if(e===`mon`){let e=Dd();return q`≈${Nd(e>0n?a*e/wp:0n,`MON`)}`}return q`≈${Nd(a,`shMON`)}`}};$([Z()],Np.prototype,`prefix`,void 0),Np=$([X(`live-bonding-mcap`)],Np);var Pp=class extends Tp{constructor(...e){super(...e),this.prefix=``}liveKeys(){return this.prefix?[`${this.prefix}:K`,`${this.prefix}:P0`,`${this.prefix}:totalSupply`,`${this.prefix}:symbol`]:[]}render(){let e=this.val(`${this.prefix}:symbol`);if(!this.prefix||!this.loaded(`${this.prefix}:K`,`${this.prefix}:P0`,`${this.prefix}:totalSupply`))return q`${this.spin}${e?q` shMON/$${e}`:``}`;let t=this.val(`${this.prefix}:K`)??0n,n=(this.val(`${this.prefix}:P0`)??0n)+t*(this.val(`${this.prefix}:totalSupply`)??0n)/wp;return q`${(Number(n)/0xde0b6b3a7640000).toFixed(9)} shMON/$${e??`?`}`}};$([Z()],Pp.prototype,`prefix`,void 0),Pp=$([X(`live-bonding-price`)],Pp);var Fp=class extends Tp{constructor(...e){super(...e),this.allocKey=``,this.depositKey=``,this.totalKey=``,this.symbolKey=``}liveKeys(){return[this.allocKey,this.depositKey,this.totalKey,this.symbolKey]}render(){let e=this.val(this.symbolKey);if(!this.allocKey||!this.depositKey||!this.totalKey||!this.loaded(this.allocKey,this.depositKey,this.totalKey))return q`${this.spin}${e?q` $${e}`:``}`;let t=this.val(this.allocKey)??0n,n=this.val(this.depositKey)??0n,r=this.val(this.totalKey)??0n;return r===0n||t===0n?q`—`:q`${Nd(t*n/r,e??`?`)}`}};$([Z({attribute:`alloc-key`})],Fp.prototype,`allocKey`,void 0),$([Z({attribute:`deposit-key`})],Fp.prototype,`depositKey`,void 0),$([Z({attribute:`total-key`})],Fp.prototype,`totalKey`,void 0),$([Z({attribute:`symbol-key`})],Fp.prototype,`symbolKey`,void 0),Fp=$([X(`live-series-tokens`)],Fp);var Ip=`fomonad-fomon-visited`,Lp=class extends Y{constructor(...e){super(...e),this.open=!1,this.providerOpen=!1,this.referralCode=``,this.activeRefInput=``,this.registerPending=!1,this.claimPending=!1,this.confirmReg=!1,this.regEligError=!1,this.copiedRef=!1,this.copied=!1,this.denomPref=Cd(),this._wrongNetworkClicks=0,this._subscribedKeys=[],this._onLiveUpdate=e=>{this._subscribedKeys.some(t=>e.detail.has(t))&&this.requestUpdate()},this._onDenomChanged=()=>{this.denomPref=Cd(),this.requestUpdate()},this._onWalletChanged=()=>{L?.chainId===10143&&(this._wrongNetworkClicks=0),this._resyncKeys(),this.requestUpdate()},this._onDocClick=e=>{e.composedPath().includes(this)||(this.open=!1,this.providerOpen=!1)}}static{this.styles=K`
  ...
  `}connectedCallback(){super.connectedCallback(),I.addEventListener(`wallet-changed`,this._onWalletChanged),window.addEventListener(`live-values-updated`,this._onLiveUpdate),window.addEventListener(`denom-changed`,this._onDenomChanged),document.addEventListener(`click`,this._onDocClick),this._resyncKeys()}disconnectedCallback(){super.disconnectedCallback(),I.removeEventListener(`wallet-changed`,this._onWalletChanged),window.removeEventListener(`live-values-updated`,this._onLiveUpdate),window.removeEventListener(`denom-changed`,this._onDenomChanged),document.removeEventListener(`click`,this._onDocClick);for(let e of this._subscribedKeys)U(e);this._subscribedKeys=[]}_resyncKeys(){for(let e of this._subscribedKeys)U(e);let e=L?.address.toLowerCase()??``,t=F[10143]?.fomon?.toLowerCase()??``;this._subscribedKeys=e?[`token:${t}:balance:${e}`,`user:${e}:fomonOptedIn`,`user:${e}:fomonClaimable`,`user:${e}:referralCode`,`user:${e}:referralEarned`]:[];for(let e of this._subscribedKeys)od(e)}async _connect(e){await Gu(e),qu(e.info.rdns),await nd(P),this.providerOpen=!1,this.requestUpdate()}_disconnect(){Ku(),Ju(),this.open=!1}_showNotifDot(e,t){if((H.get(`user:${e}:fomonClaimable`)??0n)>0n)return!0;let n=H.get(`token:${t}:balance:${e}`)??0n,r=localStorage.getItem(Ip);return n>0n&&!r}async _copyAddress(){L&&(await navigator.clipboard.writeText(L.address),this.copied=!0,setTimeout(()=>{this.copied=!1},1500))}async _claimFoMon(){let e=F[P],t=z();if(!(!t||!L)){this.claimPending=!0;try{let n=await t.writeContract({address:e.fomon,abi:rd,functionName:`claim`,account:L.address,chain:t.chain});G.refreshUser(),xp(n,`Claimed foMON dividends`)}catch(e){Sp(e)}finally{this.claimPending=!1}}}async _optIn(){let e=F[P],t=z();if(!(!t||!L))try{let n=await t.writeContract({address:e.fomon,abi:rd,functionName:`optIn`,account:L.address,chain:t.chain});G.refreshUser(),xp(n,`Opted in to foMON dividends`)}catch(e){Sp(e)}}async _checkEligAndConfirm(){if(!this.referralCode||!L)return;let e=F[P],t=$u();try{await t.simulateContract({address:e.game,abi:B,functionName:`registerReferral`,args:[Vd(this.referralCode)],account:L.address}),this.regEligError=!1,this.confirmReg=!0}catch{this.regEligError=!0}}async _registerReferral(){if(!this.referralCode||!this.confirmReg)return;let e=F[P],t=z();if(!(!t||!L)){this.registerPending=!0,this.confirmReg=!1;try{let n=Vd(this.referralCode),r=await t.writeContract({address:e.game,abi:B,functionName:`registerReferral`,args:[n],account:L.address,chain:t.chain}),i=this.referralCode;this.referralCode=``,xp(r,`Registered referral code "${i}"`)}catch(e){Sp(e)}finally{this.registerPending=!1}}}async _copyRefUrl(e){await navigator.clipboard.writeText(`https://fomon.rip/testnet/${e}`),this.copiedRef=!0,setTimeout(()=>{this.copiedRef=!1},1500)}_setDenom(e){wd(e),this.denomPref=e}render(){if(!L){let e=Hu;return e.length===0?q`<button class="connect-btn" style="cursor:default;opacity:0.4">NO WALLET</button>`:e.length===1?q`
      ...
      `}if(L.chainId!==10143)return q`
        <button class="connect-btn" style="background:#ff2d6b22;border-color:#ff2d6b;color:#ff2d6b" @click=${()=>{this._wrongNetworkClicks++,this._wrongNetworkClicks>=2?(Ku(),Ju(),this._wrongNetworkClicks=0):nd(P)}}>${`SWITCH TO ${F[10143]?.name?.toUpperCase()??`CORRECT NETWORK`}`}</button>
      `;let e=L.address.toLowerCase(),t=F[P],n=t?.shmon?.toLowerCase()??``,r=t?.fomon?.toLowerCase()??``,i=H.get(`user:${e}:fomonOptedIn`)??!1,a=H.get(`user:${e}:fomonClaimable`)??0n,o=H.get(`user:${e}:referralCode`)??null,s=H.get(`user:${e}:referralEarned`)??0n,c=this._showNotifDot(e,r);return q`
      <div style="position:relative">
        <button
          class="address-btn ${this.open?`active`:``}"
          @click=${()=>{this.open=!this.open,this.open&&localStorage.setItem(Ip,`1`)}}
        >
          ${c?q`<span class="notif-dot"></span>`:``}
          ${L.icon?q`<img src=${L.icon} alt="" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;border-radius:2px" />`:``}
          ${this.copied?q`<span class="copied-flash">COPIED!</span>`:q`<span>${Fd(L.address)}</span>`}
        ...
        ${this.open?q`
          <div class="dropdown">
            ...
            <div class="row">
              <span class="lbl">shMON${t.shmonUrl?q`&nbsp;<a href="${t.shmonUrl}" target="_blank" rel="noopener" title="redeem shMON for MON">Redeem ↗</a>`:``}</span>
    ...
    `}};$([Q()],Lp.prototype,`open`,void 0),$([Q()],Lp.prototype,`providerOpen`,void 0),$([Q()],Lp.prototype,`referralCode`,void 0),$([Q()],Lp.prototype,`activeRefInput`,void 0),$([Q()],Lp.prototype,`registerPending`,void 0),$([Q()],Lp.prototype,`claimPending`,void 0),$([Q()],Lp.prototype,`confirmReg`,void 0),$([Q()],Lp.prototype,`regEligError`,void 0),$([Q()],Lp.prototype,`copiedRef`,void 0),$([Q()],Lp.prototype,`copied`,void 0),$([Q()],Lp.prototype,`denomPref`,void 0),$([Q()],Lp.prototype,`_wrongNetworkClicks`,void 0),Lp=$([X(`wallet-widget`)],Lp);var Rp=class extends Y{constructor(...e){super(...e),this.currentHash=window.location.hash||`#pump`,this._onHash=()=>{this.currentHash=window.location.hash||`#pump`}}static{this.styles=K`
    :host {
      display: block;
    }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`hashchange`,this._onHash)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._onHash)}render(){return q`
    ...
    `}};$([Q()],Rp.prototype,`currentHash`,void 0),Rp=$([X(`header-bar`)],Rp);var zp=class extends Y{constructor(...e){super(...e),this.claimPendingA=!1,this.claimPendingB=!1,this._subscribedKeys=[],this._tickInterval=0,this._onLiveUpdate=e=>{this._subscribedKeys.some(t=>e.detail.has(t))&&this.requestUpdate()},this._onWalletChanged=()=>{this._resyncKeys(),this.requestUpdate()}}static{this.styles=K`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),I.addEventListener(`wallet-changed`,this._onWalletChanged),this._resyncKeys(),this._tickInterval=window.setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate),I.removeEventListener(`wallet-changed`,this._onWalletChanged);for(let e of this._subscribedKeys)U(e);this._subscribedKeys=[],clearInterval(this._tickInterval)}_resyncKeys(){for(let e of this._subscribedKeys)U(e);let e=L?.address.toLowerCase()??``;this._subscribedKeys=[`game:phase`,`game:phaseEndTime`,`game:vibecoin`,`game:cycle`,`meta:chainTimeDelta`],e&&this._subscribedKeys.push(`user:${e}:seriesADeposit`,`user:${e}:seriesBDeposit`);for(let e of this._subscribedKeys)od(e)}_openBuy(e){window.dispatchEvent(new CustomEvent(`open-buy-dialog`,{detail:{target:e}}))}async _claimA(){let e=H.get(`game:cycle`),t=z();if(!(!t||!L||e===void 0)){this.claimPendingA=!0;try{let n=F[P],r=await t.writeContract({address:n.game,abi:B,functionName:`claimSeriesATokens`,args:[e],account:L.address,chain:t.chain});G.refresh(),G.refreshUser(),xp(r,`Claimed Insider Vibecoins`)}catch(e){console.error(`Claim A error:`,e)}finally{this.claimPendingA=!1}}}async _claimB(){let e=H.get(`game:cycle`),t=z();if(!(!t||!L||e===void 0)){this.claimPendingB=!0;try{let n=F[P],r=await t.writeContract({address:n.game,abi:B,functionName:`claimSeriesBTokens`,args:[e],account:L.address,chain:t.chain});G.refresh(),G.refreshUser(),xp(r,`Claimed "Insider" Vibecoins`)}catch(e){console.error(`Claim B error:`,e)}finally{this.claimPendingB=!1}}}_phaseStatus(e){let t=H.get(`game:phase`);if(t!==e)return e<(t??0)?`ENDED`:`PENDING`;let n=H.get(`game:phaseEndTime`)??0n;if(n===0n)return e===0?`DEPOSIT TO KICK OFF (0.1 shMON needed)`:`PENDING`;let r=H.get(`meta:chainTimeDelta`)??0,i=Number(n)-(Date.now()/1e3+r);return i>0?`ENDS IN ${Math.floor(i/3600)}h ${Math.floor(i%3600/60)}m ${Math.floor(i%60)}s`:`ENDED`}render(){let e=L?.address.toLowerCase()??``,t=H.get(`game:phase`)??0,n=t,r=t===0,i=t===1,a=t===2||t===3,o=(H.get(`game:vibecoin`)??``).toLowerCase(),s=H.get(`user:${e}:seriesADeposit`)??0n,c=H.get(`user:${e}:seriesBDeposit`)??0n,l=e?s>0n:!1,u=e?c>0n:!1;return q`
      <div class="cards">
        <!-- Series A -->
        <div class="card ${t>0&&!a?`disabled`:``}">
          ...
          ${r?q`
            ...
            ${n===0&&(H.get(`game:phaseEndTime`)??0n)===0n?q`
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),I.addEventListener(`wallet-changed`,this._onWalletChanged),this._resyncKeys()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate),I.removeEventListener(`wallet-changed`,this._onWalletChanged);for(let e of this._subscribedKeys)U(e);this._subscribedKeys=[]}updated(e){e.has(`vcAddress`)&&this._resyncKeys()}_resyncKeys(){for(let e of this._subscribedKeys)U(e);let e=this.vcAddress.toLowerCase(),t=L?.address.toLowerCase()??``,n=H.get(`token:${e}:cycle`);this._subscribedKeys=[`token:${e}:K`,`token:${e}:P0`,`token:${e}:totalSupply`,`token:${e}:symbol`,`token:${e}:unlocked`,`token:${e}:athPrice`,`token:${e}:athPostRug`,`token:${e}:cycle`,`game:phase`],n!==void 0&&this._subscribedKeys.push(`game:tokensSeriesA:${n}`,`game:tokensSeriesB:${n}`),t&&this._subscribedKeys.push(`token:${e}:balance:${t}`,`token:${e}:dividendsClaimable:${t}`,`token:${e}:optedIn:${t}`);for(let e of this._subscribedKeys)od(e)}async _optToggle(e){let t=z();if(!(!t||!L)){this._optPending=!0,this.requestUpdate();try{let n=e?`optOut`:`optIn`;xp(await t.writeContract({address:this.vcAddress,abi:V,functionName:n,account:L.address,chain:t.chain}),e?`Vibes undesired`:`Vibes desired`),G.refreshUser()}catch(e){Sp(e)}finally{this._optPending=!1,this.requestUpdate()}}}async _claimDividends(){let e=z();if(!(!e||!L)){this._claimPending=!0,this.requestUpdate();try{xp(await e.writeContract({address:this.vcAddress,abi:V,functionName:`claimDividends`,args:[L.address],account:L.address,chain:e.chain}),`Claim Vibes`),G.refreshUser()}catch(e){console.error(`Claim dividends error:`,e)}finally{this._claimPending=!1,this.requestUpdate()}}}_renderChart(e,t,n,r,i,a,o){let s=n*3n/2n||10n**18n,c=10n**18n,l=[],u=[];for(let n=0;n<=60;n++){let r=s*BigInt(n)/BigInt(60),i=e+t*r/c;l.push(Number(r)/0xde0b6b3a7640000),u.push(Number(i)/1e9)}let d=l[l.length-1]||1,f=Math.max(...u)||1,p=e=>38+e/d*262,m=e=>66-e/f*66,h=l.map((e,t)=>`${t===0?`M`:`L`}${p(e).toFixed(1)},${m(u[t]).toFixed(1)}`).join(` `),g=Math.round(p(Number(n)/0xde0b6b3a7640000)),_=Math.round(p(Number(a))),v=r>0n?p(Number(r)/0xde0b6b3a7640000):null,y=r>0n&&i>0n?p(Number(r+i)/0xde0b6b3a7640000):null,b=e=>String(e).split(``).map(e=>`₀₁₂₃₄₅₆₇₈₉`[+e]).join(``),x=e=>{let t=e/1e9;if(t>=.001)return t.toPrecision(3);let n=t.toFixed(30).split(`.`)[1]??``,r=0;for(let e of n)if(e===`0`)r++;else break;let i=n.slice(r,r+3);return`0.0${b(r-1)}${i}`};return q`
    ...
    `}render(){let e=this.vcAddress;if(!e||e===`0x0000000000000000000000000000000000000000`)return q`<div style="color:#3a3a4a;font-family:monospace;font-size:11px">No vibecoin active</div>`;let t=e.toLowerCase(),n=L?.address.toLowerCase()??``,r=H.get(`token:${t}:K`)??0n,i=H.get(`token:${t}:P0`)??0n,a=H.get(`token:${t}:totalSupply`)??0n,o=H.get(`token:${t}:unlocked`)??!1,s=H.get(`token:${t}:athPrice`)??0n,c=H.get(`token:${t}:athPostRug`)??!1,l=H.get(`token:${t}:cycle`),u=H.get(`game:phase`)!==void 0,d=l===void 0?``:`game:tokensSeriesA:${l}`,f=l===void 0?``:`game:tokensSeriesB:${l}`,p=(d?H.get(d):void 0)??0n,m=(f?H.get(f):void 0)??0n,h=s>0n&&r>0n&&s>i?(s-i)*10n**18n/r:0n,g=n?H.get(`token:${t}:balance:${n}`)??0n:0n,_=n?H.get(`token:${t}:dividendsClaimable:${n}`)??0n:0n,v=n?H.get(`token:${t}:optedIn:${n}`)??!1:!1,y=o,b=H.has(`token:${t}:K`)&&H.has(`token:${t}:P0`)&&H.has(`token:${t}:totalSupply`);return q`
      <div class="panel">
        <div class="row">
        ...
        <div class="row">
          ...
          <span class="val"><live-token-amount long value-key="token:${t}:totalSupply"></live-token-amount></span>
        </div>
        ${L?q`
          <div class="row">
            <span class="key">YOUR BALANCE</span>
            <span class="val">
              <live-token-amount long value-key="token:${t}:balance:${n}"></live-token-amount>
        ...
        ${b?this._renderChart(i,r,a,p,m,h,c):q`<div class="chart-area chart-loading">LOADING CHART…</div>`}
        ...
        ${u&&!y?q`
          ...
          <div class="row">
            ...
            <span class="val"><live-token-amount long style="color: #ffe600" value-key="token:${F[P].fomon.toLowerCase()}:remainingCeiling"></live-token-amount></span>
          </div>
          <div class="row">
            <span class="key">AIRDROP REMAINING</span>
            <span class="val">
              <live-token-amount long value-key="token:${F[P].fomon.toLowerCase()}:airdropRemaining"></live-token-amount>
              (<live-percent num-key="token:${F[P].fomon.toLowerCase()}:airdropRemaining" dem-key="fomon:totalSupplyConst"></live-percent>)
            </span>
          </div>
        `:``}
        ...
        <div class="dividends">
          ...
          <span class="div-val"><live-token-amount long value-key="token:${t}:dividendsClaimable:${n}"></live-token-amount></span>
          <button
            class="claim-btn"
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),this._resyncKeys(),this._maybeRequestReferralCode(),this._timerInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate);for(let e of this._subscribedKeys)U(e);this._subscribedKeys=[],this._timerInterval&&clearInterval(this._timerInterval)}_resyncKeys(){for(let e of this._subscribedKeys)U(e);let e=H.get(`game:cycle`)??0n,t=(H.get(`game:lastBuyer`)??``).toLowerCase(),n=e>0n?e-1n:0n;this._subscribedKeys=[`game:phase`,`game:phaseEndTime`,`game:lastBuyer`,`game:cycle`,`meta:chainTimeDelta`],t&&this._subscribedKeys.push(`user:${t}:referralCode`),this.forceSettled&&e>0n&&this._subscribedKeys.push(`game:winner:${n}`,`game:prize:${n}`);for(let e of this._subscribedKeys)od(e)}_maybeRequestReferralCode(){let e=H.get(`game:lastBuyer`)??``;e&&window.dispatchEvent(new CustomEvent(`request-referral-code`,{detail:{address:e}}))}async _triggerSettlement(){let e=z();if(!(!e||!L)){this.triggerPending=!0;try{let t=F[P];xp(await e.writeContract({address:t.game,abi:B,functionName:`triggerSettlement`,account:L.address,chain:e.chain}),`Rugpull triggered — settlement in progress`)}catch(e){Sp(e)}finally{this.triggerPending=!1}}}render(){let e=H.get(`game:phase`);if(e===void 0)return q``;let t=H.get(`game:phaseEndTime`)??0n,n=H.get(`meta:chainTimeDelta`)??0,r=e===2&&t>0n?Math.max(0,Number(t)*1e3-(Date.now()+n*1e3)):0,i=e===3||e===2&&r===0,a=this.forceSettled,o=H.get(`game:cycle`)??0n,s=o>0n?o-1n:0n,c=`0x0000000000000000000000000000000000000000`,l=this.forceSettled?H.get(`game:winner:${s}`)??c:H.get(`game:lastBuyer`)??c,u=this.forceSettled?`game:prize:${s}`:`game:prizePool`,d=H.get(`user:${l.toLowerCase()}:referralCode`),f=F[P].explorer;return q`
      <div class="panel">
        ${a?q`
        ...
        `:q`
          <div class="rugpuller-block">
            <div class="rugpuller-label">Current Rugpuller${d?q` · <span style="color:#ffe600">${d}</span>`:``}</div>
            ${f?q`
  ...
  `}_resyncByCycleSub(){let e=H.get(`game:cycle`);if(!e||e<=1n)return;let t=`vibecoins:byCycle:${Number(e)-1}`;this._subscribedKeys.includes(t)||(od(t),this._subscribedKeys.push(t));let n=H.get(t);if(n){let e=n.toLowerCase();for(let t of[`token:${e}:palette`,`token:${e}:name`,`token:${e}:symbol`])this._subscribedKeys.includes(t)||(od(t),this._subscribedKeys.push(t))}}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),this._subscribedKeys=[`game:phase`,`game:phaseEndTime`,`game:vibecoin`,`game:cycle`,`meta:chainTimeDelta`,`vibecoins:addresses`,`game:const:seriesADuration`];for(let e of this._subscribedKeys)od(e);this._resyncByCycleSub(),this._maybeLoadTokenUri(),this._genInterval=setInterval(()=>this._tickGen(),150),this._tickInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate);for(let e of this._subscribedKeys)U(e);this._subscribedKeys=[],this._genInterval&&clearInterval(this._genInterval),this._tickInterval&&clearInterval(this._tickInterval)}_computeDisplayAddr(){let e=`0x0000000000000000000000000000000000000000`,t=H.get(`game:phase`),n=H.get(`game:cycle`)??0n,r=H.get(`game:phaseEndTime`)??0n,i=H.get(`meta:chainTimeDelta`)??0,a=(H.get(`game:vibecoin`)??e).toLowerCase(),o=t===0&&r>0n?Number(r)-(Date.now()/1e3+i):1/0,s=Number(H.get(`game:const:seriesADuration`)??7200n);if(t===0&&n>1n&&(r===0n||o>s/2)){let e=Number(n)-1;return H.get(`vibecoins:byCycle:${e}`)??null}return a&&a!==e?a:null}_maybeLoadTokenUri(){let e=this._computeDisplayAddr();e&&e!==this.lastVibecoin&&(this.lastVibecoin=e,this._loadTokenUri(e))}async _settle(){let e=z();if(!(!e||!L)){this.settlePending=!0;try{let t=F[P],n=await e.writeContract({address:t.game,abi:B,functionName:`settleSeriesB`,account:L.address,chain:e.chain});G.refresh(),xp(n,`Vibecoin generated — insider rounds settled`)}catch(e){Sp(e)}finally{this.settlePending=!1}}}_tickGen(){this._scrambledTicker=Up(),this._tickCount++;let e=this._genStats;if(e.tokensConsumed+=Math.floor(Math.random()*14)+4,e.exitLiquidity+=Math.floor(Math.random()*8)+2,e.hopium+=+(Math.random()<.3),e.bagHolders+=Math.floor(Math.random()*6)+1,e.rugThreads=Math.min(99.7,e.rugThreads+Math.random()*.02),Math.random()<.02&&(e.grayAreas+=1),Math.random()<.01&&(e.marketMakers+=1),this._tickCount%20==0){let e=this._headingIdx;for(;e===this._headingIdx;)e=Math.floor(Math.random()*Wp.length);this._headingIdx=e}if(this._tickCount%27==0){let e=this._statIdx;for(;e===this._statIdx;)e=Math.floor(Math.random()*Gp.length);this._statIdx=e,(e===9||e===10)&&(this._genStats={...this._genStats,grayAreas:Math.floor(Math.random()*3)+1,marketMakers:Math.floor(Math.random()*2)+1})}}async _loadTokenUri(e){try{this.tokenUri=await $u(P).readContract({address:e,abi:V,functionName:`tokenURI`})}catch{this.tokenUri=``}}_getSvgFromUri(){if(!this.tokenUri)return``;if(this.tokenUri.startsWith(`data:application/json`))try{let e=atob(this.tokenUri.split(`,`)[1]);return JSON.parse(e).image??``}catch{return``}return``}render(){let e=F[P],t=`0x0000000000000000000000000000000000000000`,n=H.get(`game:phase`),r=H.get(`game:cycle`)??0n,i=H.get(`game:phaseEndTime`)??0n,a=H.get(`meta:chainTimeDelta`)??0,o=(H.get(`game:vibecoin`)??t).toLowerCase(),s=n===0&&i>0n?Number(i)-(Date.now()/1e3+a):1/0,c=Number(H.get(`game:const:seriesADuration`)??7200n),l=n===0&&r>1n&&(i===0n||s>c/2),u=Number(r)-1,d=l?H.get(`vibecoins:byCycle:${u}`)??null:null,f=l?d?.toLowerCase():o&&o!==t?o.toLowerCase():void 0,p=f?H.get(`token:${f}:palette`):void 0,m=p?.[0]??`#00e5ff`,h=p?.[1]??`#0066ff`,g=d?.toLowerCase()??``,_=g?H.get(`token:${g}:name`):void 0,v=g?H.get(`token:${g}:symbol`):void 0,y=g?H.get(`token:${g}:cycle`):void 0,b=!!(o&&o!==t),x=b?o.toLowerCase():``,S=x?H.get(`token:${x}:name`)??`--`:`--`,C=x?H.get(`token:${x}:symbol`)??`--`:`--`,w=x?H.get(`token:${x}:cycle`):void 0,ee=n===2||n===3,te=this._getSvgFromUri(),ne=o&&o!==t,re=n===void 0||l&&!d,ie=!re&&n===0&&i===0n,ae=!re&&!ie&&!ne,oe=Date.now()/1e3+a,T=ae&&n===1&&i>0n&&oe>=Number(i);return q`
      <div class="container">
        <div
          class="vibecoin-hero"
          style="--vc-col1:${m};--vc-col2:${h};border-color:${l?`#ff2d6b33`:ie?`#252540`:ae?`#1a1a30`:m+`22`}"
        >
          ${l?q`
            ${te?q`
            ...
            `}
            ${e.explorer&&d?q`
              <a class="vc-name" style="color:${m};text-shadow:0 0 20px ${m},0 0 60px ${m}44" href="${e.explorer}/address/${d}" target="_blank">${_??q`<span class="spin"></span>`}</a>
            `:q`
              <div class="vc-name" style="color:${m};text-shadow:0 0 20px ${m},0 0 60px ${m}44">${_??q`<span class="spin"></span>`}</div>
            `}
            <div class="vc-ticker-row">
              <span class="vc-ticker">${v===void 0?q`<span class="spin"></span>`:q`\$${v}`}</span>
              ${y===void 0?``:q`<span class="round-badge">#${y}</span>`}
            </div>
            ${d?q`
          ...
          `:re?q`
            <div class="awaiting">
              <div class="vc-pending-phase"><span class="spin"></span></div>
            </div>
          `:ie?q`
            <div class="awaiting">
          ...
          `:ae?q`
            <div class="vc-pending">
              ...
              <div class="vc-pending-phase">${T?`Vibecoin Ready`:Wp[this._headingIdx]}</div>
              <div class="vc-pending-ticker">\$${this._scrambledTicker}</div>
              <div class="vc-pending-tokens">
                ${T?`Insider rounds have concluded. Vibecoin may now be generated.`:`Vibecoin will be generated after insider rounds conclude.`}<br>
          ...
          `:q`
            ...
            `:q`<div class="vc-name">${S}</div>`}
            <div class="vc-ticker-row">
              <span class="vc-ticker">\$${C}</span>
              ${w===void 0?``:q`<span class="round-badge">#${w}</span>`}
            </div>
    ...
    `}};$([Q()],qp.prototype,`tokenUri`,void 0),$([Q()],qp.prototype,`lastVibecoin`,void 0),$([Q()],qp.prototype,`_scrambledTicker`,void 0),$([Q()],qp.prototype,`_headingIdx`,void 0),$([Q()],qp.prototype,`_statIdx`,void 0),$([Q()],qp.prototype,`_genStats`,void 0),$([Q()],qp.prototype,`settlePending`,void 0),qp=$([X(`pump-tab`)],qp);var Jp=class extends Y{constructor(...e){super(...e),this.address=``,this.tokenUri=``,this.uriLoaded=!1,this.claimAPending=!1,this.claimBPending=!1,this.claimDivPending=!1,this.optInPending=!1,this.observer=null,this._subscribedKeys=[],this._onLiveUpdate=e=>{this._subscribedKeys.some(t=>e.detail.has(t))&&this.requestUpdate()},this._onWalletEvent=()=>{this._resyncKeys(),this.address&&window.dispatchEvent(new CustomEvent(`request-vc-card-data`,{detail:{address:this.address}}))}}static{this.styles=K`
    :host { display: block; }
    ...
    .btn:disabled { opacity: 0.25; cursor: not-allowed; }
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),I.addEventListener(`wallet-changed`,this._onWalletEvent),this._resyncKeys(),this.observer=new IntersectionObserver(e=>{e[0].isIntersecting&&(this._loadTokenUri(),this.address&&window.dispatchEvent(new CustomEvent(`request-vc-card-data`,{detail:{address:this.address}})),this.observer?.disconnect())},{threshold:.1}),this.observer.observe(this)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate),I.removeEventListener(`wallet-changed`,this._onWalletEvent),this._unsubscribeAll(),this.observer?.disconnect()}_unsubscribeAll(){for(let e of this._subscribedKeys)U(e);this._subscribedKeys=[]}_resyncKeys(){this._unsubscribeAll();let e=this.address.toLowerCase();if(!e)return;let t=H.get(`token:${e}:cycle`)?.toString()??``,n=t?H.get(`game:winner:${t}`):void 0,r=L?.address.toLowerCase(),i=[`token:${e}:name`,`token:${e}:cycle`,`token:${e}:unlocked`,`token:${e}:athPrice`,`token:${e}:athPostRug`,`token:${e}:palette`,...t?[`game:winner:${t}`]:[],...n?[`user:${n.toLowerCase()}:referralCode`]:[],...r?[`token:${e}:balance:${r}`,`token:${e}:earned:${r}`,`token:${e}:optedIn:${r}`]:[],...r&&t?[`game:seriesADeposit:${t}:${r}`,`game:seriesBDeposit:${t}:${r}`]:[]];for(let e of i)od(e);this._subscribedKeys=i}updated(e){e.has(`address`)&&this._resyncKeys()}async _loadTokenUri(){if(!this.uriLoaded)try{this.tokenUri=await $u(P).readContract({address:this.address,abi:V,functionName:`tokenURI`}),this.uriLoaded=!0}catch{this.uriLoaded=!0}}_getSvgFromUri(){if(!this.tokenUri)return``;if(this.tokenUri.startsWith(`data:application/json`))try{let e=atob(this.tokenUri.split(`,`)[1]);return JSON.parse(e).image??``}catch{return``}return``}async _claimDividends(){let e=z();if(!(!e||!L)){this.claimDivPending=!0,this.requestUpdate();try{xp(await e.writeContract({address:this.address,abi:V,functionName:`claimDividends`,args:[L.address],account:L.address,chain:e.chain}),`Claim Vibes`)}catch(e){Sp(e)}finally{this.claimDivPending=!1,this.requestUpdate()}}}async _toggleOptIn(e){let t=z();if(!(!t||!L)){this.optInPending=!0,this.requestUpdate();try{xp(await t.writeContract({address:this.address,abi:V,functionName:e?`optOut`:`optIn`,account:L.address,chain:t.chain}),e?`Vibes undesired`:`Vibes desired`)}catch(e){Sp(e)}finally{this.optInPending=!1,this.requestUpdate()}}}async _claimA(){let e=z();if(!e||!L)return;let t=H.get(`token:${this.address.toLowerCase()}:cycle`);if(t!==void 0){this.claimAPending=!0,this.requestUpdate();try{let n=F[P];xp(await e.writeContract({address:n.game,abi:B,functionName:`claimSeriesATokens`,args:[BigInt(t)],account:L.address,chain:e.chain}),`Claimed Insider Vibecoins`)}catch(e){Sp(e)}finally{this.claimAPending=!1,this.requestUpdate()}}}async _claimB(){let e=z();if(!e||!L)return;let t=H.get(`token:${this.address.toLowerCase()}:cycle`);if(t!==void 0){this.claimBPending=!0,this.requestUpdate();try{let n=F[P];xp(await e.writeContract({address:n.game,abi:B,functionName:`claimSeriesBTokens`,args:[BigInt(t)],account:L.address,chain:e.chain}),`Claimed "Insider" Vibecoins`)}catch(e){Sp(e)}finally{this.claimBPending=!1,this.requestUpdate()}}}render(){let e=this.address.toLowerCase(),t=H.get(`token:${e}:name`)??``,n=H.get(`token:${e}:cycle`),r=H.get(`token:${e}:unlocked`)??!1,i=H.get(`token:${e}:athPrice`)??0n,a=H.get(`token:${e}:athPostRug`)??!1,o=H.get(`token:${e}:palette`)??[`#00e5ff`,`#0066ff`],s=n?.toString()??``,c=s?H.get(`game:winner:${s}`):void 0,l=L?.address.toLowerCase(),u=l?H.get(`token:${e}:balance:${l}`)??0n:0n,d=l?H.get(`token:${e}:earned:${l}`)??0n:0n,f=l?H.get(`token:${e}:optedIn:${l}`)??!1:!1,p=l&&s?H.get(`game:seriesADeposit:${s}:${l}`)??0n:0n,m=l&&s?H.get(`game:seriesBDeposit:${s}:${l}`)??0n:0n,h=this._getSvgFromUri(),g=o[0],_=o[1],v=g+`14`,y=g+`2e`,b=g+`40`,x=g+`4d`,S=g+`66`,C=g+`99`,w=g+`2a`,ee=F[P].explorer;return q`
      ...
      ">
        ...
        <div class="info">
          <div class="name-row">
            ${ee?q`
              <a class="vc-name" href="${ee}/address/${this.address}" target="_blank">${t}</a>
            `:q`
              <span class="vc-name">${t}</span>
            `}
            ${n===void 0?``:q`<span class="cycle-badge">Round ${n}</span>`}
          </div>
          ...
          <div class="stats">
            ...
            ${L&&d>0n?q`
              ...
              <span class="stat-val own"><live-token-amount value-key="token:${e}:earned:${l}"></live-token-amount></span>
            `:``}
            ${c?q`
              ...
              <span class="stat-val">
                ...
                ${ee?q`
                  <a style="color:#ff2d6b;text-decoration:none" href="${ee}/address/${c}" target="_blank">${Fd(c)}</a>
                `:q`<span style="color:#ff2d6b">${Fd(c)}</span>`}
              </span>
            `:``}
          </div>
        </div>
        ...
        <div class="btn-row">
          ...
          ${L&&u>0n?q`
            <button class="btn" style="color:#888;border-color:#88888844" @click=${()=>this._toggleOptIn(f)} ?disabled=${this.optInPending}>
              ${this.optInPending?`…`:f?`OPT OUT`:`OPT IN`}
            </button>
          `:``}
        </div>
      </div>
    `}};$([Z()],Jp.prototype,`address`,void 0),$([Q()],Jp.prototype,`tokenUri`,void 0),$([Q()],Jp.prototype,`uriLoaded`,void 0),Jp=$([X(`vibecoin-card`)],Jp);var Yp=20,Xp=class extends Y{constructor(...e){super(...e),this.search=``,this.visibleCount=Yp,this.observer=null,this.sentinelEl=null,this._onLiveUpdate=e=>{e.detail.has(`vibecoins:addresses`)&&this.requestUpdate()},this._onHashChange=()=>this._applyHash()}static{this.styles=K`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`live-values-updated`,this._onLiveUpdate),window.addEventListener(`hashchange`,this._onHashChange),od(`vibecoins:addresses`),this._applyHash()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`live-values-updated`,this._onLiveUpdate),window.removeEventListener(`hashchange`,this._onHashChange),U(`vibecoins:addresses`),this.observer?.disconnect()}_applyHash(){let e=window.location.hash.split(`?`)[0].match(/^#vibecoins\/(0x[0-9a-fA-F]{40})$/);e&&(this.search=e[1],this.visibleCount=Yp)}updated(){let e=this.renderRoot?.querySelector(`.sentinel`);e&&e!==this.sentinelEl&&(this.observer?.disconnect(),this.sentinelEl=e,this.observer=new IntersectionObserver(e=>{e[0].isIntersecting&&(this.visibleCount+=Yp)},{rootMargin:`200px`}),this.observer.observe(e))}_filtered(){let e=H.get(`vibecoins:addresses`)??[],t=this.search.toLowerCase().trim();return t?e.filter(e=>{let n=e.toLowerCase(),r=(H.get(`token:${n}:name`)??``).toLowerCase(),i=(H.get(`token:${n}:symbol`)??``).toLowerCase();return r.includes(t)||i.includes(t)||n.includes(t)}):e}render(){let e=this._filtered(),t=e.slice(0,this.visibleCount),n=!H.has(`vibecoins:addresses`);return q`
      <div class="container">
  ...
  `}open(e){this.target=e.target,this.vibecoinAddress=e.vibecoinAddress,this._tokenBalance=e.tokenBalance??0n,this._resolveVcMeta(e.vibecoinAddress),this._refCode=Gd(),this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.approved=!1,this.error=``,this._sellShMonPreview=0n,this.visible=!0}_resolveVcMeta(e){if(this.target===`seriesA`||this.target===`seriesB`){this._loading=!1;return}let t=(e??H.get(`game:vibecoin`))?.toLowerCase()??``,n=t?H.get(`token:${t}:symbol`):void 0,r=t?H.get(`token:${t}:name`):void 0;this._vcSymbol=n??`token`,this._vcName=r??``,t&&(!n||!r)?(this._waitingForAddr=t,this._loading=!0,window.addEventListener(`live-values-updated`,this._onLiveValuesWhileLoading)):this._loading=!1}async _updateSellPreview(){let e=this._amountWei;if(e===0n||!this.vibecoinAddress){this._sellShMonPreview=0n;return}try{this._sellShMonPreview=await $u(P).readContract({address:this.vibecoinAddress,abi:V,functionName:`shMonForTokens`,args:[e]})}catch{this._sellShMonPreview=0n}}_close(){this.visible=!1,this._loading=!1,window.removeEventListener(`live-values-updated`,this._onLiveValuesWhileLoading)}get _amountWei(){try{let e=parseFloat(this.amountStr);return isNaN(e)||e<=0?0n:BigInt(Math.floor(e*0xde0b6b3a7640000))}catch{return 0n}}_getTitle(){switch(this.target){case`seriesA`:return`INSIDER ROUND`;case`seriesB`:return`"INSIDER" ROUND`;case`publicBuy`:return this._vcName?`PUMP ${this._vcName.toUpperCase()}`:`PUMP`;case`vibecoin`:return this._vcName?`BUY ${this._vcName.toUpperCase()}`:`BUY`;case`vibecoinSell`:return this._vcName?`SELL ${this._vcName.toUpperCase()}`:`SELL`}}_lv(e){return H.get(e)}_constsLoaded(){return H.has(`game:const:seriesAProtocol`)}_getFeeBreakdown(){if(this.target===`vibecoin`)return[[`Dividend pool`,1000n],[`To buyer`,9000n]];if(!this._constsLoaded())switch(this.target){case`seriesA`:return[[`Protocol`,400n],[`Referral`,600n],[`Rugpull Fund`,2000n],[`Vibecoins minted`,7000n]];case`seriesB`:return[[`Protocol`,400n],[`Referral`,600n],[`Rugpull Fund`,1000n],[`Vibecoins minted`,8000n]];default:return[[`Protocol`,400n],[`Referral`,600n],[`Rugpull Fund`,2400n],[`Vibecoin mint`,6600n]]}switch(this.target){case`seriesA`:{let e=this._lv(`game:const:seriesAProtocol`)??400n,t=this._lv(`game:const:seriesAReferral`)??600n,n=this._lv(`game:const:seriesAPrize`)??2000n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoins minted`,10000n-e-t-n]]}case`seriesB`:{let e=this._lv(`game:const:seriesBProtocol`)??400n,t=this._lv(`game:const:seriesBReferral`)??600n,n=this._lv(`game:const:seriesBPrize`)??1000n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoins minted`,10000n-e-t-n]]}default:{let e=this._lv(`game:const:pumpProtocol`)??400n,t=this._lv(`game:const:pumpReferral`)??600n,n=this._lv(`game:const:pumpPrize`)??2400n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoin mint`,10000n-e-t-n]]}}}get _isClosed(){if(this.target!==`vibecoin`)return!1;let e=this.vibecoinAddress?.toLowerCase();return e?H.get(`token:${e}:unlocked`)??!1:!1}_computeTokenPreview(){let e=this._amountWei;if(e===0n)return 0n;let t=this.useShMon?e:e*10n**18n/Dd();if(this.target===`publicBuy`){let e=(H.get(`game:vibecoin`)??``).toLowerCase();if(!e)return 0n;let n=H.get(`token:${e}:totalSupply`)??0n,r=H.get(`token:${e}:P0`)??0n,i=H.get(`token:${e}:K`)??0n;return Qp(t*(this._getFeeBreakdown().find(([e])=>e===`Vibecoin mint`)?.[1]??6600n)/10000n,n,r,i)}if(this.target===`vibecoin`){let e=this.vibecoinAddress?.toLowerCase()??``;return e?Qp(t,H.get(`token:${e}:totalSupply`)??0n,H.get(`token:${e}:P0`)??0n,H.get(`token:${e}:K`)??0n):0n}return 0n}_computeTimerExtension(){let e=H.get(`game:settlementTimestamp`)??0n;if(e===0n)return`--`;let t=this._amountWei;if(t===0n)return`--`;let n=H.get(`game:totalSeriesA`)??0n,r=H.get(`game:totalSeriesB`)??0n,i=H.get(`game:totalPublicCurveDeposited`)??0n,a=H.get(`game:phaseEndTime`)??0n,o=Number(n+r)/0xde0b6b3a7640000;if(o===0)return`--`;let s=this.useShMon?t:t*10n**18n/Dd(),c=Number(i+s)/0xde0b6b3a7640000+1e4,l=o*2419200/(o+2e6)*c*c*1e3/(c*c+40*o*o),u=Number(e)*1e3+36e5+l,d=Math.max(0,u-Number(a)*1e3);if(d===0)return`at max`;let f=Math.floor(d/1e3),p=Math.floor(f/3600),m=Math.floor(f%3600/60);return p>0?`+${p}h ${m}m`:m>0?`+${m}m ${f}s`:`+${f}.${Math.floor(d)%1e3}s`}async _approve(){let e=F[P],t=z();if(!t||!L)return;let n=this._amountWei;if(n!==0n){this.approvePending=!0,this.error=``;try{let r=this.target===`vibecoin`?this.vibecoinAddress:e.game;await t.writeContract({address:e.shmon,abi:id,functionName:`approve`,args:[r,n],account:L.address,chain:t.chain}),this.approved=!0}catch(e){Sp(e)}finally{this.approvePending=!1}}}async _buy(){let e=F[P],t=z();if(!t||!L)return;let n=this._amountWei;if(n!==0n){if(this.target===`seriesA`||this.target===`seriesB`){let e=this.target===`seriesA`?0:1,t=H.get(`game:phase`);if(t===void 0||t!==e){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}let n=H.get(`game:phaseEndTime`)??0n,r=H.get(`meta:chainTimeDelta`)??0;if(n>0n&&Date.now()/1e3+r>=Number(n)){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}}this.pending=!0,this.error=``;try{let r=Yd(Gd()??``),i;i=this.target===`vibecoinSell`?await t.writeContract({address:this.vibecoinAddress,abi:V,functionName:`sell`,args:[n,L.address],account:L.address,chain:t.chain}):this.useShMon?this.target===`seriesA`?await t.writeContract({address:e.game,abi:B,functionName:`depositSeriesAWithShMon`,args:[r,n],account:L.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:B,functionName:`depositSeriesBWithShMon`,args:[r,n],account:L.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:B,functionName:`buyWithShMon`,args:[r,n],account:L.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:V,functionName:`buyWithShMon`,args:[L.address,n,!0],account:L.address,chain:t.chain}):this.target===`seriesA`?await t.writeContract({address:e.game,abi:B,functionName:`depositSeriesA`,args:[r],value:n,account:L.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:B,functionName:`depositSeriesB`,args:[r],value:n,account:L.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:B,functionName:`buy`,args:[r],value:n,account:L.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:V,functionName:`buy`,args:[L.address,!0],value:n,account:L.address,chain:t.chain}),await Promise.all([G.refresh(),G.refreshUser()]),this.target===`vibecoinSell`&&this.vibecoinAddress&&G.refreshVcBal(this.vibecoinAddress),this._close(),xp(i,this._actionLabel())}catch(e){Sp(e)}finally{this.pending=!1}}}_actionLabel(){let e=this.useShMon?kd(this._amountWei):Ad(this._amountWei);switch(this.target){case`seriesA`:return`${e} insider deposit`;case`seriesB`:return`${e} "insider" deposit`;case`publicBuy`:return`Spend ${e} pumping ${this._vcName}`;case`vibecoin`:return`Spend ${e} on ${this._vcSymbol}`;case`vibecoinSell`:return`Sell ${Nd(this._amountWei,this._vcSymbol)} for shMON`}}_renderPreview(){let e=this._amountWei;if(this.target===`vibecoinSell`)return q`
        <div class="preview">
      ...
      `;let t=this._getFeeBreakdown(),n=this._computeTokenPreview(),r=this.target===`vibecoin`?10000n:t.find(([e])=>e===`Vibecoin`)?.[1]??6600n,i=this.target===`vibecoin`?e:e*r/10000n,a=n>0n?i*10n**18n/n:0n,o=this.target===`seriesA`||this.target===`seriesB`,s=this._refCode,c=n*(this.target===`vibecoin`?t.find(([e])=>e===`To buyer`)?.[1]??9000n:10000n)/10000n,l=e*(this.target===`vibecoin`?10000n:t.find(([e])=>e===`Vibecoins minted`||e===`Vibecoin mint`)?.[1]??6600n)/10000n,u=Dd(),d=this.useShMon?l:l*10n**18n/u,f=(this.target===`seriesA`?H.get(`game:totalSeriesA`)??0n:H.get(`game:totalSeriesB`)??0n)+d,p=d>0n&&f>0n?(Number(d*10000n/f)/100).toFixed(2):null,m=this.target===`seriesA`?`insider buy-in`:`"insider" buy-in`;return q`
      <div class="preview">
        ${o?q`
          <div class="preview-row">
        ...
        <div class="fee-section">
          ${t.map(([t,r])=>{let i=this.target===`vibecoin`&&n>0n?` · ${Nd(n*r/10000n,this._vcSymbol)}`:this.target!==`vibecoin`&&e>0n?` · ${this.useShMon?jd(e*r/10000n):Ad(e*r/10000n)}`:``;return q`
              <div class="preview-row">
      ...
      `:``}
    `}render(){if(!this.visible)return q``;let e=this.target===`vibecoinSell`,t=!e&&this.useShMon,n=!!L,r=this.target===`seriesA`||this.target===`seriesB`,i=e?this._tokenBalance:0n;return q`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="modal">
          ...
          <div class="field">
            <label style="display:flex;justify-content:space-between">
              ...
              ${e&&i>0n?q`<span style="color:#d8d8e8">BAL: ${Nd(i,this._vcSymbol)}</span>`:``}
            </label>
    ...
    `}};$([Q()],$p.prototype,`visible`,void 0),$([Q()],$p.prototype,`_loading`,void 0),$([Q()],$p.prototype,`target`,void 0),$([Q()],$p.prototype,`vibecoinAddress`,void 0),$([Q()],$p.prototype,`amountStr`,void 0),$([Q()],$p.prototype,`useShMon`,void 0),$([Q()],$p.prototype,`pending`,void 0),$([Q()],$p.prototype,`approvePending`,void 0),$([Q()],$p.prototype,`approved`,void 0),$([Q()],$p.prototype,`error`,void 0),$([Q()],$p.prototype,`_sellShMonPreview`,void 0),$([Q()],$p.prototype,`_tokenBalance`,void 0),$p=$([X(`buy-dialog`)],$p);var em=`disclaimer-accepted-v1`,tm=class extends Y{constructor(...e){super(...e),this.open=!localStorage.getItem(em)}_accept(){localStorage.setItem(em,`1`),this.open=!1}static{this.styles=K`
    .overlay {
  ...
  `}_renderContent(){return q`
    ...
    `}};$([Q()],tm.prototype,`open`,void 0),tm=$([X(`disclaimer-modal`)],tm);var nm=class extends Y{constructor(...e){super(...e),this.tab=`pump`,this._handleHash=()=>{Jd();let e=window.location.hash.split(`?`)[0];e===`#vibecoins`||e.startsWith(`#vibecoins/`)?this.tab=`vibecoins`:this.tab=`pump`},this._onWalletChanged=()=>{G.refreshUser(),this.requestUpdate()},this._onOpenBuyDialog=e=>{let t=this.renderRoot?.querySelector(`buy-dialog`);t&&typeof t.open==`function`&&t.open(e.detail)}}static{this.styles=K`
    :host {
      display: block;
      min-height: 100vh;
    }
  ...
  `}connectedCallback(){super.connectedCallback(),this._handleHash(),window.addEventListener(`hashchange`,this._handleHash),I.addEventListener(`wallet-changed`,this._onWalletChanged),window.addEventListener(`open-buy-dialog`,this._onOpenBuyDialog),G.start(),G.refreshUser()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._handleHash),I.removeEventListener(`wallet-changed`,this._onWalletChanged),window.removeEventListener(`open-buy-dialog`,this._onOpenBuyDialog)}render(){return q`
    ...
    `}};$([Q()],nm.prototype,`tab`,void 0),nm=$([X(`app-root`)],nm);function rm(e){let t=e instanceof Error?e.message:String(e),n=e instanceof Error?e.stack??e.message:String(e);window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:t,full:n,name:`UNCAUGHT ERROR`}}))}window.addEventListener(`error`,e=>rm(e.error??e.message)),window.addEventListener(`unhandledrejection`,e=>rm(e.reason));