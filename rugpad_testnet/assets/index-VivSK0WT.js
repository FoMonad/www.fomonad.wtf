...
`)[0]||String(e),r=typeof t?.message==`string`&&t.message||String(e),i=typeof t?.name==`string`&&t.name!==`Error`&&t.name||`ERROR`;window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:n,full:r,name:i}}))}var fp=class extends J{constructor(...e){super(...e),this.open=!1,this.hash=null,this.action=``,this.transfers=null,this.errorMsg=``,this.errorFull=``,this.errorName=``,this._onError=e=>{this.errorMsg=e.detail.short,this.errorFull=e.detail.full,this.errorName=e.detail.name,this.hash=null,this.open=!0},this._onShow=e=>{this.hash=e.detail.hash,this.action=e.detail.action,this.transfers=null,this.open=!0,this._fetchTransfers(e.detail.hash)}}connectedCallback(){super.connectedCallback(),window.addEventListener(`tx-confirm-show`,this._onShow),window.addEventListener(`app-error-show`,this._onError)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`tx-confirm-show`,this._onShow),window.removeEventListener(`app-error-show`,this._onError)}async _fetchTransfers(e){let{connectedWallet:t}=await Jt(async()=>{let{connectedWallet:e}=await Promise.resolve().then(()=>Lu);return{connectedWallet:e}},void 0,import.meta.url);if(!t){this.transfers=[];return}let n=L[I],r=z(I),i=t.address.slice(2).toLowerCase(),a;for(let t=0;t<20;t++){try{if(a=await r.getTransactionReceipt({hash:e}),a)break}catch{}await new Promise(e=>setTimeout(e,800))}if(!a){this.transfers=[];return}W.refresh(),W.refreshUser(),setTimeout(()=>{W.refresh(),W.refreshUser()},1500);let o=a.logs.filter(e=>e.topics[0]?.toLowerCase()===cp&&e.topics[2]?.slice(-40).toLowerCase()===i);if(o.length===0){this.transfers=[];return}let s=[];for(let e of o){let t=e.address,i=`0x`+(e.topics[1]??`0x`).slice(-40),a=BigInt(e.data),o;if(t.toLowerCase()===n.fomon.toLowerCase())o=`foMON`;else if(t.toLowerCase()===n.shmon.toLowerCase())o=`shMON`;else{let e=W.vibecoins.find(e=>e.address.toLowerCase()===t.toLowerCase());if(e)o=e.symbol;else try{o=await r.readContract({address:t,abi:H,functionName:`symbol`})}catch{o=t.slice(0,6)+`...`}}let c=i.toLowerCase()===lp&&t.toLowerCase()===n.fomon.toLowerCase();s.push({symbol:o,amount:a,isAirdrop:c})}this.transfers=s}_close(){this.open=!1}_breakPascal(e){return e.replace(/([a-z])([A-Z])/g,`$1​$2`)}async _copy(){this.hash&&await navigator.clipboard.writeText(this.hash)}static{this.styles=G`
  ...
  `}render(){if(!this.open)return K``;if(!this.hash)return K`
    ...
    `;let e=L[I],t=this.hash.slice(0,10)+`…`+this.hash.slice(-6),n=e.explorer?`${e.explorer}/tx/${this.hash}`:null;return K`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="dialog">
          <button class="close" @click=${this._close}>×</button>
          ...
          ${this.transfers===null||this.transfers.length>0?K`
            ...
            ${this.transfers===null?K`<div class="loading"><div class="spinner"></div>awaiting confirmation…</div>`:this.transfers.map(e=>K`
                <div class="transfer">
                  ...
                  <span class="t-amount">${Sd(e.amount)}</span>
    ...
    `}};Z([X()],fp.prototype,`open`,void 0),Z([X()],fp.prototype,`hash`,void 0),Z([X()],fp.prototype,`action`,void 0),Z([X()],fp.prototype,`transfers`,void 0),Z([X()],fp.prototype,`errorMsg`,void 0),Z([X()],fp.prototype,`errorFull`,void 0),Z([X()],fp.prototype,`errorName`,void 0),fp=Z([Y(`tx-confirm-dialog`)],fp);var pp=`fomonad-fomon-visited`,Q=class extends J{constructor(...e){super(...e),this.open=!1,this.providerOpen=!1,this.referralCode=``,this.activeRefInput=``,this.registerPending=!1,this.claimPending=!1,this.confirmReg=!1,this.regEligError=!1,this.copiedRef=!1,this.copied=!1,this.denomPref=pd(),this._onWalletChanged=()=>this.requestUpdate(),this._onStoreUpdated=()=>this.requestUpdate(),this._onDocClick=e=>{e.composedPath().includes(this)||(this.open=!1,this.providerOpen=!1)}}static{this.styles=G`
  ...
  `}connectedCallback(){super.connectedCallback(),zu.addEventListener(`wallet-changed`,this._onWalletChanged),window.addEventListener(`store-updated`,this._onStoreUpdated),document.addEventListener(`click`,this._onDocClick)}disconnectedCallback(){super.disconnectedCallback(),zu.removeEventListener(`wallet-changed`,this._onWalletChanged),window.removeEventListener(`store-updated`,this._onStoreUpdated),document.removeEventListener(`click`,this._onDocClick)}async _connect(e){await Uu(e),Gu(e.info.rdns),await Xu(I),this.providerOpen=!1,this.requestUpdate()}_disconnect(){Wu(),Ku(),this.open=!1}_showNotifDot(){let e=W.userState;if(!e)return!1;if(e.fomonClaimable>0n)return!0;let t=localStorage.getItem(pp);return e.fomonBalance>0n&&!t}async _copyAddress(){R&&(await navigator.clipboard.writeText(R.address),this.copied=!0,setTimeout(()=>{this.copied=!1},1500))}async _claimFoMon(){let e=L[I],t=B();if(!(!t||!R)){this.claimPending=!0;try{let n=await t.writeContract({address:e.fomon,abi:Zu,functionName:`claim`,account:R.address,chain:t.chain});await W.refreshUser(),up(n,`Claimed foMON dividends`)}catch(e){dp(e)}finally{this.claimPending=!1}}}async _optIn(){let e=L[I],t=B();if(!(!t||!R))try{let n=await t.writeContract({address:e.fomon,abi:Zu,functionName:`optIn`,account:R.address,chain:t.chain});await W.refreshUser(),up(n,`Opted in to foMON dividends`)}catch(e){dp(e)}}async _checkEligAndConfirm(){if(!this.referralCode||!R)return;let e=L[I],t=z();try{await t.simulateContract({address:e.game,abi:V,functionName:`registerReferral`,args:[Od(this.referralCode)],account:R.address}),this.regEligError=!1,this.confirmReg=!0}catch{this.regEligError=!0}}async _registerReferral(){if(!this.referralCode||!this.confirmReg)return;let e=L[I],t=B();if(!(!t||!R)){this.registerPending=!0,this.confirmReg=!1;try{let n=Od(this.referralCode),r=await t.writeContract({address:e.game,abi:V,functionName:`registerReferral`,args:[n],account:R.address,chain:t.chain}),i=this.referralCode;this.referralCode=``,up(r,`Registered referral code "${i}"`)}catch(e){dp(e)}finally{this.registerPending=!1}}}async _copyRefUrl(e){await navigator.clipboard.writeText(`https://fomon.rip/${e}`),this.copiedRef=!0,setTimeout(()=>{this.copiedRef=!1},1500)}_setDenom(e){md(e),this.denomPref=e,window.dispatchEvent(new Event(`store-updated`))}render(){if(!R){let e=Bu;return e.length===0?K`<button class="connect-btn" style="cursor:default;opacity:0.4">NO WALLET</button>`:e.length===1?K`
      ...
      `}let e=W.userState,t=this._showNotifDot();return K`
      <div style="position:relative">
        <button
          class="address-btn ${this.open?`active`:``}"
          @click=${()=>{this.open=!this.open,this.open&&localStorage.setItem(pp,`1`)}}
        >
          ${t?K`<span class="notif-dot"></span>`:``}
          ${this.copied?K`<span class="copied-flash">COPIED!</span>`:K`<span>${Cd(R.address)}</span>`}
        ...
        ${this.open?K`
          <div class="dropdown">
            ...
            <div class="row">
              ...
              <span class="val">${e?yd(e.monBalance):`…`}</span>
            </div>
            <div class="row">
              <span class="lbl">shMON${L[10143].shmonUrl?K`&nbsp;<a href="${L[I].shmonUrl}" target="_blank" rel="noopener" title="redeem shMON for MON">Redeem ↗</a>`:``}</span>
              <span class="val">${e?vd(e.shmonBalance):`…`}</span>
            </div>
            <div class="row">
            ...
            ${e&&e.fomonClaimable>0n?K`
              <div class="row" style="margin-top:6px">
                <span class="lbl">PENDING shMON</span>
                <span class="val accent">${U(e.fomonClaimable)}</span>
              </div>
              <div class="row" style="justify-content:flex-end">
            ...
            <div>
              ...
              <div style="display:flex;gap:6px">
                <input
                  class="ref-input"
                  placeholder=${Md()??`enter code to use`}
                  .value=${this.activeRefInput}
                  @input=${e=>{this.activeRefInput=e.target.value}}
                />
                <button
                  class="action-btn"
                  @click=${()=>{this.activeRefInput&&(Nd(this.activeRefInput),this.activeRefInput=``,this.requestUpdate())}}
            ...
            <div>
              ...
              `:K`
                ...
                `}
              `}
              ${e&&e.referralEarned>0n?K`
              <div class="row">
                <span class="lbl">REFERRAL EARNED</span>
                <span class="val gold">${U(e.referralEarned)}</span>
              </div>
            `:``}
            </div>
    ...
    `}};Z([X()],Q.prototype,`open`,void 0),Z([X()],Q.prototype,`providerOpen`,void 0),Z([X()],Q.prototype,`referralCode`,void 0),Z([X()],Q.prototype,`activeRefInput`,void 0),Z([X()],Q.prototype,`registerPending`,void 0),Z([X()],Q.prototype,`claimPending`,void 0),Z([X()],Q.prototype,`confirmReg`,void 0),Z([X()],Q.prototype,`regEligError`,void 0),Z([X()],Q.prototype,`copiedRef`,void 0),Z([X()],Q.prototype,`copied`,void 0),Z([X()],Q.prototype,`denomPref`,void 0),Q=Z([Y(`wallet-widget`)],Q);var mp=class extends J{constructor(...e){super(...e),this.currentHash=window.location.hash||`#pump`,this._onHash=()=>{this.currentHash=window.location.hash||`#pump`}}static{this.styles=G`
    :host {
      display: block;
    }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`hashchange`,this._onHash)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._onHash)}render(){return K`
    ...
    `}};Z([X()],mp.prototype,`currentHash`,void 0),mp=Z([Y(`header-bar`)],mp);var hp=class extends J{constructor(...e){super(...e),this.claimPendingA=!1,this.claimPendingB=!1,this.tokensAllocA=0n,this.tokensAllocB=0n,this._lastSettledCycle=-1n,this._tickInterval=0,this._onUpdate=()=>{let e=W.gameState,t=e?zd(e):0;(t===2||t===3)&&e&&e.cycle!==this._lastSettledCycle&&(this._lastSettledCycle=e.cycle,this._fetchAllocations(e.cycle)),this.requestUpdate()}}static{this.styles=G`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onUpdate),this._tickInterval=window.setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onUpdate),clearInterval(this._tickInterval)}async _fetchAllocations(e){let t=L[I],n=z(I),r=t.game,[i,a]=await Promise.all([n.readContract({address:r,abi:V,functionName:`tokensSeriesAByCycle`,args:[e]}),n.readContract({address:r,abi:V,functionName:`tokensSeriesBByCycle`,args:[e]})]);this.tokensAllocA=i,this.tokensAllocB=a}_openBuy(e){window.dispatchEvent(new CustomEvent(`open-buy-dialog`,{detail:{target:e}}))}async _claimA(){let e=W.gameState,t=B();if(!(!t||!R||!e)){this.claimPendingA=!0;try{let n=L[I],r=await t.writeContract({address:n.game,abi:V,functionName:`claimSeriesATokens`,args:[e.cycle],account:R.address,chain:t.chain});await Promise.all([W.refresh(),W.refreshUser()]),up(r,`Claimed Insider Vibecoins`)}catch(e){console.error(`Claim A error:`,e)}finally{this.claimPendingA=!1}}}async _claimB(){let e=W.gameState,t=B();if(!(!t||!R||!e)){this.claimPendingB=!0;try{let n=L[I],r=await t.writeContract({address:n.game,abi:V,functionName:`claimSeriesBTokens`,args:[e.cycle],account:R.address,chain:t.chain});await Promise.all([W.refresh(),W.refreshUser()]),up(r,`Claimed "Insider" Vibecoins`)}catch(e){console.error(`Claim B error:`,e)}finally{this.claimPendingB=!1}}}_phaseStatus(e,t){let n=Bd(e,t);if(n===0n)return t===0?`DEPOSIT TO KICK OFF (0.1 shMON needed)`:`PENDING`;let r=Number(n)-Ld();return r>0?`ENDS IN ${Math.floor(r/3600)}h ${Math.floor(r%3600/60)}m ${Math.floor(r%60)}s`:`ENDED`}render(){let e=W.gameState,t=W.userState,n=e?zd(e):0,r=n===0,i=n===1,a=e?W.vibecoins.find(t=>t.address.toLowerCase()===e.currentVibecoin.toLowerCase())?.symbol??`?`:`?`,o=t?t.seriesADeposit>0n:!1,s=t?t.seriesBDeposit>0n:!1,c=n===2||n===3,l=e?.totalSeriesA??0n,u=e?.totalSeriesB??0n,d=c&&o&&this.tokensAllocA>0n&&l>0n?this.tokensAllocA*t.seriesADeposit/l:0n,f=c&&s&&this.tokensAllocB>0n&&u>0n?this.tokensAllocB*t.seriesBDeposit/u:0n;return K`
      <div class="cards">
        <!-- Series A -->
        <div class="card ${n>0&&!c?`disabled`:``}">
          ...
          <div class="row">
            ...
            <span class="val">${e?U(e.totalSeriesA):`--`}</span>
          </div>
          ${t?K`
            <div class="row">
              <span class="key">YOUR DEPOSIT</span>
              <span class="val ${o?`highlight`:``}">${U(t.seriesADeposit)} (${Td(t.seriesADeposit,e?.totalSeriesA||0n)})</span>
            </div>
          `:``}
          <div class="row">
          ...
          ${c&&o?K`
            <div class="row">
              ...
              <span class="val highlight">${d>0n?xd(d,a):`…`}</span>
            </div>
          `:``}
        ...
        <div class="card ${n<1?`disabled`:``}">
          ...
          <div class="row">
            <span class="key">TOTAL DEPOSITED</span>
            <span class="val">${e?U(e.totalSeriesB):`--`}</span>
          </div>
          ${t?K`
            <div class="row">
              <span class="key">YOUR DEPOSIT</span>
              <span class="val ${s?`highlight`:``}">${U(t.seriesBDeposit)} (${Td(t.seriesBDeposit,e?.totalSeriesB||0n)})</span>
            </div>
          `:``}
          <div class="row">
            <span class="key">STATUS</span>
          ...
          ${c&&s?K`
            <div class="row">
              <span class="key">YOUR VIBECOINS</span>
              <span class="val highlight">${f>0n?xd(f,a):`…`}</span>
            </div>
          `:``}
    ...
    `}};Z([X()],hp.prototype,`claimPendingA`,void 0),Z([X()],hp.prototype,`claimPendingB`,void 0),Z([X()],hp.prototype,`tokensAllocA`,void 0),Z([X()],hp.prototype,`tokensAllocB`,void 0),hp=Z([Y(`seed-section`)],hp);var gp=class extends J{constructor(...e){super(...e),this.vcAddress=`0x0000000000000000000000000000000000000000`,this.earned=0n,this.optedIn=!1,this.vcBalance=0n,this.claimPending=!1,this.optPending=!1,this.seriesASupply=0n,this.seriesBSupply=0n,this.athSupply=0n,this.athIsPostRug=!1,this._onUpdate=()=>{this.requestUpdate(),this._loadDividendState()}}static{this.styles=G`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onUpdate),this._loadDividendState()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onUpdate)}async _loadDividendState(){let e=R;if(!(!e||!this.vcAddress||this.vcAddress===`0x0000000000000000000000000000000000000000`))try{let t=z(I),n=L[I],r=W.vibecoins.find(e=>e.address.toLowerCase()===this.vcAddress.toLowerCase()),i=r?BigInt(r.cycle):W.gameState?.cycle,[a,o,s,c,l,u,d]=await Promise.all([t.readContract({address:this.vcAddress,abi:H,functionName:`earned`,args:[e.address]}),t.readContract({address:this.vcAddress,abi:H,functionName:`optedIn`,args:[e.address]}),t.readContract({address:this.vcAddress,abi:H,functionName:`balanceOf`,args:[e.address]}),i===void 0?Promise.resolve(0n):t.readContract({address:n.game,abi:V,functionName:`tokensSeriesAByCycle`,args:[i]}),i===void 0?Promise.resolve(0n):t.readContract({address:n.game,abi:V,functionName:`tokensSeriesBByCycle`,args:[i]}),t.readContract({address:this.vcAddress,abi:H,functionName:`athPrice`}),t.readContract({address:this.vcAddress,abi:H,functionName:`athPostRug`})]);this.earned=a,this.optedIn=o,this.vcBalance=s,this.seriesASupply=c,this.seriesBSupply=l,u>0n&&r&&r.K>0n?(this.athSupply=u>BigInt(r.P0)?(u-BigInt(r.P0))*10n**18n/BigInt(r.K):0n,this.athIsPostRug=d):(this.athSupply=0n,this.athIsPostRug=!1)}catch{}}async _optToggle(){let e=B();if(!(!e||!R)){this.optPending=!0;try{let t=this.optedIn,n=t?`optOut`:`optIn`;up(await e.writeContract({address:this.vcAddress,abi:H,functionName:n,account:R.address,chain:e.chain}),t?`Opted out of vibes`:`Opted in to vibes`),await this._loadDividendState()}catch(e){dp(e)}finally{this.optPending=!1}}}async _claimDividends(){let e=B();if(!(!e||!R)){this.claimPending=!0;try{let t=await e.writeContract({address:this.vcAddress,abi:H,functionName:`claimDividends`,args:[R.address],account:R.address,chain:e.chain});await this._loadDividendState(),up(t,`Claimed vibecoin dividends`)}catch(e){console.error(`Claim dividends error:`,e)}finally{this.claimPending=!1}}}_renderChart(e){let t=e.P0,n=e.K,r=e.totalSupply,i=r*3n/2n||10n**18n,a=10n**18n,o=[],s=[];for(let e=0;e<=60;e++){let r=i*BigInt(e)/BigInt(60),c=t+n*r/a;o.push(Number(r)/0xde0b6b3a7640000),s.push(Number(c)/1e9)}let c=o[o.length-1]||1,l=Math.max(...s)||1,u=e=>38+e/c*262,d=e=>66-e/l*66,f=o.map((e,t)=>`${t===0?`M`:`L`}${u(e).toFixed(1)},${d(s[t]).toFixed(1)}`).join(` `),p=Math.round(u(Number(r)/0xde0b6b3a7640000)),m=Math.round(u(Number(this.athSupply))),h=this.seriesASupply>0n?u(Number(this.seriesASupply)/0xde0b6b3a7640000):null,g=this.seriesASupply>0n&&this.seriesBSupply>0n?u(Number(this.seriesASupply+this.seriesBSupply)/0xde0b6b3a7640000):null,_=e=>String(e).split(``).map(e=>`₀₁₂₃₄₅₆₇₈₉`[+e]).join(``),v=e=>{let t=e/1e9;if(t>=.001)return t.toPrecision(3);let n=t.toFixed(30).split(`.`)[1]??``,r=0;for(let e of n)if(e===`0`)r++;else break;let i=n.slice(r,r+3);return`0.0${_(r-1)}${i}`};return console.log(`AAAAAAAAAAAA`,this.athSupply,r),K`
      <div class="chart-area">
        <svg viewBox="0 0 ${300} ${80}" style="width:100%;height:${84}px" font-family="'JetBrains Mono',monospace" font-size="10">
          ...
          <!-- ATH/rug supply -->
          ${this.athSupply>0n&&Math.abs(m-p)>1?Bf`
    ...
    `}render(){let e=W.gameState,t=W.userState,n=this.vcAddress;if(!n||n===`0x0000000000000000000000000000000000000000`)return K`<div style="color:#3a3a4a;font-family:monospace;font-size:11px">No vibecoin active</div>`;let r=W.vibecoins.find(e=>e.address.toLowerCase()===n.toLowerCase()),i=r?r.P0+r.K*r.totalSupply/10n**18n:0n,a=r&&t&&this.vcBalance>0n?i*this.vcBalance/10n**18n:0n,o=r?.unlocked??!1;return K`
      <div class="panel">
        <div class="row">
        ...
        <div class="row">
          ...
          <span class="val">${r?xd(r.totalSupply,r.symbol):`--`}</span>
        </div>
        ${t?K`
          <div class="row">
            <span class="key">YOUR BALANCE</span>
            <span class="val">${xd(this.vcBalance,r?.symbol||`vibecoins`)} (${Td(this.vcBalance,r?.totalSupply||0n)}, ${U(a)})</span>
          </div>
        `:``}
        ...
        <div class="dividends">
          ...
          <span class="div-val">${xd(this.earned,r?.symbol??``)}</span>
          <button
            class="claim-btn"
    ...
    `}};Z([sp({type:String})],gp.prototype,`vcAddress`,void 0),Z([X()],gp.prototype,`earned`,void 0),Z([X()],gp.prototype,`optedIn`,void 0),Z([X()],gp.prototype,`vcBalance`,void 0),Z([X()],gp.prototype,`claimPending`,void 0),Z([X()],gp.prototype,`optPending`,void 0),Z([X()],gp.prototype,`seriesASupply`,void 0),Z([X()],gp.prototype,`seriesBSupply`,void 0),Z([X()],gp.prototype,`athSupply`,void 0),Z([X()],gp.prototype,`athIsPostRug`,void 0),gp=Z([Y(`launch-section`)],gp);var _p=class extends J{constructor(...e){super(...e),this.forceSettled=!1,this.timerMs=0,this.triggerPending=!1,this.prevWinner=null,this.prevPrize=0n,this.rugpullerCode=null,this._rugpullerAddrFetched=``,this.timerInterval=null,this._onUpdate=()=>{this.forceSettled&&this._loadPrevWinner();let e=W.gameState?.lastBuyer??``;e&&e!==this._rugpullerAddrFetched&&this._fetchRugpullerCode(e),this.requestUpdate()}}static{this.styles=G`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onUpdate),this.timerInterval=setInterval(()=>this._tick(),50),this.forceSettled&&this._loadPrevWinner()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onUpdate),this.timerInterval&&clearInterval(this.timerInterval)}async _fetchRugpullerCode(e){this._rugpullerAddrFetched=e;try{let t=z(I),n=L[I];this.rugpullerCode=kd(await t.readContract({address:n.game,abi:V,functionName:`referralCodeOf`,args:[e]}))}catch{this.rugpullerCode=null}}async _loadPrevWinner(){let e=W.gameState;if(!e||e.cycle<2n)return;let t=e.cycle-1n;try{let e=z(I),n=L[I],r=await e.readContract({address:n.game,abi:V,functionName:`winnerByCycle`,args:[t]}),i=await e.readContract({address:n.game,abi:V,functionName:`prizeByCycle`,args:[t]});this.prevWinner=r,this.prevPrize=i}catch{}}_tick(){let e=W.gameState;if(!e||e.timerExpiry===0n){this.timerMs=0;return}this.timerMs=Math.max(0,Number(e.timerExpiry)-Rd()),this.requestUpdate()}async _triggerSettlement(){let e=B();if(!(!e||!R)){this.triggerPending=!0;try{let t=L[I],n=await e.writeContract({address:t.game,abi:V,functionName:`triggerSettlement`,account:R.address,chain:e.chain});await W.refresh(),up(n,`Rugpull triggered — settlement in progress`)}catch(e){dp(e)}finally{this.triggerPending=!1}}}render(){let e=W.gameState;if(!e)return K``;let t=e.phase,n=e.isTimerExpired||this.timerMs===0,r=!n&&this.timerMs<300*1e3,i=t===3||this.forceSettled,a=`0x0000000000000000000000000000000000000000`,o=this.forceSettled?this.prevWinner??a:e.lastBuyer,s=this.forceSettled?this.prevPrize:e.prizePool,c=s*90n/100n,l=s-c;return K`
      <div class="panel">
        ${i?K`
        ...
        `:K`
          <div class="rugpuller-block">
            <div class="rugpuller-label">Current Rugpuller${this.rugpullerCode?K` · <span style="color:#ffe600">${this.rugpullerCode}</span>`:``}</div>
            ${L[10143].explorer?K`
              <a class="rugpuller-addr" href="${L[I].explorer}/address/${o}" target="_blank">
                <span class="addr-short">${Cd(o)}</span>
          ...
          <div class="row">
            ...
            <span class="val">${U(c)}</span>
          </div>
          <div class="cut-note">${U(l)} rolls to next cycle</div>
        `}
    ...
    `}};Z([sp({type:Boolean})],_p.prototype,`forceSettled`,void 0),Z([X()],_p.prototype,`timerMs`,void 0),Z([X()],_p.prototype,`triggerPending`,void 0),Z([X()],_p.prototype,`prevWinner`,void 0),Z([X()],_p.prototype,`prevPrize`,void 0),Z([X()],_p.prototype,`rugpullerCode`,void 0),_p=Z([Y(`rugpull-section`)],_p);var vp=`ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789`;function yp(){let e=3+Math.floor(Math.random()*4);return Array.from({length:e},()=>vp[Math.floor(Math.random()*36)]).join(``)}var bp=`GENERATING VIBECOIN….BOOTSTRAPPING CONSENSUS….ALIGNING INCENTIVES….MANIFESTING UTILITY….DEPLOYING HOPIUM….CONSULTING THE VIBE ORACLE….LAUNDERING DEGENERACY….DECENTRALIZING SOMETHING….LOADING FUTURE BAGS….SYNCING WITH BLOCKCHAIN™….SOURCING GREATER FOOLS….CALIBRATING RUG TENSION….INFLATING TOKEN METRICS….FABRICATING ROADMAP….ONBOARDING EXIT LIQUIDITY….COMPILING VIBES….TRAINING TOKENOMICS AI….SCHEDULING INFLUENCER POSTS….AUDITING THE UNAUDITABLE….PLAGIARIZING WHITEPAPER….SEEDING TELEGRAM GROUP….RANDOMIZING 'INSURANCE FUND'….GENERATING PLAUSIBLE DENIABILITY….STRESS-TESTING EXIT STRATEGY….TOKENIZING NOTHING….BOOTSTRAPPING ORGANIC COMMUNITY….ACTIVATING ANONYMOUS FOUNDERS….APPLYING GIRLMATH…`.split(`.`),xp=[{label:`AI Tokens Consumed`,value:e=>e.tokensConsumed.toLocaleString()},{label:`Exit Liquidity Manifested`,value:e=>`${e.exitLiquidity.toLocaleString()} wallets`},{label:`Hopium Injected`,value:e=>`${e.hopium} cc`},{label:`Bag Holders Hallucinated`,value:e=>e.bagHolders.toLocaleString()},{label:`Rug Threads Woven`,value:e=>`${e.rugThreads.toFixed(1)}%`},{label:`Utility`,value:()=>`undefined`},{label:`Financial Advisors Ignored`,value:()=>`1`},{label:`Diamond Hands Detected`,value:()=>`0`},{label:`NFT Royalties Waived`,value:()=>`0`},{label:`Regulatory Gray Areas Exploited`,value:e=>String(e.grayAreas)},{label:`Market Makers Angered`,value:e=>String(e.marketMakers)},{label:`Decentralization Theater`,value:()=>`STAGING`},{label:`Plausible Deniability`,value:e=>`${e.plausibleDeniability}%`},{label:`Whitepaper Pages`,value:()=>`0`},{label:`Legal Opinions Sought`,value:()=>`0`},{label:`Liquidity Lock`,value:()=>`awaiting rugpull`},{label:`Net Cost After Girlmath`,value:()=>`$0 💖`}];function Sp(){return{tokensConsumed:Math.floor(Math.random()*45e5)+5e5,exitLiquidity:Math.floor(Math.random()*15e3)+5e3,hopium:Math.floor(Math.random()*700)+100,bagHolders:Math.floor(Math.random()*2e3)+500,rugThreads:60+Math.random()*25,grayAreas:Math.floor(Math.random()*3)+1,marketMakers:Math.floor(Math.random()*2)+1,plausibleDeniability:Math.floor(Math.random()*40)+60}}var Cp=class extends J{constructor(...e){super(...e),this.tokenUri=``,this.lastVibecoin=`0x0000000000000000000000000000000000000000`,this._scrambledTicker=yp(),this._headingIdx=0,this._statIdx=0,this._genStats=Sp(),this.settlePending=!1,this._tickCount=0,this._genInterval=null,this._onStoreUpdated=()=>{let e=W.gameState,t=e?.cycle??0n,n=e?.seriesAEnd?Number(e.seriesAEnd)-Ld():1/0,r=e&&e.phase===0&&t>1n&&(e.seriesAEnd===0n||n>Number(W.gameConstants?.SERIES_A_DURATION??7200n)/2)?(W.vibecoins.find(e=>e.cycle===Number(t)-1)??W.vibecoins[0])?.address??null:e?.currentVibecoin&&e.currentVibecoin!==`0x0000000000000000000000000000000000000000`?e.currentVibecoin:null;r&&r!==this.lastVibecoin&&(this.lastVibecoin=r,this._loadTokenUri(r)),this.requestUpdate()}}static{this.styles=G`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onStoreUpdated),this._genInterval=setInterval(()=>this._tickGen(),150)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onStoreUpdated),this._genInterval&&clearInterval(this._genInterval)}async _settle(){let e=B();if(!(!e||!R)){this.settlePending=!0;try{let t=L[I],n=await e.writeContract({address:t.game,abi:V,functionName:`settleSeriesB`,account:R.address,chain:e.chain});await W.refresh(),up(n,`Vibecoin generated — insider rounds settled`)}catch(e){dp(e)}finally{this.settlePending=!1}}}_tickGen(){this._scrambledTicker=yp(),this._tickCount++;let e=this._genStats;if(e.tokensConsumed+=Math.floor(Math.random()*14)+4,e.exitLiquidity+=Math.floor(Math.random()*8)+2,e.hopium+=+(Math.random()<.3),e.bagHolders+=Math.floor(Math.random()*6)+1,e.rugThreads=Math.min(99.7,e.rugThreads+Math.random()*.02),Math.random()<.02&&(e.grayAreas+=1),Math.random()<.01&&(e.marketMakers+=1),this._tickCount%20==0){let e=this._headingIdx;for(;e===this._headingIdx;)e=Math.floor(Math.random()*bp.length);this._headingIdx=e}if(this._tickCount%27==0){let e=this._statIdx;for(;e===this._statIdx;)e=Math.floor(Math.random()*xp.length);this._statIdx=e,(e===9||e===10)&&(this._genStats={...this._genStats,grayAreas:Math.floor(Math.random()*3)+1,marketMakers:Math.floor(Math.random()*2)+1})}}async _loadTokenUri(e){try{this.tokenUri=await z(I).readContract({address:e,abi:H,functionName:`tokenURI`})}catch{this.tokenUri=``}}_getSvgFromUri(){if(!this.tokenUri)return``;if(this.tokenUri.startsWith(`data:application/json`))try{let e=atob(this.tokenUri.split(`,`)[1]);return JSON.parse(e).image??``}catch{return``}return``}render(){let e=W.gameState;L[I];let t=`0x0000000000000000000000000000000000000000`,n=e?.cycle??0n,r=e?.seriesAEnd?Number(e.seriesAEnd)-Ld():1/0,i=!!(e&&e.phase===0&&n>1n&&(e.seriesAEnd===0n||r>Number(W.gameConstants?.SERIES_A_DURATION??7200n)/2)),a=i?W.vibecoins.find(e=>e.cycle===Number(n)-1)??W.vibecoins[0]??null:null,o=e?.currentVibecoin?W.vibecoins.find(t=>t.address.toLowerCase()===e.currentVibecoin.toLowerCase()):null,s=i?a:o,c=s?.palette[0]??`#00e5ff`,l=s?.palette[1]??`#0066ff`,u=e?zd(e):0,d=u===2||u===3,f=this._getSvgFromUri(),p=e?.currentVibecoin&&e.currentVibecoin!==t,m=!e||e.seriesAEnd===0n,h=!m&&!p,g=h&&!!e&&e.seriesBEnd>0n&&Ld()>=Number(e.seriesBEnd);return K`
      <div class="container">
        <div
          class="vibecoin-hero"
        ...
        >
          ${i?K`
            ...
            <div class="vc-ticker-row">
              ...
              ${a?K`<span class="round-badge">#${a.cycle}</span>`:``}
            </div>
            ${a?K`
              <div class="hero-stats">
                <div class="hero-stat">
                  <span class="hero-stat-val">${U(a.poolBalance)}</span>
                  <span class="hero-stat-label">Locked In</span>
                </div>
                <div class="hero-stat">
                  <span class="hero-stat-val">${U((a.P0+a.K*a.totalSupply/10n**18n)*a.totalSupply/10n**18n)}</span>
          ...
          `:h?K`
            <div class="vc-pending">
              ...
              <div class="vc-pending-phase">${g?`Vibecoin Ready`:bp[this._headingIdx]}</div>
              ...
              <div class="vc-pending-tokens">
                ...
                ${xp[this._statIdx].label}: <span class="count">${xp[this._statIdx].value(this._genStats)}</span>
              </div>
              ${g?K`
          ...
          `:K`
            ...
            ${L[10143].explorer&&o?K`
              <a class="vc-name" href="${L[I].explorer}/address/${o.address}" target="_blank">${o.name}</a>
            `:K`<div class="vc-name">${o?.name??`--`}</div>`}
            <div class="vc-ticker-row">
              <span class="vc-ticker">\$${o?.symbol??`--`}</span>
              ${o?K`<span class="round-badge">#${o.cycle}</span>`:``}
            </div>

            ${e&&o?K`
              <div class="hero-stats">
               <div class="hero-stat">
                  <span class="hero-stat-val">${U(o.poolBalance)}</span>
                  <span class="hero-stat-label">Locked In</span>
                </div>
                <div class="hero-stat">
                  <span class="hero-stat-val">${U((o.P0+o.K*o.totalSupply/10n**18n)*o.totalSupply/10n**18n)}</span>
                  <span class="hero-stat-label">Market Cap</span>
                </div>
                <div class="hero-stat">
                  <span class="hero-stat-val">${U(e.prizePool)}</span>
        ...
        <div class="sections">
          ...
          `:d&&p?K`
            ...
            <div>
              ...
              <launch-section .vcAddress=${e?.currentVibecoin??t}></launch-section>
            </div>
          `:``}
          <div>
    ...
    `}};Z([X()],Cp.prototype,`tokenUri`,void 0),Z([X()],Cp.prototype,`lastVibecoin`,void 0),Z([X()],Cp.prototype,`_scrambledTicker`,void 0),Z([X()],Cp.prototype,`_headingIdx`,void 0),Z([X()],Cp.prototype,`_statIdx`,void 0),Z([X()],Cp.prototype,`_genStats`,void 0),Z([X()],Cp.prototype,`settlePending`,void 0),Cp=Z([Y(`pump-tab`)],Cp);var $=class extends J{constructor(...e){super(...e),this.cycleId=0,this.tokenUri=``,this.uriLoaded=!1,this.userBalance=0n,this.seriesADeposit=0n,this.seriesBDeposit=0n,this.claimAPending=!1,this.claimBPending=!1,this.winner=null,this.prize=0n,this.winnerCode=null,this.observer=null,this._onUpdate=()=>{this.requestUpdate(),this._loadUserData()}}static{this.styles=G`
    :host { display: block; }
    ...
    .btn:disabled { opacity: 0.25; cursor: not-allowed; }
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onUpdate),this.observer=new IntersectionObserver(e=>{e[0].isIntersecting&&(this._loadTokenUri(),this._loadUserData(),this.observer?.disconnect())},{threshold:.1}),this.observer.observe(this)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onUpdate),this.observer?.disconnect()}async _loadTokenUri(){if(!this.uriLoaded)try{this.tokenUri=await z(I).readContract({address:this.vibecoin.address,abi:H,functionName:`tokenURI`}),this.uriLoaded=!0}catch{this.uriLoaded=!0}}async _loadUserData(){let e=R;if(e)try{let t=z(I),n=L[I],r=`0x0000000000000000000000000000000000000000`,i=BigInt(this.cycleId),[a,o,s,c,l]=await Promise.all([t.readContract({address:this.vibecoin.address,abi:H,functionName:`balanceOf`,args:[e.address]}),t.readContract({address:n.game,abi:V,functionName:`seriesADeposits`,args:[i,e.address]}),t.readContract({address:n.game,abi:V,functionName:`seriesBDeposits`,args:[i,e.address]}),t.readContract({address:n.game,abi:V,functionName:`winnerByCycle`,args:[i]}),t.readContract({address:n.game,abi:V,functionName:`prizeByCycle`,args:[i]})]);this.userBalance=a,this.seriesADeposit=o,this.seriesBDeposit=s,this.winner=c===r?null:c,this.prize=l,c&&c!==r&&(this.winnerCode=kd(await t.readContract({address:n.game,abi:V,functionName:`referralCodeOf`,args:[c]})))}catch{}}_getSvgFromUri(){if(!this.tokenUri)return``;if(this.tokenUri.startsWith(`data:application/json`))try{let e=atob(this.tokenUri.split(`,`)[1]);return JSON.parse(e).image??``}catch{return``}return``}async _claimA(){let e=B();if(!(!e||!R)){this.claimAPending=!0;try{let t=L[I];up(await e.writeContract({address:t.game,abi:V,functionName:`claimSeriesATokens`,args:[BigInt(this.cycleId)],account:R.address,chain:e.chain}),`Claimed Insider Vibecoins`),await this._loadUserData()}catch(e){dp(e)}finally{this.claimAPending=!1}}}async _claimB(){let e=B();if(!(!e||!R)){this.claimBPending=!0;try{let t=L[I];up(await e.writeContract({address:t.game,abi:V,functionName:`claimSeriesBTokens`,args:[BigInt(this.cycleId)],account:R.address,chain:e.chain}),`Claimed "Insider" Vibecoins`),await this._loadUserData()}catch(e){dp(e)}finally{this.claimBPending=!1}}}render(){let e=this.vibecoin,t=this._getSvgFromUri(),n=e.palette[0],r=e.palette[1],i=n+`14`,a=n+`2e`,o=n+`40`,s=n+`4d`,c=n+`66`,l=n+`99`,u=n+`2a`,d=e.P0+e.K*e.totalSupply/10n**18n,f=d*e.totalSupply/10n**18n,p=L[I].explorer;return K`
      ...
      ">
        ...
        <div class="info">
          ...
          <div class="stats">
            ...
            <span class="stat-val">${bd(d,e.symbol)}</span>
            <span class="stat-key">SUPPLY</span>
            <span class="stat-val">${xd(e.totalSupply,e.symbol)}</span>
            <span class="stat-key">MCAP</span>
            <span class="stat-val">${U(f)}</span>
            <span class="stat-key">TVL</span>
            <span class="stat-val">${U(e.poolBalance)}</span>
            <span class="stat-key">VOL 24H</span>
            <span class="stat-val">${U(e.volume24h)}</span>
            <span class="stat-key">VOL ALL</span>
            <span class="stat-val">${U(e.totalVolume)}</span>
            ${e.athPrice>0n?K`
              <span class="stat-key">${e.athPostRug?`ATH`:`RUG`} PRICE</span>
              <span class="stat-val">${bd(e.athPrice,e.symbol)}</span>
            `:``}

            ${R&&(this.userBalance>0n||this.winner)?K`
              <hr class="stat-divider" />
            `:``}

            ${R&&this.userBalance>0n?K`
              <span class="stat-key">YOUR BAL</span>
              <span class="stat-val own">${xd(this.userBalance,e.symbol)}</span>
            `:``}
            ${this.winner?K`
              <span class="stat-key">RUGGED</span>
              <span class="stat-val">
                ${U(this.prize)} to
                ${this.winnerCode?K`<span style="color:#ffe600">${this.winnerCode}</span>`:``}
                ${p?K`
                  <a style="color:#ff2d6b;text-decoration:none" href="${p}/address/${this.winner}" target="_blank">${Cd(this.winner)}</a>
                `:K`<span style="color:#ff2d6b">${Cd(this.winner)}</span>`}
              </span>
            `:``}
          </div>
        </div>
        ...
        <div class="btn-row">
          ...
          ${e.unlocked&&this.userBalance>0n?K`
    ...
    `}};Z([sp({attribute:!1})],$.prototype,`vibecoin`,void 0),Z([sp({type:Number})],$.prototype,`cycleId`,void 0),Z([X()],$.prototype,`tokenUri`,void 0),Z([X()],$.prototype,`uriLoaded`,void 0),Z([X()],$.prototype,`userBalance`,void 0),Z([X()],$.prototype,`seriesADeposit`,void 0),Z([X()],$.prototype,`seriesBDeposit`,void 0),Z([X()],$.prototype,`claimAPending`,void 0),Z([X()],$.prototype,`claimBPending`,void 0),Z([X()],$.prototype,`winner`,void 0),Z([X()],$.prototype,`prize`,void 0),Z([X()],$.prototype,`winnerCode`,void 0),$=Z([Y(`vibecoin-card`)],$);var wp=20,Tp=class extends J{constructor(...e){super(...e),this.search=``,this.visibleCount=wp,this.observer=null,this.sentinelEl=null,this._onUpdate=()=>this.requestUpdate(),this._onHashChange=()=>this._applyHash()}static{this.styles=G`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onUpdate),window.addEventListener(`hashchange`,this._onHashChange),this._applyHash()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onUpdate),window.removeEventListener(`hashchange`,this._onHashChange),this.observer?.disconnect()}_applyHash(){let e=window.location.hash.split(`?`)[0].match(/^#vibecoins\/(0x[0-9a-fA-F]{40})$/);e&&(this.search=e[1],this.visibleCount=wp)}updated(){let e=this.renderRoot?.querySelector(`.sentinel`);e&&e!==this.sentinelEl&&(this.observer?.disconnect(),this.sentinelEl=e,this.observer=new IntersectionObserver(e=>{e[0].isIntersecting&&(this.visibleCount+=wp)},{rootMargin:`200px`}),this.observer.observe(e))}_filtered(){let e=this.search.toLowerCase().trim();return e?W.vibecoins.filter(t=>t.name.toLowerCase().includes(e)||t.symbol.toLowerCase().includes(e)||t.address.toLowerCase().includes(e)):W.vibecoins}render(){let e=this._filtered(),t=e.slice(0,this.visibleCount),n=W.loading;return K`
      <div class="container">
        <div class="search-bar">
          <input
            ...
            .value=${this.search}
            @input=${e=>{this.search=e.target.value,this.visibleCount=wp}}
          />
        </div>

        ${n&&W.vibecoins.length===0?K`
          <div class="loading">
    ...
    `}};Z([X()],Tp.prototype,`search`,void 0),Z([X()],Tp.prototype,`visibleCount`,void 0),Tp=Z([Y(`vibecoins-tab`)],Tp);function Ep(e){if(e<0n)throw Error(`sqrt of negative`);if(e===0n)return 0n;let t=e,n=(t+1n)/2n;for(;n<t;)t=n,n=(t+e/t)/2n;return t}function Dp(e,t,n,r){let i=10n**18n,a=n*t/i+r*t*t/(2n*i*i)+e,o=Ep(n*n+2n*r*a);if(r===0n)return 0n;let s=(o-n)*i/r;return s>t?s-t:0n}var Op=class extends J{constructor(...e){super(...e),this.visible=!1,this.target=`publicBuy`,this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.approved=!1,this.error=``,this._sellShMonPreview=0n,this._tokenBalance=0n,this._vcSymbol=`token`,this._vcName=`BUY`,this._refCode=null,this._gameConstants=W.gameConstants}static{this.styles=G`
    :host { display: block; }
  ...
  `}open(e){this.target=e.target,this.vibecoinAddress=e.vibecoinAddress,this._tokenBalance=e.tokenBalance??0n;let t=e.vibecoinAddress??W.gameState?.currentVibecoin,n=W.vibecoins.find(e=>e.address.toLowerCase()===t?.toLowerCase());this._vcSymbol=n?.symbol??`token`,this._vcName=n?.name??``,this._refCode=Md(),this._gameConstants=W.gameConstants,this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.approved=!1,this.error=``,this._sellShMonPreview=0n,this.visible=!0}async _updateSellPreview(){let e=this._amountWei;if(e===0n||!this.vibecoinAddress){this._sellShMonPreview=0n;return}try{this._sellShMonPreview=await z(I).readContract({address:this.vibecoinAddress,abi:H,functionName:`shMonForTokens`,args:[e]})}catch{this._sellShMonPreview=0n}}_close(){this.visible=!1}get _amountWei(){try{let e=parseFloat(this.amountStr);return isNaN(e)||e<=0?0n:BigInt(Math.floor(e*0xde0b6b3a7640000))}catch{return 0n}}_getTitle(){switch(this.target){case`seriesA`:return`INSIDER ROUND`;case`seriesB`:return`"INSIDER" ROUND`;case`publicBuy`:return`BUY`;case`vibecoin`:return this._vcName?this._vcName.toUpperCase():`BUY`;case`vibecoinSell`:return this._vcSymbol?`SELL ${this._vcSymbol}`:`SELL`}}_vcBps(e,t){switch(t){case`seriesA`:return 10000n-e.SERIES_A_PROTOCOL-e.SERIES_A_REFERRAL-e.SERIES_A_PRIZE;case`seriesB`:return 10000n-e.SERIES_B_PROTOCOL-e.SERIES_B_REFERRAL-e.SERIES_B_PRIZE;default:return 10000n-e.PUMP_PROTOCOL-e.PUMP_REFERRAL-e.PUMP_PRIZE}}_getFeeBreakdown(){if(this.target===`vibecoin`)return[[`Dividend pool`,1000n],[`To buyer`,9000n]];let e=this._gameConstants;if(!e)switch(this.target){case`seriesA`:return[[`Protocol`,400n],[`Referral`,600n],[`Rugpull Fund`,2000n],[`Vibecoins minted`,7000n]];case`seriesB`:return[[`Protocol`,400n],[`Referral`,600n],[`Rugpull Fund`,1000n],[`Vibecoins minted`,8000n]];default:return[[`Protocol`,400n],[`Referral`,600n],[`Rugpull Fund`,2400n],[`Vibecoin mint`,6600n]]}switch(this.target){case`seriesA`:return[[`Protocol`,e.SERIES_A_PROTOCOL],[`Referral`,e.SERIES_A_REFERRAL],[`Rugpull Fund`,e.SERIES_A_PRIZE],[`Vibecoins minted`,this._vcBps(e,`seriesA`)]];case`seriesB`:return[[`Protocol`,e.SERIES_B_PROTOCOL],[`Referral`,e.SERIES_B_REFERRAL],[`Rugpull Fund`,e.SERIES_B_PRIZE],[`Vibecoins minted`,this._vcBps(e,`seriesB`)]];default:return[[`Protocol`,e.PUMP_PROTOCOL],[`Referral`,e.PUMP_REFERRAL],[`Rugpull Fund`,e.PUMP_PRIZE],[`Vibecoin mint`,this._vcBps(e,`publicBuy`)]]}}get _isClosed(){return this.target===`vibecoin`?W.vibecoins.find(e=>e.address.toLowerCase()===this.vibecoinAddress?.toLowerCase())?.unlocked??!1:!1}_computeTokenPreview(){let e=this._amountWei;if(e===0n)return 0n;let t=W.gameConstants;if(this.target===`publicBuy`){let n=W.gameState;if(!n)return 0n;let r=W.vibecoins.find(e=>e.address.toLowerCase()===n.currentVibecoin.toLowerCase());return r?Dp(e*(t?this._vcBps(t,`publicBuy`):6600n)/10000n,r.totalSupply,r.P0,r.K):0n}if(this.target===`vibecoin`){let t=W.vibecoins.find(e=>e.address.toLowerCase()===this.vibecoinAddress?.toLowerCase());return t?Dp(e,t.totalSupply,t.P0,t.K):0n}return 0n}_computeTimerExtension(){let e=W.gameState;if(!e||e.settlementTimestamp===0n)return`--`;let t=this._amountWei;if(t===0n)return`--`;let n=Number(e.totalSeriesA+e.totalSeriesB)/0xde0b6b3a7640000;if(n===0)return`--`;let r=Number(e.totalPublicCurveDeposited+t)/0xde0b6b3a7640000+1e4,i=n*2419200/(n+2e6)*r*r*1e3/(r*r+40*n*n),a=Number(e.settlementTimestamp)*1e3+36e5+i,o=Math.max(0,a-Number(e.timerExpiry));if(o===0)return`at max`;let s=Math.floor(o/1e3),c=Math.floor(s/3600),l=Math.floor(s%3600/60);return c>0?`+${c}h ${l}m`:l>0?`+${l}m ${s}s`:`+${s}.${Math.floor(o)%1e3}s`}async _approve(){let e=L[I],t=B();if(!t||!R)return;let n=this._amountWei;if(n!==0n){this.approvePending=!0,this.error=``;try{let r=this.target===`vibecoin`?this.vibecoinAddress:e.game;await t.writeContract({address:e.shmon,abi:Qu,functionName:`approve`,args:[r,n],account:R.address,chain:t.chain}),this.approved=!0}catch(e){dp(e)}finally{this.approvePending=!1}}}async _buy(){let e=L[I],t=B();if(!t||!R)return;let n=this._amountWei;if(n!==0n){if(this.target===`seriesA`||this.target===`seriesB`){let e=W.gameState,t=this.target===`seriesA`?0:1;if(!e||zd(e)!==t){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}let n=Bd(e,t);if(n>0n&&Ld()>=Number(n)){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}}this.pending=!0,this.error=``;try{let r=Id(Md()??``),i;i=this.target===`vibecoinSell`?await t.writeContract({address:this.vibecoinAddress,abi:H,functionName:`sell`,args:[n,R.address],account:R.address,chain:t.chain}):this.useShMon?this.target===`seriesA`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesAWithShMon`,args:[r,n],account:R.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesBWithShMon`,args:[r,n],account:R.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:V,functionName:`buyWithShMon`,args:[r,n],account:R.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:H,functionName:`buyWithShMon`,args:[R.address,n,!0],account:R.address,chain:t.chain}):this.target===`seriesA`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesA`,args:[r],value:n,account:R.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesB`,args:[r],value:n,account:R.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:V,functionName:`buy`,args:[r],value:n,account:R.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:H,functionName:`buy`,args:[R.address,!0],value:n,account:R.address,chain:t.chain}),await Promise.all([W.refresh(),W.refreshUser()]),this._close(),up(i,this._actionLabel())}catch(e){dp(e)}finally{this.pending=!1}}}_actionLabel(){let e=U(this._amountWei);switch(this.target){case`seriesA`:return`Insider deposit — ${e}`;case`seriesB`:return`"Insider" deposit — ${e}`;case`publicBuy`:return`Pump — ${e}`;case`vibecoin`:return`Bought ${W.vibecoins.find(e=>e.address.toLowerCase()===this.vibecoinAddress?.toLowerCase())?.symbol??`Vibecoin`} — ${e}`;case`vibecoinSell`:{let e=W.vibecoins.find(e=>e.address.toLowerCase()===this.vibecoinAddress?.toLowerCase());return`Sold ${e?.symbol??`Vibecoin`} — ${xd(this._amountWei,e?.symbol??`vibecoins`)}`}}}_renderPreview(){let e=this._amountWei;if(this.target===`vibecoinSell`)return K`
        <div class="preview">
          <div class="preview-row">
            <span class="key">SHMON RECEIVED (EST)</span>
            <span class="val accent">${e>0n?U(this._sellShMonPreview):`—`}</span>
          </div>
          <div class="fee-section">
            <div class="preview-row">
              <span class="key">DIVIDEND CUT</span>
              <span class="val">${e>0n&&this._sellShMonPreview>0n?`10% · ${U(this._sellShMonPreview/9n)}`:`10%`}</span>
            </div>
          </div>
        </div>
      `;let t=this._getFeeBreakdown(),n=this._computeTokenPreview(),r=this.target===`vibecoin`?10000n:t.find(([e])=>e===`Vibecoin`)?.[1]??6600n,i=this.target===`vibecoin`?e:e*r/10000n,a=n>0n?i*10n**18n/n:0n,o=this.target===`seriesA`||this.target===`seriesB`,s=this._refCode,c=n*(this.target===`vibecoin`?t.find(([e])=>e===`To buyer`)?.[1]??9000n:10000n)/10000n,l=W.gameState,u=W.gameConstants,d=e*(u?this._vcBps(u,this.target):this.target===`seriesA`?7000n:this.target===`seriesB`?8000n:6600n)/10000n,f=(this.target===`seriesA`?l?.totalSeriesA??0n:l?.totalSeriesB??0n)+d,p=d>0n&&f>0n?(Number(d*10000n/f)/100).toFixed(2):null,m=this.target===`seriesA`?`insider buy-in`:`"insider" buy-in`;return K`
      <div class="preview">
        ${o?K`
          <div class="preview-row">
        ...
        `:K`
          <div class="preview-row">
            ...
            <span class="val accent">${c>0n?xd(c,this._vcSymbol):`—`}</span>
          </div>
          <div class="preview-row">
            <span class="key">EFFECTIVE PRICE</span>
            <span class="val">${a>0n?`${U(a,6)} / \$${this._vcSymbol}`:`—`}</span>
          </div>
        `}
        ...
        <div class="fee-section">
          ${t.map(([t,r])=>{let i=this.target===`vibecoin`&&n>0n?` · ${xd(n*r/10000n,this._vcSymbol)}`:this.target!==`vibecoin`&&e>0n?` · ${this.useShMon?U(e*r/10000n):yd(e*r/10000n)}`:``;return K`
              <div class="preview-row">
      ...
      `:``}
    `}render(){if(!this.visible)return K``;let e=this.target===`vibecoinSell`,t=!e&&this.useShMon,n=!!R,r=this.target===`seriesA`||this.target===`seriesB`,i=e?this._tokenBalance:0n;return K`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="modal">
          ...
          <div class="field">
            <label style="display:flex;justify-content:space-between">
              ...
              ${e&&i>0n?K`<span style="color:#d8d8e8">BAL: ${xd(i,this._vcSymbol)}</span>`:``}
            </label>
    ...
    `}};Z([X()],Op.prototype,`visible`,void 0),Z([X()],Op.prototype,`target`,void 0),Z([X()],Op.prototype,`vibecoinAddress`,void 0),Z([X()],Op.prototype,`amountStr`,void 0),Z([X()],Op.prototype,`useShMon`,void 0),Z([X()],Op.prototype,`pending`,void 0),Z([X()],Op.prototype,`approvePending`,void 0),Z([X()],Op.prototype,`approved`,void 0),Z([X()],Op.prototype,`error`,void 0),Z([X()],Op.prototype,`_sellShMonPreview`,void 0),Z([X()],Op.prototype,`_tokenBalance`,void 0),Op=Z([Y(`buy-dialog`)],Op);var kp=`disclaimer-accepted-v1`,Ap=class extends J{constructor(...e){super(...e),this.open=!localStorage.getItem(kp)}_accept(){localStorage.setItem(kp,`1`),this.open=!1}static{this.styles=G`
    .overlay {
  ...
  `}_renderContent(){return K`
    ...
    `}};Z([X()],Ap.prototype,`open`,void 0),Ap=Z([Y(`disclaimer-modal`)],Ap);var jp=class extends J{constructor(...e){super(...e),this.tab=`pump`,this.pollIntervals=[],this._handleHash=()=>{Fd();let e=window.location.hash.split(`?`)[0];e===`#vibecoins`||e.startsWith(`#vibecoins/`)?this.tab=`vibecoins`:this.tab=`pump`},this._onStoreUpdated=()=>{this.requestUpdate()},this._onWalletChanged=()=>{W.refreshUser(),this.requestUpdate()},this._onOpenBuyDialog=e=>{let t=this.renderRoot?.querySelector(`buy-dialog`);t&&typeof t.open==`function`&&t.open(e.detail)}}static{this.styles=G`
    :host {
      display: block;
      min-height: 100vh;
    }
  ...
  `}connectedCallback(){super.connectedCallback(),this._handleHash(),window.addEventListener(`hashchange`,this._handleHash),window.addEventListener(`store-updated`,this._onStoreUpdated),zu.addEventListener(`wallet-changed`,this._onWalletChanged),window.addEventListener(`open-buy-dialog`,this._onOpenBuyDialog),W.refresh(),W.refreshUser(),this.pollIntervals.push(setInterval(()=>{W.refresh()},5e3),setInterval(()=>{W.refreshUser()},1e4))}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._handleHash),window.removeEventListener(`store-updated`,this._onStoreUpdated),zu.removeEventListener(`wallet-changed`,this._onWalletChanged),window.removeEventListener(`open-buy-dialog`,this._onOpenBuyDialog),this.pollIntervals.forEach(clearInterval),this.pollIntervals=[]}render(){return K`
    ...
    `}};Z([X()],jp.prototype,`tab`,void 0),jp=Z([Y(`app-root`)],jp);var Mp=class extends J{constructor(...e){super(...e),this.airdropReserve=0n,this.totalAirdropMinted=0n,this.optPending=!1,this.claimPending=!1,this._onUpdate=()=>{this.requestUpdate(),this._loadAirdropData()}}static{this.styles=G`
    :host { display: block; }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`store-updated`,this._onUpdate),this._loadAirdropData()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`store-updated`,this._onUpdate)}async _loadAirdropData(){try{let e=L[I],t=z(I),[n,r]=await Promise.all([t.readContract({address:e.fomon,abi:Zu,functionName:`AIRDROP_RESERVE`}),t.readContract({address:e.fomon,abi:Zu,functionName:`totalAirdropMinted`})]);this.airdropReserve=n,this.totalAirdropMinted=r}catch{}}async _optIn(){let e=L[I],t=B();if(!(!t||!R)){this.optPending=!0;try{await t.writeContract({address:e.fomon,abi:Zu,functionName:`optIn`,account:R.address,chain:t.chain}),await W.refreshUser()}catch(e){console.error(`OptIn error:`,e)}finally{this.optPending=!1}}}async _claim(){let e=L[I],t=B();if(!(!t||!R)){this.claimPending=!0;try{await t.writeContract({address:e.fomon,abi:Zu,functionName:`claim`,account:R.address,chain:t.chain}),await W.refreshUser()}catch(e){console.error(`Claim error:`,e)}finally{this.claimPending=!1}}}render(){let e=W.userState;if(!e)return K`<div class="blurb">Connect wallet to view foMON details</div>`;let t=this.airdropReserve>0n?Number(this.totalAirdropMinted*100n/this.airdropReserve):0;return K`
      <div class="panel">
        <div class="row">
        ...
        <div class="row">
          ...
          ${e.fomonOptedIn?K`<span class="blurb">Earning shMON dividends</span>`:K`
        ...
        ${e.fomonClaimable>0n?K`
          <hr class="divider" />
          <div class="row">
            <span class="key">PENDING shMON</span>
            <span class="val" style="color:#00e5ff">${vd(e.fomonClaimable)}</span>
        ...
        ${e.referralEarned>0n?K`
          <hr class="divider" />
          <div class="row">
            <span class="key">REFERRAL EARNED</span>
            <span class="val" style="color:#ffe600">${vd(e.referralEarned)}</span>
          </div>
        `:``}
    ...
    `}};Z([X()],Mp.prototype,`airdropReserve`,void 0),Z([X()],Mp.prototype,`totalAirdropMinted`,void 0),Z([X()],Mp.prototype,`optPending`,void 0),Z([X()],Mp.prototype,`claimPending`,void 0),Mp=Z([Y(`fomon-panel`)],Mp);