...
`)[0]||String(e),a=typeof t?.message==`string`&&t.message||String(e),o=n||typeof t?.name==`string`&&t.name!==`Error`&&t.name||`ERROR`;window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:i,full:a,name:o}}))}var Pp=class extends lp{constructor(...e){super(...e),this.open=!1,this.hash=null,this.action=``,this.transfers=null,this.errorMsg=``,this.errorFull=``,this.errorName=``,this._onError=e=>{this.errorMsg=e.detail.short,this.errorFull=e.detail.full,this.errorName=e.detail.name,this.hash=null,this.open=!0},this._onShow=e=>{this.hash=e.detail.hash,this.action=e.detail.action,this.transfers=null,this.open=!0,this._fetchTransfers(e.detail.hash)}}connectedCallback(){super.connectedCallback(),window.addEventListener(`tx-confirm-show`,this._onShow),window.addEventListener(`app-error-show`,this._onError)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`tx-confirm-show`,this._onShow),window.removeEventListener(`app-error-show`,this._onError)}async _fetchTransfers(e){let{connectedWallet:t}=await Yt(async()=>{let{connectedWallet:e}=await Promise.resolve().then(()=>Ku);return{connectedWallet:e}},void 0,import.meta.url);if(!t){this.transfers=[];return}let n=j,r=ad(Zn),i=t.address.slice(2).toLowerCase(),a;for(let t=0;t<20;t++){try{if(a=await r.getTransactionReceipt({hash:e}),a)break}catch{}await new Promise(e=>setTimeout(e,800))}if(!a){this.transfers=[];return}let o=a.logs.filter(e=>e.topics[0]?.toLowerCase()===jp&&e.topics[2]?.slice(-40).toLowerCase()===i);if(o.length===0){this.transfers=[];return}let s=[];for(let e of o){let t=e.address,i=`0x`+(e.topics[1]??`0x`).slice(-40),a=BigInt(e.data),o;if(t.toLowerCase()===n.fomon.toLowerCase())o=`foMON`;else if(t.toLowerCase()===n.shmon.toLowerCase())o=`shMON`;else{let e=X(`token:${t.toLowerCase()}:symbol`);if(e)o=e;else try{o=await r.readContract({address:t,abi:H,functionName:`symbol`})}catch{o=t.slice(0,6)+`...`}}let c=i.toLowerCase()===Mp&&t.toLowerCase()===n.fomon.toLowerCase();s.push({symbol:o,amount:a,isAirdrop:c})}this.transfers=s}_close(){this.open=!1}_breakPascal(e){return e.replace(/([a-z])([A-Z])/g,`$1​$2`)}async _copy(){this.hash&&await navigator.clipboard.writeText(this.hash)}static{this.styles=af`
  ...
  `}render(){if(!this.open)return U``;if(!this.hash)return U`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="dialog error">
          ...
          ${this.errorFull&&this.errorFull!==this.errorMsg?U`
            <div class="err-detail">${this.errorFull}</div>
          `:``}
        </div>
      </div>
    `;let e=j,t=this.hash.slice(0,10)+`…`+this.hash.slice(-6),n=e.explorer?`${e.explorer}/tx/${this.hash}`:null;return U`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="dialog">
          <button class="close" @click=${this._close}>×</button>
          <div class="check">${this.transfers===null?U`<span class="spinner" style="display:inline-block;vertical-align:middle;margin-right:6px"></span>PENDING…`:`✓ CONFIRMED`}</div>
          <div class="action">${this.action}</div>

          <div class="hash-row">
            <span class="hash">${t}</span>
            <button class="copy-btn" @click=${this._copy}>copy</button>
            ${n?U`<a class="explorer-link" href=${n} target="_blank">view ↗</a>`:``}
          </div>

          ${this.transfers===null||this.transfers.length>0?U`
            <hr class="divider"/>
            <div class="transfers-title">token transfers</div>
            ${this.transfers===null?U`<div class="loading"><div class="spinner"></div>awaiting confirmation…</div>`:this.transfers.map(e=>U`
                <div class="transfer">
                  <span class="plus">+</span>
                  <span class="t-amount">${wp(e.amount)}</span>
                  <span class="t-symbol">${e.symbol}</span>
                  ${e.isAirdrop?U`<span class="airdrop-badge">airdrop</span>`:``}
                </div>
              `)}
          `:``}
        </div>
      </div>
    `}};J([q()],Pp.prototype,`open`,void 0),J([q()],Pp.prototype,`hash`,void 0),J([q()],Pp.prototype,`action`,void 0),J([q()],Pp.prototype,`transfers`,void 0),J([q()],Pp.prototype,`errorMsg`,void 0),J([q()],Pp.prototype,`errorFull`,void 0),J([q()],Pp.prototype,`errorName`,void 0),Pp=J([G(`tx-confirm-dialog`)],Pp);var Fp=null,Ip=!1;async function Lp(){if(!Fp||Ip)return;Ip=!0;let e=await nr(()=>z(e=>e.readContract({address:j.game,abi:V,functionName:`cycle`})),!1,`vibecoin_db_cycle`);for(;e>0n;){if(await Fp.getFromIndex(`vibecoins`,`by-cycle`,Number(e))!=null){e--;continue}let t=await nr(()=>z(t=>t.readContract({address:j.game,abi:V,functionName:`vibecoinByCycle`,args:[e]})),!1,`vibecoin_db_cycle_`+e);if(!t||t==`0x0000000000000000000000000000000000000000`){e--;continue}let[n,r]=await Promise.all([nr(()=>z(e=>e.readContract({address:t,abi:H,functionName:`name`})),!0,`vibecoin_db_cycle_`+e),nr(()=>z(e=>e.readContract({address:t,abi:H,functionName:`symbol`})),!0,`vibecoin_db_cycle_`+e)]);await Fp.put(`vibecoins`,{address:t.toLowerCase(),cycle:Number(e),name:n.toLowerCase(),symbol:r.toLowerCase()}),e--}Ip=!1}window.addEventListener(`lls-updated`,({detail:e})=>{for(let[t,n]of e)t==`game.phase`&&n==2&&Lp().catch(e=>{Y(e)})}),(async()=>{Fp=await Bd(`rugpad`,1,{upgrade(e){let t=e.createObjectStore(`vibecoins`,{keyPath:`address`});t.createIndex(`by-cycle`,`cycle`),t.createIndex(`by-symbol`,`symbol`),t.createIndex(`by-name`,`name`)}}),await Lp()})().catch(e=>{Y(e)});function Rp(e){if(!e)return IDBKeyRange.lowerBound(``);let t=e.length-1,n=e.substring(0,t)+String.fromCharCode(e.charCodeAt(t)+1);return IDBKeyRange.bound(e,n,!1,!0)}async function zp(e){let t=Rp(e);if(!Fp)return null;if(e.startsWith(`0x`))return(await Fp.getAll(`vibecoins`,t,10)).map(e=>e.cycle);let n=(await Fp.getAllFromIndex(`vibecoins`,`by-symbol`,t,10)).map(e=>e.cycle);n.length<10&&n.push(...(await Fp.getAllFromIndex(`vibecoins`,`by-name`,t,10-n.length)).map(e=>e.cycle));for(let e=0;e<n.length;e+=1)n.indexOf(n[e],e+1)>=0&&(n.splice(e,1),--e);return n}async function Bp(e){if(e=Number(e),Fp)return(await Fp.getFromIndex(`vibecoins`,`by-cycle`,e))?.address}var Vp=300*1e3,Hp=30*1e3,Up=3333;function Wp(){Q(`meta.chainTimeDelta`,[`meta.chainTimeDelta`],async()=>{let e=await z(e=>e.getBlock({blockTag:`latest`})),t=Number(e.timestamp)-Date.now()/1e3;return t<60&&(t=0),[t]},Z.Static),Q(`meta.shmonRate`,[`meta.shmonRate`],async()=>[await z(e=>e.readContract({address:j.shmon,abi:ud,functionName:`convertToAssets`,args:[10n**18n]}))],Z.Live,Vp),Q(`game.state`,[`game.phase`,`game.cycle`,`game.phaseEndTime`,`game.prizePool`,`game.lastBuyer`,`game.vibecoin`,`game.isSettled`],async()=>{let e=await z(e=>e.readContract({address:j.game,abi:V,functionName:`gameState`,args:[!1]})),t=e.phase===3;return[e.phase,e.cycle,e.phaseEndTime,e.prizePool,e.lastBuyer,e.vibecoin.toLowerCase(),t]},Z.Live),Q(`game.vibecoinByCycle$`,[`game.vibecoinByCycle$`],async e=>{let t=await Bp(Number(e));return t===`0x0000000000000000000000000000000000000000`?[void 0]:[t]},Z.StaticWhenKnown),Q(`game.seriesADeposit$from$`,[`game.seriesADeposit$from$`],async(e,t)=>[await z(n=>n.readContract({address:j.game,abi:V,functionName:`seriesADeposits`,args:[BigInt(e),t]}))],Z.Live,Up),Q(`game.seriesBDeposit$from$`,[`game.seriesBDeposit$from$`],async(e,t)=>[await z(n=>n.readContract({address:j.game,abi:V,functionName:`seriesBDeposits`,args:[BigInt(e),t]}))],Z.Live,Up),Q(`game.seriesADeposit$`,[`game.seriesADeposit$`],async e=>{let t=BigInt(e),n=X(`game.cycle`),r=X(`game.phase`);return n==null||r==null?[void 0]:t==n&&r<2?[await z(e=>e.readContract({address:j.game,abi:V,functionName:`totalSeriesA`,args:[]}))]:[await z(e=>e.readContract({address:j.game,abi:V,functionName:`totalSeriesAByCycle`,args:[t]}))]},Z.Live,Hp,[`game.cycle`,`game.phase`]),Q(`game.seriesBDeposit$`,[`game.seriesBDeposit$`],async e=>{let t=BigInt(e),n=X(`game.cycle`),r=X(`game.phase`);return n==null||r==null?[void 0]:t==n&&r<2?[await z(e=>e.readContract({address:j.game,abi:V,functionName:`totalSeriesB`,args:[]}))]:[await z(e=>e.readContract({address:j.game,abi:V,functionName:`totalSeriesBByCycle`,args:[t]}))]},Z.Live,Hp,[`game.cycle`,`game.phase`]);for(let e of[`tokensSeriesAByCycle`,`tokensSeriesBByCycle`,`winnerByCycle`,`prizeByCycle`])Q(`game.`+e+`$`,[`game.`+e+`$`],async t=>{let n=BigInt(t);return[await z(t=>t.readContract({address:j.game,abi:V,functionName:e,args:[n]}))]},Z.Live,Hp);Q(`game.referralEarned$`,[`game.referralEarned$`],async e=>[await z(t=>t.readContract({address:j.game,abi:V,functionName:`referralEarned`,args:[e]}))],Z.Live,Hp),Q(`game.referralCodeOf$`,[`game.referralCodeOf$`],async e=>[gd(await z(t=>t.readContract({address:j.game,abi:V,functionName:`referralCodeOf`,args:[e]})))||``],Z.Live,Hp),Q(`game.settlementTimestamp`,[`game.settlementTimestamp`],async()=>[await z(e=>e.readContract({address:j.game,abi:V,functionName:`settlementTimestamp`,args:[]}))],Z.Live,Hp),Q(`game.consts`,[`game.const.seriesADuration`,`game.const.seriesAProtocol`,`game.const.seriesAReferral`,`game.const.seriesAPrize`,`game.const.seriesBProtocol`,`game.const.seriesBReferral`,`game.const.seriesBPrize`,`game.const.pumpProtocol`,`game.const.pumpReferral`,`game.const.pumpPrize`],async()=>{let e=[`SERIES_A_DURATION`,`SERIES_A_PROTOCOL`,`SERIES_A_REFERRAL`,`SERIES_A_PRIZE`,`SERIES_B_PROTOCOL`,`SERIES_B_REFERRAL`,`SERIES_B_PRIZE`,`PUMP_PROTOCOL`,`PUMP_REFERRAL`,`PUMP_PRIZE`],t=[];for(let n of e)try{t.push(await z(e=>e.readContract({address:j.game,abi:V,functionName:n}))),await new Promise(e=>setTimeout(e,95))}catch{t.push(void 0)}return t},Z.StaticWhenKnown),Q(`token$balance$`,[`token$balance$`],async(e,t)=>e==`native`?[await z(e=>e.getBalance({address:t}))]:[await z(n=>n.readContract({address:e,abi:H,functionName:`balanceOf`,args:[t]}))],Z.Live,Up),Q(`token$earned$`,[`token$earned$`],async(e,t)=>[await z(n=>n.readContract({address:e,abi:H,functionName:`earned`,args:[t]}))],Z.Live,Up),Q(`token$pendingRewards$`,[`token$pendingRewards$`],async(e,t)=>[await z(n=>n.readContract({address:e,abi:ld,functionName:`pendingRewards`,args:[t]}))],Z.Live,Up),Q(`token$optedIn$`,[`token$optedIn$`],async(e,t)=>[await z(n=>n.readContract({address:e,abi:ld,functionName:`optedIn`,args:[t]}))],Z.Live,Up),Q(`token$seed`,[`token$seed`,`token$palette`],async e=>{let t=await z(t=>t.readContract({address:e,abi:H,functionName:`seed`,args:[]}));return[t,fd(t)]},Z.StaticWhenKnown);for(let e of[`name`,`symbol`,`K`,`P0`])Q(`token$`+e,[`token$`+e],async t=>[await z(n=>n.readContract({address:t,abi:H,functionName:e,args:[]}))],Z.StaticWhenKnown);Q(`token$logoUri`,[`token$logoUri`],async e=>{let t=await z(t=>t.readContract({address:e,abi:H,functionName:`tokenURI`,args:[]}));return t.startsWith(`data:application/json`)?[JSON.parse(atob(t.split(`,`)[1])).image]:[void 0]},Z.StaticWhenKnown);for(let e of[`totalSupply`,`poolBalance`,`athPrice`,`totalVolume`,`volume24h`])Q(`token$`+e,[`token$`+e],async t=>[await z(n=>n.readContract({address:t,abi:H,functionName:e,args:[]}))],Z.Live,Hp);for(let e of[`unlocked`,`athPostRug`])Q(`token$`+e,[`token$`+e],async t=>{let n=await z(n=>n.readContract({address:t,abi:H,functionName:e,args:[]}));return n&&ir.remove(`token:`+t+`:`+e),[n]},Z.Live,Hp);Q(`fomon.consts`,[`fomon.const.airdropReserve`,`fomon.const.totalSupply`],async()=>{let e=[`AIRDROP_RESERVE`,`TOTAL_SUPPLY`],t=[];for(let n of e)try{t.push(await z(e=>e.readContract({address:j.fomon,abi:ld,functionName:n}))),await new Promise(e=>setTimeout(e,95))}catch{t.push(void 0)}return t},Z.StaticWhenKnown),Q(`fomon.minted`,[`fomon.airdropMinted`,`fomon.airdropRemaining`],async()=>{let e=await z(e=>e.readContract({address:j.fomon,abi:ld,functionName:`totalAirdropMinted`})),t=X(`fomon.const.airdropReserve`);return[e,t==null?void 0:t-e]},Z.Live,0,[`fomon.const.airdropReserve`]),Q(`fomon.mintable`,[`fomon.mintable`],async()=>[await z(e=>e.readContract({address:j.fomon,abi:ld,functionName:`remainingCeiling`}))],Z.Live)}var Gp=new Map([[`token:native:name`,`Monad`],[`token:native:symbol`,`MON`],[`token:${j.shmon.toLowerCase()}:symbol`,`shMON`],[`token:${j.fomon.toLowerCase()}:symbol`,`foMON`]]),Kp=new Map,qp=new Set,Jp=new Map,Yp=new Map,Xp=new Map,Zp=1;Qn(`rugpad_storage_revision`)!==Zp&&(localStorage.clear(),$n(`rugpad_storage_revision`,Zp));function Qp(e){let t=[],n=``,r=0,i=e.indexOf(`:`);for(;i!=-1;)n+=e.substring(r,i)+`$`,r=i+1,i=e.indexOf(`:`,r),i==-1&&(i=e.length),t.push(e.substring(r,i)),r=i+1,i=e.indexOf(`:`,r);return n+=e.substring(r),[n,t]}function $p(e,t){t=[...t].reverse();let n=e.replaceAll(`$`,()=>`:`+(t.pop()??console.warn(e,`fed too little arguments`)??``)+`:`);return n.endsWith(`:`)?n.substring(0,n.length-1):n}function X(e,t,...n){let r=qp.has(e);if(r){let[t,r]=Qp(e);em($p(Kp.get(t),r),n)}let i=Gp.get(e);if(i!==void 0)return i;if(i=Qn(`lls:`+Zn+`:`+e),i!==void 0)return Gp.set(e,i),i;if(!r){let[r,i]=Qp(e),a=Kp.get(r);if(!a){console.error(e,`has no associated task! This may result in an infinite loading circle.`);return}let o=Jp.get(a);if(!o){console.error(a,`was not defined! This may result in an infinite loading circle.`);return}qp.add(e);let s=$p(a,i);for(let e of Yp.get(a)||[]){X($p(e,i),t,...n);let a=Kp.get(r);if(!a)continue;let o=$p(a,i),c=Xp.get(s);c||(c=new Set,Xp.set(s,c)),c.add(o)}let c=o.fn,l={name:s,fn:()=>c(...i),repeatMs:o.repeatMs};t?ir.prepend(l):ir.upsert(l),ir.elementRequestedTask(s,...n)}}function em(e,t,n=new Set){if(!n.has(e)){n.add(e),ir.elementRequestedTask(e,...t);for(let r of Xp.get(e)||[])em(r,t,n)}}var Z=function(e){return e[e.Static=0]=`Static`,e[e.StaticWhenKnown=1]=`StaticWhenKnown`,e[e.Live=2]=`Live`,e}({});function Q(e,t,n,r,i=0,a=[]){Yp.set(e,a),Jp.set(e,{template:e,fn:async(...i)=>{let a=await n(...i),o=!1,s=new Map;for(let e=0;e<t.length;e+=1){let n=t[e],c=$p(n,i),l=Gp.get(c),u=a[e];l!==u&&s.set(c,u),u===void 0?o=!0:(Gp.set(c,u),r<=Z.StaticWhenKnown&&$n(`lls:`+Zn+`:`+c,u))}if(r==Z.StaticWhenKnown&&!o){let t=$p(e,i);ir.remove(t)}s.size&&window.dispatchEvent(new CustomEvent(`lls-updated`,{detail:s}))},repeatMs:r==Z.Static?void 0:i});for(let n of t)Kp.set(n,e)}function tm(){Wp()}var nm=25,rm=5,im=1,am=60,om=90,sm=class extends HTMLElement{constructor(...e){super(...e),this._phosphor=null,this._rafId=0,this._marqueeX=0,this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`,this._candles=[],this._loadingVcAddr=``,this._currentVcAddr=``,this._K=0n,this._P0=0n,this._onLiveUpdate=()=>{let e=X(`game.phase`,!1,this),t=X(`game.cycle`,!1,this),n=X(`game.phaseEndTime`,!1,this),r=X(`meta.chainTimeDelta`,!1,this)??0,i=Number(X(`game.const.seriesADuration`,!1,this));if(e===void 0||t===void 0||n==null||isNaN(i)){this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`;return}let a=Date.now()/1e3+r,o=e===0&&n>0n?Number(n)-a:1/0,s=e===0&&t>1n&&(n===0n||o>i/2),c=s?t-1n:t,l=s?3:e,u=s||l===2||l===3?X(`game.vibecoinByCycle:${c}`,!1,this):l===1?X(`game.vibecoin`,!1,this):void 0;if(u===void 0){this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`;return}let d=u.toLowerCase();if(l===2&&a<Number(n)&&d!==`0x0000000000000000000000000000000000000000`){let e=X(`token:${d}:name`,!1,this),t=X(`token:${d}:symbol`,!1,this);e===void 0||t===void 0?(this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`):(this._marqueeText=`PUMP ${e}! — $${t} TO THE MOON`,this._marqueeColor=`rgba(0,229,255,0.9)`)}else l===3||l===2&&a>=Number(n)?(this._marqueeText=`GET RUGGED`,this._marqueeColor=`rgba(255,45,107,0.9)`):(this._marqueeText=`GENERATING VIBECOIN`,this._marqueeColor=`rgba(255,230,0,0.9)`);let f=X(`token:${d}:K`,!1,this),p=X(`token:${d}:P0`,!1,this);if(u!==this._currentVcAddr&&d!==`0x0000000000000000000000000000000000000000`&&p!=null&&f!=null){this._K=f,this._P0=p,this._currentVcAddr=u,this._candles=[];let e=this._buf.width||Math.ceil(window.innerWidth/nm);this._startCandleLoad(u,this._K,this._P0,Math.ceil(e/rm)+2)}let m=X(`token:${d}:totalSupply`,!1,this);if(m!==void 0&&this._candles[0]&&p!=null&&f!=null){let e=Number(this._P0+this._K*m/10n**18n),t=this._candles[0];this._candles[0]={open:t.open,close:e,high:Math.max(t.high,e),low:Math.min(t.low,e)}}}}connectedCallback(){this.style.cssText=`position:fixed;inset:0;z-index:0;pointer-events:none;overflow:hidden`,this._canvas=document.createElement(`canvas`),this._canvas.style.cssText=`display:block;width:100%;height:100%`,this.appendChild(this._canvas),this._ctx=this._canvas.getContext(`2d`),this._buf=document.createElement(`canvas`),this._bCtx=this._buf.getContext(`2d`),this._bloom=document.createElement(`canvas`),this._blCtx=this._bloom.getContext(`2d`),this._ro=new ResizeObserver(()=>this._onResize()),this._ro.observe(this),window.addEventListener(`lls-updated`,this._onLiveUpdate),this._onLiveUpdate(),this._startAnimation()}disconnectedCallback(){cancelAnimationFrame(this._rafId),this._ro.disconnect(),window.removeEventListener(`lls-updated`,this._onLiveUpdate)}_onResize(){let e=Math.round(this.offsetWidth),t=Math.round(this.offsetHeight);if(!e||!t)return;this._canvas.width=e,this._canvas.height=t,this._bloom.width=e,this._bloom.height=t;let n=Math.ceil(e/nm),r=Math.ceil(t/nm);this._buf.width=n,this._buf.height=r;let i=document.createElement(`canvas`);i.width=nm,i.height=nm;let a=i.getContext(`2d`),o=nm-4,s=Math.floor(o/3);a.fillStyle=`#000`,a.fillRect(0,0,nm,nm),a.fillStyle=`rgb(255,0,0)`,a.fillRect(2,2,s,o),a.fillStyle=`rgb(0,255,0)`,a.fillRect(2+s,2,s,o),a.fillStyle=`rgb(0,0,255)`,a.fillRect(2+s*2,2,s,o),this._phosphor=this._ctx.createPattern(i,`repeat`);let c=Math.ceil(n/rm)+2;c>this._candles.length&&this._currentVcAddr&&this._startCandleLoad(this._currentVcAddr,this._K,this._P0,c)}_startCandleLoad(e,t,n,r){this._loadingVcAddr=e,setTimeout(()=>{this._loadingVcAddr===e&&this._loadCandles(e,t,n,r)},3e3)}async _loadCandles(e,t,n,r){let i=ad(),a=10n**18n,o,s;try{let t=await nr(()=>i.getBlock(),!1,`candle:${e}:ref`);o=t.number,s=Number(t.timestamp)}catch{return}if(this._loadingVcAddr!==e)return;let c=150n;try{let t=await nr(()=>i.getBlock({blockNumber:o-50n}),!1,`candle:${e}:probe`),n=s-Number(t.timestamp);n>0&&(c=BigInt(Math.round(50*am/n)))}catch{}if(this._loadingVcAddr!==e)return;let l=[];for(let s=0;s<=r;s++){if(this._loadingVcAddr!==e)return;let r=o-BigInt(s)*c;if(r<0n){l.push(null);continue}try{let o=await nr(()=>i.readContract({address:e,abi:H,functionName:`totalSupply`,blockNumber:r}),!1,`candle:${e}:s${s}`);l.push(Number(n+t*o/a))}catch{l.push(null)}}if(this._loadingVcAddr===e)for(let e=0;e<r;e++){let t=l[e],n=l[e+1];t!==null&&n!==null&&(this._candles[e]={close:t,open:n,high:Math.max(t,n),low:Math.min(t,n)})}}_startAnimation(){let e=performance.now(),t=n=>{let r=(n-e)/1e3;e=n,this._marqueeX-=om*r,this._draw(),this._rafId=requestAnimationFrame(t)};this._rafId=requestAnimationFrame(t)}_draw(){let e=this._canvas.width,t=this._canvas.height,n=this._buf.width,r=this._buf.height;if(!e||!t||!n||!r)return;let i=this._bCtx;i.fillStyle=`rgb(6,6,8)`,i.fillRect(0,0,n,r),this._drawCandlesBuf(i,n,r),this._drawMarqueeBuf(i,n,r);let a=n*nm,o=r*nm,s=this._ctx;s.imageSmoothingEnabled=!1,s.drawImage(this._buf,0,0,a,o),this._phosphor&&(s.globalCompositeOperation=`multiply`,s.fillStyle=this._phosphor,s.fillRect(0,0,a,o),s.globalCompositeOperation=`source-over`);let c=this._blCtx;c.clearRect(0,0,e,t),c.filter=`blur(5px)`,c.drawImage(this._canvas,0,0),c.filter=`none`,s.globalCompositeOperation=`screen`,s.globalAlpha=.35,s.drawImage(this._bloom,0,0),s.globalAlpha=1,s.globalCompositeOperation=`source-over`}_drawCandlesBuf(e,t,n){let r=this._candles.filter(e=>e!==null);if(r.length<2)return;let i=1/0,a=-1/0;for(let e of r)e.low<i&&(i=e.low),e.high>a&&(a=e.high);if(a===i)return;let o=Math.round(n*.1),s=Math.round(n*.1),c=n-o-s,l=e=>n-s-Math.round((e-i)/(a-i)*c);for(let n=0;n<r.length;n++){let i=r[n],a=t-(n+1)*rm;if(a+rm<0)break;e.fillStyle=i.close>=i.open?`rgba(0,229,255,0.2)`:`rgba(255,45,107,0.2)`;let o=a+Math.floor((rm-im)/2),s=l(i.high),c=l(i.low);e.fillRect(o,s,im,Math.max(1,c-s));let u=l(Math.max(i.open,i.close)),d=l(Math.min(i.open,i.close));e.fillRect(a,u,rm,Math.max(1,d-u))}}_drawMarqueeBuf(e,t,n){let r=Math.round(n*.8);e.save(),e.font=`${r}px "Bebas Neue", sans-serif`,e.fillStyle=this._marqueeColor,e.textBaseline=`middle`;let i=this._marqueeText+` — `,a=e.measureText(i).width;if(a<1){e.restore();return}let o=(this._marqueeX/nm%a-a)%a;for(;o<t+a;)e.fillText(i,o,n/2),o+=a;e.restore()}};customElements.get(`app-background`)||customElements.define(`app-background`,sm);var cm=10n**18n,$=class extends lp{constructor(...e){super(...e),this.liveKeys=new Set,this._onLiveUpdate=e=>{for(let t of this.liveKeys)if(e.detail.has(t)){this.requestUpdate();break}},this._onDenomChanged=()=>this.requestUpdate()}static{this.styles=af`
  ...
  `}get spin(){return U`<span class="spin"></span>`}connectedCallback(){super.connectedCallback(),window.addEventListener(`lls-updated`,this._onLiveUpdate),window.addEventListener(`denom-changed`,this._onDenomChanged),this.requestUpdate()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`lls-updated`,this._onLiveUpdate),window.removeEventListener(`denom-changed`,this._onDenomChanged)}willUpdate(e){this.liveKeys.clear()}liveValue(e,t){return this.liveKeys.add(e),X(e,t,this)}},lm=class extends ${constructor(...e){super(...e),this.key=``,this.numerator=1,this.denominator=1}_tokenPrefix(){if(this.key.startsWith(`fomon.`))return`token:`+j.fomon.toLocaleLowerCase();let e=this.key.match(/^token:(0x[a-fA-F0-9]+|native)/);return e?`token:${e[1]}`:null}render(){if(!this.key)return U`${this.spin}`;let e=`token:${j.shmon.toLowerCase()}`,t=[`:poolBalance`,`:totalVolume`,`:volume24h`].some(e=>this.key.endsWith(e))||this.key.includes(`:pendingRewards`)?e:this._tokenPrefix()??e,n=hp(),r=n===`mon`?`token:native`:e,i=this.liveValue(`${r}:symbol`);if(i==null)return U`${this.spin}`;let a=this.liveValue(this.key);if(this.liveValue(`meta.shmonRate`)==null||a==null)return U`${this.spin} ${i}`;let o=e=>this.numerator!==1||this.denominator!==1?e*BigInt(this.numerator)/BigInt(this.denominator):e,s;if(t===`token:native`){let e=_p();s=e>0n?o(a)*cm/e:0n}else if(t===e)s=o(a);else{let e=this.liveValue(`${t}:K`),n=this.liveValue(`${t}:P0`),r=this.liveValue(`${t}:totalSupply`);if(e==null||n==null||r==null)return U`${this.spin} ${i}`;s=(n+e*r/cm)*o(a)/cm}let c=t===r?``:`≈`;return n===`mon`?U`${c}${Cp(s*_p()/cm,i)}`:U`${c}${Cp(s,i)}`}};J([K()],lm.prototype,`key`,void 0),J([K({type:Number})],lm.prototype,`numerator`,void 0),J([K({type:Number})],lm.prototype,`denominator`,void 0),lm=J([G(`live-valuation`)],lm);var um=class extends ${constructor(...e){super(...e),this.valueKey=``,this.long=!1}_symbolKey(){return this.valueKey.startsWith(`fomon.`)?`token:`+j.fomon.toLocaleLowerCase()+`:symbol`:this.valueKey.replace(/:(?!0x|native).*$/,`:symbol`)}render(){let e=this._symbolKey(),t=this.liveValue(e),n=this.liveValue(this.valueKey);if(!this.valueKey||n===void 0||t===void 0)return U`${this.spin}${t?U` $${t}`:``}`;if(this.long){let e=Number(n/10n**15n)/1e3;return U`${Intl.NumberFormat().format(e)} \$${t}`}return U`${Cp(n,t)}`}};J([K({attribute:`value-key`})],um.prototype,`valueKey`,void 0),J([K({type:Boolean})],um.prototype,`long`,void 0),um=J([G(`live-token-amount`)],um);var dm=class extends ${constructor(...e){super(...e),this.valueKey=``}_symbolKey(){return this.valueKey.startsWith(`fomon.`)?`token:`+j.fomon.toLocaleLowerCase()+`:symbol`:this.valueKey.replace(/:(?!0x|native).*$/,`:symbol`)}render(){let e=this._symbolKey(),t=this.liveValue(e),n=this.liveValue(this.valueKey);this.liveValue(`meta.shmonRate`);let r=hp()===`mon`?`MON`:`shMON`;return!this.valueKey||n===void 0||t===void 0?U`${this.spin} ${r}${t?U`/$${t}`:``}`:U`${Sp(n,t)}`}};J([K({attribute:`value-key`})],dm.prototype,`valueKey`,void 0),dm=J([G(`live-token-price`)],dm);var fm=class extends ${constructor(...e){super(...e),this.numKey=``,this.demKey=``}render(){let e=this.liveValue(this.numKey),t=this.liveValue(this.demKey);return!this.numKey||!this.demKey||e===void 0||t===void 0?U`${this.spin}%`:U`${Op(e,t)}`}};J([K({attribute:`num-key`})],fm.prototype,`numKey`,void 0),J([K({attribute:`dem-key`})],fm.prototype,`demKey`,void 0),fm=J([G(`live-percent`)],fm);var pm=class extends ${constructor(...e){super(...e),this.key=``,this.full=!1}render(){let e=this.liveValue(this.key);if(!this.key||e===void 0)return U`0x${this.spin}…${this.spin}`;let t=this.full?e:Tp(e),n=j.explorer;return n?U`<a href="${n}/address/${e}" target="_blank" rel="noopener noreferrer">${t}</a>`:U`${t}`}};J([K()],pm.prototype,`key`,void 0),J([K({type:Boolean})],pm.prototype,`full`,void 0),pm=J([G(`live-address`)],pm);var mm=class extends ${constructor(...e){super(...e),this.key=``,this.fallback=`--`}render(){let e=this.liveValue(this.key);return!this.key||e===void 0?U`${this.spin}`:U`${e??this.fallback}`}};J([K()],mm.prototype,`key`,void 0),J([K()],mm.prototype,`fallback`,void 0),mm=J([G(`live-string`)],mm);var hm=class extends ${constructor(...e){super(...e),this.key=``,this.milliseconds=!1,this.warningMs=300*1e3,this._interval=null}static{this.styles=[$.styles,af`
      :host(.warning) {
        color: #ff2d6b;
    ...
    `]}connectedCallback(){super.connectedCallback();let e=this.milliseconds?50:1e3;this._interval=setInterval(()=>this.requestUpdate(),e)}disconnectedCallback(){super.disconnectedCallback(),this._interval!==null&&(clearInterval(this._interval),this._interval=null)}render(){let e=this.liveValue(this.key);if(!this.key||e===void 0){this.classList.remove(`warning`);let e=this.spin;return this.milliseconds?U`${e}:${e}:${e}.${e}`:U`${e}:${e}:${e}`}let t=Number(e)*1e3-Date.now();return this.classList.toggle(`warning`,t>0&&t<=this.warningMs),U`${(this.milliseconds?Dp:Ep)(t)}`}};J([K()],hm.prototype,`key`,void 0),J([K({type:Boolean})],hm.prototype,`milliseconds`,void 0),J([K({type:Number,attribute:`warning-ms`})],hm.prototype,`warningMs`,void 0),hm=J([G(`live-countdown`)],hm);var gm=class extends ${constructor(...e){super(...e),this.prefix=``}render(){let e=hp(),t=e===`mon`?`MON`:`shMON`,n=this.liveValue(`${this.prefix}:K`),r=this.liveValue(`${this.prefix}:P0`),i=this.liveValue(`${this.prefix}:totalSupply`);if(this.liveValue(`meta.shmonRate`),!this.prefix||n===void 0||r===void 0||i===void 0)return U`${this.spin} ${t}`;let a=(r+n*i/cm)*i/cm;if(e===`mon`){let e=_p();return U`≈${Cp(e>0n?a*e/cm:0n,`MON`)}`}return U`≈${Cp(a,`shMON`)}`}};J([K()],gm.prototype,`prefix`,void 0),gm=J([G(`live-bonding-mcap`)],gm);var _m=class extends ${constructor(...e){super(...e),this.prefix=``}render(){let e=this.liveValue(`${this.prefix}:symbol`),t=this.liveValue(`${this.prefix}:K`),n=this.liveValue(`${this.prefix}:P0`),r=this.liveValue(`${this.prefix}:totalSupply`);if(!this.prefix||t===void 0||n===void 0||r===void 0)return U`${this.spin}${e?U` shMON/$${e}`:``}`;let i=n+t*r/cm;return U`${(Number(i)/0xde0b6b3a7640000).toFixed(9)} shMON/$${e??`?`}`}};J([K()],_m.prototype,`prefix`,void 0),_m=J([G(`live-bonding-price`)],_m);var vm=class extends ${constructor(...e){super(...e),this.allocKey=``,this.depositKey=``,this.totalKey=``,this.symbolKey=``}render(){let e=this.liveValue(this.symbolKey),t=this.liveValue(this.allocKey),n=this.liveValue(this.depositKey),r=this.liveValue(this.totalKey);return!this.allocKey||!this.depositKey||!this.totalKey||t===void 0||n===void 0||r===void 0?U`${this.spin}${e?U` $${e}`:``}`:r===0n||t===0n?U`—`:U`${Cp(t*n/r,e??`?`)}`}};J([K({attribute:`alloc-key`})],vm.prototype,`allocKey`,void 0),J([K({attribute:`deposit-key`})],vm.prototype,`depositKey`,void 0),J([K({attribute:`total-key`})],vm.prototype,`totalKey`,void 0),J([K({attribute:`symbol-key`})],vm.prototype,`symbolKey`,void 0),vm=J([G(`live-series-tokens`)],vm);var ym=`fomonad-fomon-visited`,bm=class extends ${constructor(...e){super(...e),this.open=!1,this.providerOpen=!1,this.referralCode=``,this.activeRefInput=``,this.registerPending=!1,this.claimPending=!1,this.confirmReg=!1,this.regEligError=!1,this.copiedRef=!1,this.copied=!1,this.denomPref=hp(),this._wrongNetworkClicks=0,this._onWalletChanged=()=>{R?.chainId===10143&&(this._wrongNetworkClicks=0),this.requestUpdate()},this._onDocClick=e=>{e.composedPath().includes(this)||(this.open=!1,this.providerOpen=!1)}}static{this.styles=[$.styles,af`
  ...
  `]}connectedCallback(){super.connectedCallback(),L.addEventListener(`wallet-changed`,this._onWalletChanged),document.addEventListener(`click`,this._onDocClick)}disconnectedCallback(){super.disconnectedCallback(),L.removeEventListener(`wallet-changed`,this._onWalletChanged),document.removeEventListener(`click`,this._onDocClick)}async _connect(e){await Zu(e),$u(e.info.rdns),await cd(Zn),this.providerOpen=!1,this.requestUpdate()}_disconnect(){Qu(),ed(),this.open=!1}async _copyAddress(){R&&(await navigator.clipboard.writeText(R.address),this.copied=!0,setTimeout(()=>{this.copied=!1},1500))}async _claimFoMon(){let e=B();if(!(!e||!R)){this.claimPending=!0;try{Np(await e.writeContract({address:j.fomon,abi:ld,functionName:`claim`,account:R.address,chain:e.chain}),`Claimed foMON dividends`)}catch(e){Y(e)}finally{this.claimPending=!1}}}async _optIn(){let e=B();if(!(!e||!R))try{Np(await e.writeContract({address:j.fomon,abi:ld,functionName:`optIn`,account:R.address,chain:e.chain}),`Opted in to foMON dividends`)}catch(e){Y(e)}}async _checkEligAndConfirm(){if(!this.referralCode||!R)return;let e=ad();try{await e.simulateContract({address:j.game,abi:V,functionName:`registerReferral`,args:[hd(this.referralCode)],account:R.address}),this.regEligError=!1,this.confirmReg=!0}catch{this.regEligError=!0}}async _registerReferral(){if(!this.referralCode||!this.confirmReg)return;let e=B();if(!(!e||!R)){this.registerPending=!0,this.confirmReg=!1;try{let t=hd(this.referralCode),n=await e.writeContract({address:j.game,abi:V,functionName:`registerReferral`,args:[t],account:R.address,chain:e.chain}),r=this.referralCode;this.referralCode=``,Np(n,`Registered referral code "${r}"`)}catch(e){Y(e)}finally{this.registerPending=!1}}}async _copyRefUrl(e){await navigator.clipboard.writeText(`https://fomon.rip/testnet/${e}`),this.copiedRef=!0,setTimeout(()=>{this.copiedRef=!1},1500)}_setDenom(e){gp(e),this.denomPref=e}render(){if(!R){let e=Ju;return e.length===0?U`<button class="connect-btn" style="cursor:default;opacity:0.4">NO WALLET</button>`:e.length===1?U`
          <button class="connect-btn" @click=${()=>this._connect(e[0])}>
            ${e[0].info.icon?U`<img src=${e[0].info.icon} alt="" style="width:14px;height:14px;vertical-align:middle;margin-right:6px;border-radius:2px" />`:``}
            ...
            <div class="dropdown" style="min-width:220px;padding:12px 16px">
              ...
              ${e.map(e=>U`
                <button class="provider-item" @click=${()=>this._connect(e)}>
                  ${e.info.icon?U`<img src=${e.info.icon} alt="" />`:``}
                  ${e.info.name}
                </button>
              `)}
            </div>
      ...
      `}if(R.chainId!==10143)return U`
        <button class="connect-btn" style="background:#ff2d6b22;border-color:#ff2d6b;color:#ff2d6b" @click=${()=>{this._wrongNetworkClicks++,this._wrongNetworkClicks>=2?(Qu(),ed(),this._wrongNetworkClicks=0):cd(Zn)}}>${`SWITCH TO ${j?.name?.toUpperCase()??`CORRECT NETWORK`}`}</button>
      `;let e=R.address.toLowerCase(),t=j,n=t?.shmon?.toLowerCase()??``,r=t?.fomon?.toLowerCase()??``,i=this.liveValue(`token:${r}:optedIn:${e}`),a=this.liveValue(`token:${r}:pendingRewards:${e}`),o=this.liveValue(`token:${r}:balance:${e}`),s=this.liveValue(`game.referralCodeOf:${e}`),c=this.liveValue(`game.referralEarned:${e}`),l=a!==void 0&&a>0n||o!==void 0&&o>0n&&!localStorage.getItem(ym);return U`
      <div style="position:relative">
        <button
          class="address-btn ${this.open?`active`:``}"
          @click=${()=>{this.open=!this.open,this.open&&localStorage.setItem(ym,`1`)}}
        >
          ${l?U`<span class="notif-dot"></span>`:``}
          ${R.icon?U`<img src=${R.icon} alt="" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;border-radius:2px" />`:``}
          ${this.copied?U`<span class="copied-flash">COPIED!</span>`:U`<span>${Tp(R.address)}</span>`}
        ...
        ${this.open?U`
          <div class="dropdown">
            ...
            <div class="row">
              <span class="lbl">shMON${t.shmonUrl?U`&nbsp;<a href="${t.shmonUrl}" target="_blank" rel="noopener" title="redeem shMON for MON">Redeem ↗</a>`:``}</span>
    ...
    `}};J([q()],bm.prototype,`open`,void 0),J([q()],bm.prototype,`providerOpen`,void 0),J([q()],bm.prototype,`referralCode`,void 0),J([q()],bm.prototype,`activeRefInput`,void 0),J([q()],bm.prototype,`registerPending`,void 0),J([q()],bm.prototype,`claimPending`,void 0),J([q()],bm.prototype,`confirmReg`,void 0),J([q()],bm.prototype,`regEligError`,void 0),J([q()],bm.prototype,`copiedRef`,void 0),J([q()],bm.prototype,`copied`,void 0),J([q()],bm.prototype,`denomPref`,void 0),J([q()],bm.prototype,`_wrongNetworkClicks`,void 0),bm=J([G(`wallet-widget`)],bm);var xm=class extends lp{constructor(...e){super(...e),this.currentHash=window.location.hash||`#pump`,this._onHash=()=>{this.currentHash=window.location.hash||`#pump`}}static{this.styles=af`
    :host {
      display: block;
    }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`hashchange`,this._onHash)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._onHash)}render(){return U`
      <header>
    ...
    `}};J([q()],xm.prototype,`currentHash`,void 0),xm=J([G(`header-bar`)],xm);var Sm=class extends ${constructor(...e){super(...e),this.claimPendingA=!1,this.claimPendingB=!1,this._timerInterval=null,this._onWalletChanged=()=>{this.requestUpdate()}}static{this.styles=[$.styles,af`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),L.addEventListener(`wallet-changed`,this._onWalletChanged),this._timerInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),L.removeEventListener(`wallet-changed`,this._onWalletChanged),this._timerInterval&&=(clearInterval(this._timerInterval),null)}async _claimA(){let e=X(`game.cycle`),t=B();if(!(!t||!R||e===void 0)){this.claimPendingA=!0;try{Np(await t.writeContract({address:j.game,abi:V,functionName:`claimSeriesATokens`,args:[e],account:R.address,chain:t.chain}),`Claimed Insider Vibecoins`)}catch(e){console.error(`Claim A error:`,e)}finally{this.claimPendingA=!1}}}async _claimB(){let e=X(`game.cycle`),t=B();if(!(!t||!R||e===void 0)){this.claimPendingB=!0;try{Np(await t.writeContract({address:j.game,abi:V,functionName:`claimSeriesBTokens`,args:[e],account:R.address,chain:t.chain}),`Claimed "Insider" Vibecoins`)}catch(e){console.error(`Claim B error:`,e)}finally{this.claimPendingB=!1}}}_phaseStatus(e,t,n,r){if(t!==e)return e<t?`ENDED`:`PENDING`;if(n===0n)return e===0?`DEPOSIT TO KICK OFF (0.1 shMON needed)`:`PENDING`;let i=Number(n)-(Date.now()/1e3+r);return i>0?`ENDS IN ${Math.floor(i/3600)}h ${Math.floor(i%3600/60)}m ${Math.floor(i%60)}s`:`ENDED`}render(){let e=R?.address.toLowerCase()??``,t=this.liveValue(`game.cycle`),n=this.liveValue(`game.phase`),r=this.liveValue(`game.phaseEndTime`),i=this.liveValue(`meta.chainTimeDelta`)??0;if(t===void 0||n===void 0)return U`<div class="cards">${this.spin}</div>`;let a=n===0,o=n===1,s=n===2||n===3,c=s?(this.liveValue(`game.vibecoinByCycle:${t}`)??``).toLowerCase():``,l=e?this.liveValue(`game.seriesADeposit:${t}:from:${e}`):void 0,u=e?this.liveValue(`game.seriesBDeposit:${t}:from:${e}`):void 0,d=e?l===void 0?void 0:l>0n:!1,f=e?u===void 0?void 0:u>0n:!1,p=r===void 0?void 0:this._phaseStatus(0,n,r,i),m=r===void 0?void 0:this._phaseStatus(1,n,r,i);return U`
      <div class="cards">
        <!-- Series A -->
    ...
    `}_openBuy(e){window.dispatchEvent(new CustomEvent(`open-buy-dialog`,{detail:{target:e}}))}};J([q()],Sm.prototype,`claimPendingA`,void 0),J([q()],Sm.prototype,`claimPendingB`,void 0),Sm=J([G(`seed-section`)],Sm);var Cm=class extends ${constructor(...e){super(...e),this.cycle=0,this._claimPending=!1,this._optPending=!1,this._onWalletChanged=()=>{this.requestUpdate()}}static{this.styles=[$.styles,af`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),L.addEventListener(`wallet-changed`,this._onWalletChanged)}disconnectedCallback(){super.disconnectedCallback(),L.removeEventListener(`wallet-changed`,this._onWalletChanged)}async _optToggle(e,t){let n=B();if(!(!n||!R)){this._optPending=!0,this.requestUpdate();try{let r=t?`optOut`:`optIn`;Np(await n.writeContract({address:e,abi:H,functionName:r,account:R.address,chain:n.chain}),t?`Vibes undesired`:`Vibes desired`)}catch(e){Y(e)}finally{this._optPending=!1,this.requestUpdate()}}}async _claimDividends(e){let t=B();if(!(!t||!R)){this._claimPending=!0,this.requestUpdate();try{Np(await t.writeContract({address:e,abi:H,functionName:`claimDividends`,args:[R.address],account:R.address,chain:t.chain}),`Claim Vibes`)}catch(e){console.error(`Claim dividends error:`,e)}finally{this._claimPending=!1,this.requestUpdate()}}}_renderChart(e,t,n,r,i,a,o){let s=n*3n/2n||10n**18n,c=10n**18n,l=[],u=[];for(let n=0;n<=60;n++){let r=s*BigInt(n)/BigInt(60),i=e+t*r/c;l.push(Number(r)/0xde0b6b3a7640000),u.push(Number(i)/1e9)}let d=l[l.length-1]||1,f=Math.max(...u)||1,p=e=>38+e/d*262,m=e=>66-e/f*66,h=l.map((e,t)=>`${t===0?`M`:`L`}${p(e).toFixed(1)},${m(u[t]).toFixed(1)}`).join(` `),g=Math.round(p(Number(n)/0xde0b6b3a7640000)),_=a===void 0?null:Math.round(p(Number(a))),v=r!==void 0&&r>0n?p(Number(r)/0xde0b6b3a7640000):null,y=r!==void 0&&i!==void 0&&r>0n&&i>0n?p(Number(r+i)/0xde0b6b3a7640000):null,b=e=>String(e).split(``).map(e=>`₀₁₂₃₄₅₆₇₈₉`[+e]).join(``),x=e=>{let t=e/1e9;if(t>=.001)return t.toPrecision(3);let n=t.toFixed(30).split(`.`)[1]??``,r=0;for(let e of n)if(e===`0`)r++;else break;let i=n.slice(r,r+3);return`0.0${b(r-1)}${i}`};return U`
    ...
    `}render(){let e=this.liveValue(`game.vibecoinByCycle:${this.cycle}`),t=this.liveValue(`game.phase`);if(e===void 0)return U`<div class="panel"><div class="chart-area chart-loading">${this.spin}</div></div>`;let n=e.toLowerCase(),r=R?.address.toLowerCase()??``,i=this.liveValue(`token:${n}:K`),a=this.liveValue(`token:${n}:P0`),o=this.liveValue(`token:${n}:totalSupply`),s=this.liveValue(`token:${n}:unlocked`),c=this.liveValue(`token:${n}:athPrice`),l=this.liveValue(`token:${n}:athPostRug`),u=this.liveValue(`game.tokensSeriesAByCycle:${this.cycle}`),d=this.liveValue(`game.tokensSeriesBByCycle:${this.cycle}`),f=r?this.liveValue(`token:${n}:balance:${r}`):void 0,p=r?this.liveValue(`token:${n}:earned:${r}`):void 0,m=r?this.liveValue(`token:${n}:optedIn:${r}`):void 0,h=10n**18n,g=(()=>{if(!(c===void 0||i===void 0||a===void 0))return c===0n||i===0n||c<=a?0n:(c-a)*h/i})(),_=i!==void 0&&a!==void 0&&o!==void 0;return U`
      <div class="panel">
        <div class="row">
        ...
        ${_?this._renderChart(a,i,o,u,d,g,l):U`<div class="chart-area chart-loading">LOADING CHART…</div>`}
        ...
        <div class="dividends">
          ...
          <span class="div-val">
            ${r?U`<live-token-amount long value-key="token:${n}:earned:${r}"></live-token-amount>`:`—`}
          </span>
          <button
            class="claim-btn"
    ...
    `}};J([K({type:Number})],Cm.prototype,`cycle`,void 0),Cm=J([G(`pump-section`)],Cm);var wm=class extends ${constructor(...e){super(...e),this.cycle=0,this.triggerPending=!1,this._timerInterval=null}static{this.styles=[$.styles,af`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),this._timerInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),this._timerInterval&&=(clearInterval(this._timerInterval),null)}async _triggerSettlement(){let e=B();if(!(!e||!R)){this.triggerPending=!0;try{Np(await e.writeContract({address:j.game,abi:V,functionName:`triggerSettlement`,account:R.address,chain:e.chain}),`Rugpull triggered — settlement in progress`)}catch(e){Y(e)}finally{this.triggerPending=!1}}}_renderBuyerBlock(e,t,n){if(e===void 0)return U`<div class="no-buyer">${this.spin}</div>`;let r=e===`0x0000000000000000000000000000000000000000`?void 0:this.liveValue(`game.referralCodeOf:${e.toLowerCase()}`),i=j.explorer;return e===`0x0000000000000000000000000000000000000000`?U`
      <div class="no-buyer">no rugpuller yet</div>
    `:U`
      <div class="rugpuller-block">
        <div class="rugpuller-label">${n?`Rugpuller`:`Current Rugpuller`}${r?U` · <span style="color:#ffe600">${r}</span>`:``}</div>
        ${i?U`
    ...
    `}render(){let e=this.liveValue(`game.cycle`);if(e===void 0)return U`<div class="panel">${this.spin}</div>`;if(this.cycle!==Number(e)){let e=this.liveValue(`game.winnerByCycle:${this.cycle}`),t=`game.prizeByCycle:${this.cycle}`;return U`
        <div class="panel">
      ...
      `}let t=this.liveValue(`game.phase`),n=this.liveValue(`game.phaseEndTime`),r=this.liveValue(`meta.chainTimeDelta`)??0,i=this.liveValue(`game.lastBuyer`);if(t===void 0||n===void 0)return U`<div class="panel">${this.spin}</div>`;let a=t===2&&n>0n?Math.max(0,Number(n)*1e3-(Date.now()+r*1e3)):0,o=t===3||t===2&&a===0;return U`
      <div class="panel">
        ${o?U`
  ...
  `]}connectedCallback(){super.connectedCallback(),this._genInterval=setInterval(()=>this._tickGen(),150),this._tickInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),this._genInterval&&=(clearInterval(this._genInterval),null),this._tickInterval&&=(clearInterval(this._tickInterval),null)}async _settle(){let e=B();if(!(!e||!R)){this.settlePending=!0;try{Np(await e.writeContract({address:j.game,abi:V,functionName:`settleSeriesB`,account:R.address,chain:e.chain}),`Vibecoin generated — insider rounds settled`)}catch(e){Y(e)}finally{this.settlePending=!1}}}_tickGen(){this._scrambledTicker=Em(),this._tickCount++;let e=this._genStats;if(e.tokensConsumed+=Math.floor(Math.random()*14)+4,e.exitLiquidity+=Math.floor(Math.random()*8)+2,e.hopium+=+(Math.random()<.3),e.bagHolders+=Math.floor(Math.random()*6)+1,e.rugThreads=Math.min(99.7,e.rugThreads+Math.random()*.02),Math.random()<.02&&(e.grayAreas+=1),Math.random()<.01&&(e.marketMakers+=1),this._tickCount%20==0){let e=this._headingIdx;for(;e===this._headingIdx;)e=Math.floor(Math.random()*Dm.length);this._headingIdx=e}if(this._tickCount%27==0){let e=this._statIdx;for(;e===this._statIdx;)e=Math.floor(Math.random()*Om.length);this._statIdx=e,(e===9||e===10)&&(this._genStats={...this._genStats,grayAreas:Math.floor(Math.random()*3)+1,marketMakers:Math.floor(Math.random()*2)+1})}}render(){let e=j,t=this.liveValue(`game.phase`),n=this.liveValue(`game.cycle`),r=this.liveValue(`game.phaseEndTime`),i=this.liveValue(`meta.chainTimeDelta`)??0,a=this.liveValue(`game.const.seriesADuration`);if(t===void 0||n===void 0||r===void 0)return U`
      ...
      `;let y=h==null?`#ffffff`:h[0],b=h==null?`#ffffff`:h[1],x=m?this.liveValue(`token:${m}:logoUri`):void 0,S=d?.toLowerCase(),C=S?this.liveValue(`token:${S}:name`):void 0,w=S?this.liveValue(`token:${S}:symbol`):void 0,T=p?.toLowerCase(),ee=T?this.liveValue(`token:${T}:name`):void 0,E=T?this.liveValue(`token:${T}:symbol`):void 0,te=v&&t===1&&r>0n&&s>=Number(r);return U`
      <div class="container">
        <div
          class="vibecoin-hero"
        ...
        >
          ${l?U`
            ...
            <div class="vc-ticker-row">
              <span class="vc-ticker">${w===void 0?this.spin:U`\$${w}`}</span>
          ...
          `:U`
            ...
            `:U`<div class="vc-name">${ee??this.spin}</div>`}
            <div class="vc-ticker-row">
              <span class="vc-ticker">${E===void 0?this.spin:U`\$${E}`}</span>
    ...
    `}};J([q()],Am.prototype,`_scrambledTicker`,void 0),J([q()],Am.prototype,`_headingIdx`,void 0),J([q()],Am.prototype,`_statIdx`,void 0),J([q()],Am.prototype,`_genStats`,void 0),J([q()],Am.prototype,`settlePending`,void 0),Am=J([G(`pump-tab`)],Am);var jm=class extends ${constructor(...e){super(...e),this.cycle=0,this.claimAPending=!1,this.claimBPending=!1,this.claimDivPending=!1,this.optInPending=!1,this._onWalletChanged=()=>{this.requestUpdate()}}static{this.styles=[$.styles,af`
    :host { display: block; }
    ...
    .btn:disabled { opacity: 0.25; cursor: not-allowed; }
  `]}connectedCallback(){super.connectedCallback(),L.addEventListener(`wallet-changed`,this._onWalletChanged)}disconnectedCallback(){super.disconnectedCallback(),L.removeEventListener(`wallet-changed`,this._onWalletChanged)}async _claimDividends(){let e=B();if(!e||!R)return;let t=X(`game.vibecoinByCycle:${this.cycle}`);if(t){this.claimDivPending=!0;try{Np(await e.writeContract({address:t,abi:H,functionName:`claimDividends`,args:[R.address],account:R.address,chain:e.chain}),`Claim Vibes`)}catch(e){Y(e)}finally{this.claimDivPending=!1}}}async _claimA(){let e=B();if(!(!e||!R)){this.claimAPending=!0;try{Np(await e.writeContract({address:j.game,abi:V,functionName:`claimSeriesATokens`,args:[BigInt(this.cycle)],account:R.address,chain:e.chain}),`Claimed Insider Vibecoins`)}catch(e){Y(e)}finally{this.claimAPending=!1}}}async _claimB(){let e=B();if(!(!e||!R)){this.claimBPending=!0;try{Np(await e.writeContract({address:j.game,abi:V,functionName:`claimSeriesBTokens`,args:[BigInt(this.cycle)],account:R.address,chain:e.chain}),`Claimed "Insider" Vibecoins`)}catch(e){Y(e)}finally{this.claimBPending=!1}}}async _optIn(){let e=B();if(!e||!R)return;let t=X(`game.vibecoinByCycle:${this.cycle}`);if(t){this.optInPending=!0;try{Np(await e.writeContract({address:t,abi:H,functionName:`optIn`,account:R.address,chain:e.chain}),`Vibes desired`)}catch(e){Y(e)}finally{this.optInPending=!1}}}render(){if(this.cycle<=0)return U`
        <div class="card" style="border-color:#1a1a30">
          <div class="icon"></div>
          <div class="info"><div class="name-row">${this.spin}</div></div>
        </div>
      `;let e=this.liveValue(`game.vibecoinByCycle:${this.cycle}`);if(e===void 0)return U`
        <div class="card" style="border-color:#1a1a30">
          <div class="icon"></div>
          <div class="info"><div class="name-row">${this.spin}</div></div>
        </div>
      `;let t=e.toLowerCase(),n=this.liveValue(`token:${t}:name`),r=this.liveValue(`token:${t}:unlocked`),i=this.liveValue(`token:${t}:athPrice`),a=this.liveValue(`token:${t}:athPostRug`),o=this.liveValue(`token:${t}:palette`),s=this.liveValue(`token:${t}:logoUri`),c=this.liveValue(`game.winnerByCycle:${this.cycle}`),l=c?this.liveValue(`game.referralCodeOf:${c.toLowerCase()}`):void 0,u=R?.address.toLowerCase(),d=u?this.liveValue(`token:${t}:balance:${u}`):void 0,f=u?this.liveValue(`token:${t}:earned:${u}`):void 0,p=u?this.liveValue(`token:${t}:optedIn:${u}`):void 0,m=u?this.liveValue(`game.seriesADeposit:${this.cycle}:from:${u}`):void 0,h=u?this.liveValue(`game.seriesBDeposit:${this.cycle}:from:${u}`):void 0,g=o?.[0]??`#00e5ff`,_=o?.[1]??`#0066ff`,v=g+`14`,y=g+`2e`,b=g+`40`,x=g+`4d`,S=g+`66`,C=g+`99`,w=g+`2a`,T=j.explorer,ee=c!==void 0&&c!==`0x0000000000000000000000000000000000000000`,E=R&&(d!==void 0&&d>0n||ee);return U`
          ...
          <div class="stats">
            ...
            ${ee?U`
              ...
              <span class="stat-val">
                ...
                ${l?U`<span style="color:#ffe600">${l}</span> `:``}
                ${T?U`
                  <a style="color:#ff2d6b;text-decoration:none" href="${T}/address/${c}" target="_blank">${Tp(c)}</a>
                `:U`<span style="color:#ff2d6b">${Tp(c)}</span>`}
              </span>
            `:``}
          </div>
        </div>
    ...
    `}};J([K({type:Number})],jm.prototype,`cycle`,void 0),J([q()],jm.prototype,`claimAPending`,void 0),J([q()],jm.prototype,`claimBPending`,void 0),J([q()],jm.prototype,`claimDivPending`,void 0),J([q()],jm.prototype,`optInPending`,void 0),jm=J([G(`vibecoin-card`)],jm);var Mm=20,Nm=class extends ${constructor(...e){super(...e),this.search=``,this.searchResults=null,this.visibleCount=Mm,this.observer=null,this.sentinelEl=null,this._searchDebounce=null,this._onHashChange=()=>this._applyHash()}static{this.styles=[$.styles,af`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),window.addEventListener(`hashchange`,this._onHashChange),this._applyHash()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._onHashChange),this.observer?.disconnect(),this._searchDebounce&&clearTimeout(this._searchDebounce)}_applyHash(){let e=window.location.hash.split(`?`)[0].match(/^#vibecoins\/(0x[0-9a-fA-F]{40})$/);e&&this._setSearch(e[1])}_setSearch(e){if(this.search=e,this.visibleCount=Mm,this._searchDebounce&&clearTimeout(this._searchDebounce),!e.trim()){this.searchResults=null;return}this.searchResults=null,this._searchDebounce=setTimeout(()=>void this._doSearch(e.trim().toLowerCase()),200)}async _doSearch(e){let t=await zp(e);this.search.trim().toLowerCase()===e&&(this.searchResults=t??[])}updated(){let e=this.renderRoot?.querySelector(`.sentinel`);e&&e!==this.sentinelEl&&(this.observer?.disconnect(),this.sentinelEl=e,this.observer=new IntersectionObserver(e=>{e[0].isIntersecting&&(this.visibleCount+=Mm)},{rootMargin:`200px`}),this.observer.observe(e))}render(){let e=this.liveValue(`game.cycle`),t=this.liveValue(`game.phase`),n=this.search.trim()!==``,r,i;if(n)r=this.searchResults??[],i=this.searchResults===null;else if(e===void 0||t===void 0)r=[],i=!0;else{let n=t>=2?Number(e):Number(e)-1;r=[];for(let e=n;e>=1;e--)r.push(e);i=!1}let a=r.slice(0,this.visibleCount);return U`
      <div class="container">
        ...
        `:U`
          <div class="list">
            ${a.map(e=>U`<vibecoin-card .cycle=${e}></vibecoin-card>`)}
            ${a.length<r.length?U`<div class="sentinel"></div>`:``}
          </div>
        `}
      </div>
    `}};J([q()],Nm.prototype,`search`,void 0),J([q()],Nm.prototype,`searchResults`,void 0),J([q()],Nm.prototype,`visibleCount`,void 0),Nm=J([G(`vibecoins-tab`)],Nm);function Pm(e){if(e<0n)throw Error(`sqrt of negative`);if(e===0n)return 0n;let t=e,n=(t+1n)/2n;for(;n<t;)t=n,n=(t+e/t)/2n;return t}function Fm(e,t,n,r){let i=10n**18n,a=n*t/i+r*t*t/(2n*i*i)+e,o=Pm(n*n+2n*r*a);if(r===0n)return 0n;let s=(o-n)*i/r;return s>t?s-t:0n}var Im=class extends ${constructor(...e){super(...e),this.visible=!1,this.target=`publicBuy`,this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.approved=!1,this.error=``,this._sellShMonPreview=0n,this._tokenBalance=0n,this._refCode=null}static{this.styles=[$.styles,af`
    :host { display: block; }
  ...
  `]}open(e){this.target=e.target,this.vibecoinAddress=e.vibecoinAddress,this._tokenBalance=e.tokenBalance??0n,this._refCode=yd(),this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.approved=!1,this.error=``,this._sellShMonPreview=0n,this.visible=!0}async _updateSellPreview(){let e=this._amountWei;if(e===0n||!this.vibecoinAddress){this._sellShMonPreview=0n;return}try{this._sellShMonPreview=await ad().readContract({address:this.vibecoinAddress,abi:H,functionName:`shMonForTokens`,args:[e]})}catch{this._sellShMonPreview=0n}}_close(){this.visible=!1}get _amountWei(){let[e,t,n]=this.amountStr.match(/^([0-9]+)\.?([0-9]+)?$/)??[];return t?(n=n?n.slice(0,18).padEnd(18,`0`):`0`,BigInt(t)*10n**18n+BigInt(n)):0n}_effectiveVcAddrLow(){return this.vibecoinAddress?this.vibecoinAddress.toLowerCase():(this.liveValue(`game.vibecoin`)??``).toLowerCase()}_getTitle(){let e=this._effectiveVcAddrLow(),t=e?this.liveValue(`token:${e}:name`):void 0;switch(this.target){case`seriesA`:return`INSIDER ROUND`;case`seriesB`:return`"INSIDER" ROUND`;case`publicBuy`:return t?`PUMP ${t.toUpperCase()}`:`PUMP`;case`vibecoin`:return t?`BUY ${t.toUpperCase()}`:`BUY`;case`vibecoinSell`:return t?`SELL ${t.toUpperCase()}`:`SELL`}}_getFeeBreakdown(){if(this.target===`vibecoin`)return[[`Dividend pool`,1000n],[`To buyer`,9000n]];switch(this.target){case`seriesA`:{let e=this.liveValue(`game.const.seriesAProtocol`)??400n,t=this.liveValue(`game.const.seriesAReferral`)??600n,n=this.liveValue(`game.const.seriesAPrize`)??2000n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoins minted`,10000n-e-t-n]]}case`seriesB`:{let e=this.liveValue(`game.const.seriesBProtocol`)??400n,t=this.liveValue(`game.const.seriesBReferral`)??600n,n=this.liveValue(`game.const.seriesBPrize`)??1000n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoins minted`,10000n-e-t-n]]}default:{let e=this.liveValue(`game.const.pumpProtocol`)??400n,t=this.liveValue(`game.const.pumpReferral`)??600n,n=this.liveValue(`game.const.pumpPrize`)??2400n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoin mint`,10000n-e-t-n]]}}}_computeTokenPreview(){let e=this._amountWei;if(e===0n)return 0n;let t=_p();if(!this.useShMon&&t===0n)return;let n=this.useShMon?e:e*10n**18n/t;if(this.target===`publicBuy`){let e=this.liveValue(`game.vibecoin`);if(!e)return;let t=e.toLowerCase(),r=this.liveValue(`token:${t}:totalSupply`),i=this.liveValue(`token:${t}:P0`),a=this.liveValue(`token:${t}:K`);return r===void 0||i===void 0||a===void 0?void 0:Fm(n*(this._getFeeBreakdown().find(([e])=>e===`Vibecoin mint`)?.[1]??6600n)/10000n,r,i,a)}if(this.target===`vibecoin`){let e=this.vibecoinAddress?.toLowerCase();if(!e)return;let t=this.liveValue(`token:${e}:totalSupply`),r=this.liveValue(`token:${e}:P0`),i=this.liveValue(`token:${e}:K`);return t===void 0||r===void 0||i===void 0?void 0:Fm(n,t,r,i)}return 0n}_computeTimerExtension(){let e=this.liveValue(`game.settlementTimestamp`);if(e===void 0)return;if(e===0n)return`--`;let t=this._amountWei;if(t===0n)return`--`;let n=this.liveValue(`game.cycle`);if(n===void 0)return;let r=this.liveValue(`game.vibecoin`)?.toLowerCase(),i=this.liveValue(`game.seriesADeposit:${n}`),a=this.liveValue(`game.seriesBDeposit:${n}`),o=r?this.liveValue(`token:${r}:poolBalance`):void 0,s=this.liveValue(`game.phaseEndTime`);if(i===void 0||a===void 0||o===void 0||s===void 0)return;let c=Number(i+a)/0xde0b6b3a7640000;if(c===0)return`--`;let l=_p();if(!this.useShMon&&l===0n)return;let u=this.useShMon?t:t*10n**18n/l,d=Number(o+u)/0xde0b6b3a7640000+1e4,f=c*2419200/(c+2e6)*d*d*1e3/(d*d+40*c*c),p=Number(e)*1e3+36e5+f,m=Math.max(0,p-Number(s)*1e3);if(m===0)return`at max`;let h=Math.floor(m/1e3),g=Math.floor(h/3600),_=Math.floor(h%3600/60);return g>0?`+${g}h ${_}m`:_>0?`+${_}m ${h}s`:`+${h}.${Math.floor(m)%1e3}s`}async _approve(){let e=j,t=B();if(!t||!R)return;let n=this._amountWei;if(n!==0n){this.approvePending=!0,this.error=``;try{let r=this.target===`vibecoin`?this.vibecoinAddress:e.game;await t.writeContract({address:e.shmon,abi:ud,functionName:`approve`,args:[r,n],account:R.address,chain:t.chain}),this.approved=!0}catch(e){Y(e)}finally{this.approvePending=!1}}}async _buy(){let e=j,t=B();if(!t||!R)return;let n=this._amountWei;if(n!==0n){if(this.target===`seriesA`||this.target===`seriesB`){let e=this.target===`seriesA`?0:1,t=X(`game.phase`);if(t===void 0||t!==e){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}let n=X(`game.phaseEndTime`)??0n,r=X(`meta.chainTimeDelta`)??0;if(n>0n&&Date.now()/1e3+r>=Number(n)){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}}this.pending=!0,this.error=``;try{let r=Cd(yd()??``),i;i=this.target===`vibecoinSell`?await t.writeContract({address:this.vibecoinAddress,abi:H,functionName:`sell`,args:[n,R.address],account:R.address,chain:t.chain}):this.useShMon?this.target===`seriesA`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesAWithShMon`,args:[r,n],account:R.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesBWithShMon`,args:[r,n],account:R.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:V,functionName:`buyWithShMon`,args:[r,n],account:R.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:H,functionName:`buyWithShMon`,args:[R.address,n,!0],account:R.address,chain:t.chain}):this.target===`seriesA`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesA`,args:[r],value:n,account:R.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:V,functionName:`depositSeriesB`,args:[r],value:n,account:R.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:V,functionName:`buy`,args:[r],value:n,account:R.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:H,functionName:`buy`,args:[R.address,!0],value:n,account:R.address,chain:t.chain}),this._close(),Np(i,this._actionLabel())}catch(e){Y(e)}finally{this.pending=!1}}}_actionLabel(){let e=this._effectiveVcAddrLowCached(),t=e?X(`token:${e}:symbol`)??`token`:`token`,n=e?X(`token:${e}:name`)??``:``,r=this.useShMon?yp(this._amountWei):bp(this._amountWei);switch(this.target){case`seriesA`:return`${r} insider deposit`;case`seriesB`:return`${r} "insider" deposit`;case`publicBuy`:return`Spend ${r} pumping ${n}`;case`vibecoin`:return`Spend ${r} on ${t}`;case`vibecoinSell`:return`Sell ${Cp(this._amountWei,t)} for shMON`}}_effectiveVcAddrLowCached(){return this.vibecoinAddress?this.vibecoinAddress.toLowerCase():(X(`game.vibecoin`)??``).toLowerCase()}_renderPreview(){let e=this._amountWei,t=this._effectiveVcAddrLow(),n=t?this.liveValue(`token:${t}:symbol`)??`???`:`???`;if(this.target===`vibecoinSell`)return U`
        <div class="preview">
      ...
      <div class="preview">
        ...
        <div class="fee-section">
          ${r.map(([t,r])=>{let a=this.target===`vibecoin`&&i!==void 0&&i>0n?` · ${Cp(i*r/10000n,n)}`:this.target!==`vibecoin`&&e>0n?` · ${this.useShMon?xp(e*r/10000n):bp(e*r/10000n)}`:``;return U`
              <div class="preview-row">
      ...
      `:``}
    `}render(){if(!this.visible)return U``;let e=this.target===`vibecoinSell`,t=!e&&this.useShMon,n=!!R,r=this.target===`seriesA`||this.target===`seriesB`,i=e?this._tokenBalance:0n,a=this._effectiveVcAddrLow(),o=a?this.liveValue(`token:${a}:symbol`)??`???`:`???`;return U`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="modal">
          ...
          <div class="field">
            <label style="display:flex;justify-content:space-between">
              ...
              ${e&&i>0n?U`<span style="color:#d8d8e8">BAL: ${Cp(i,o)}</span>`:``}
            </label>
          ...
          ${this.error?U`<div class="error">${this.error}</div>`:``}
        </div>
      </div>
    `}};J([q()],Im.prototype,`visible`,void 0),J([q()],Im.prototype,`target`,void 0),J([q()],Im.prototype,`vibecoinAddress`,void 0),J([q()],Im.prototype,`amountStr`,void 0),J([q()],Im.prototype,`useShMon`,void 0),J([q()],Im.prototype,`pending`,void 0),J([q()],Im.prototype,`approvePending`,void 0),J([q()],Im.prototype,`approved`,void 0),J([q()],Im.prototype,`error`,void 0),J([q()],Im.prototype,`_sellShMonPreview`,void 0),J([q()],Im.prototype,`_tokenBalance`,void 0),Im=J([G(`buy-dialog`)],Im);var Lm=`disclaimer-accepted-v1`,Rm=class extends lp{constructor(...e){super(...e),this.open=!localStorage.getItem(Lm)}_accept(){localStorage.setItem(Lm,`1`),this.open=!1}static{this.styles=af`
    .overlay {
  ...
  `}_renderContent(){return U`
      ...
      `}render(){if(!this.open)return U``;let e={10143:`TESTNET — HEADS UP`,143:`DISCLAIMER`}[10143]??`YOU FOUND THE DEV BUILD`,t={10143:`GOT IT — LET ME PLAY`,143:`I ACCEPT — LET ME PLAY`}[10143]??`LETS GO`;return U`
      <div class="overlay">
        <div class="dialog">
  ...
  `}connectedCallback(){super.connectedCallback(),this._handleHash(),window.addEventListener(`hashchange`,this._handleHash),L.addEventListener(`wallet-changed`,this._onWalletChanged),window.addEventListener(`open-buy-dialog`,this._onOpenBuyDialog)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._handleHash),L.removeEventListener(`wallet-changed`,this._onWalletChanged),window.removeEventListener(`open-buy-dialog`,this._onOpenBuyDialog)}render(){return U`
      <header-bar></header-bar>
      <main>
        ${this.tab===`pump`?U`<pump-tab></pump-tab>`:``}
        ${this.tab===`vibecoins`?U`<vibecoins-tab></vibecoins-tab>`:``}
      </main>
      <buy-dialog></buy-dialog>
      <tx-confirm-dialog></tx-confirm-dialog>
      <disclaimer-modal></disclaimer-modal>
    `}};J([q()],zm.prototype,`tab`,void 0),zm=J([G(`app-root`)],zm);function Bm(e){let t=e instanceof Error?e.message:String(e),n=e instanceof Error?e.stack??e.message:String(e);window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:t,full:n,name:`UNCAUGHT ERROR`}}))}window.addEventListener(`error`,e=>Bm(e.error??e.message)),window.addEventListener(`unhandledrejection`,e=>Bm(e.reason)),tm();var Vm=document.getElementById(`loading-spinner`);Vm&&Vm.remove(),X(`meta.shmonRate`,!0,document.documentElement);var Hm=document.body,Um=document.createElement(`app-background`);Hm.insertBefore(Um,Hm.firstChild);var Wm=document.createElement(`div`);Wm.id=`app`;var Gm=document.createElement(`app-root`);Wm.appendChild(Gm),Hm.appendChild(Wm);