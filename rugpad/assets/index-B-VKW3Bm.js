...
`)[0]||String(e),a=typeof t?.message==`string`&&t.message||String(e),o=n||typeof t?.name==`string`&&t.name!==`Error`&&t.name||`ERROR`;window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:i,full:a,name:o}}))}var Np=class extends up{constructor(...e){super(...e),this.open=!1,this.hash=null,this.action=``,this.transfers=null,this.errorMsg=``,this.errorFull=``,this.errorName=``,this._onError=e=>{this.errorMsg=e.detail.short,this.errorFull=e.detail.full,this.errorName=e.detail.name,this.hash=null,this.open=!0},this._onShow=e=>{this.hash=e.detail.hash,this.action=e.detail.action,this.transfers=null,this.open=!0,this._fetchTransfers(e.detail.hash)}}connectedCallback(){super.connectedCallback(),window.addEventListener(`tx-confirm-show`,this._onShow),window.addEventListener(`app-error-show`,this._onError)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`tx-confirm-show`,this._onShow),window.removeEventListener(`app-error-show`,this._onError)}async _fetchTransfers(e){let{connectedWallet:t}=await Xt(async()=>{let{connectedWallet:e}=await Promise.resolve().then(()=>Ku);return{connectedWallet:e}},void 0,import.meta.url);if(!t){this.transfers=[];return}let n=A,r=od(143),i=t.address.slice(2).toLowerCase(),a;for(let t=0;t<20;t++){try{if(a=await r.getTransactionReceipt({hash:e}),a)break}catch{}await new Promise(e=>setTimeout(e,800))}if(!a){this.transfers=[];return}let o=a.logs.filter(e=>e.topics[0]?.toLowerCase()===jp&&e.topics[2]?.slice(-40).toLowerCase()===i);if(o.length===0){this.transfers=[];return}let s=[];for(let e of o){let t=e.address,i=`0x`+(e.topics[1]??`0x`).slice(-40),a=BigInt(e.data),o;if(t.toLowerCase()===n.fomon.toLowerCase())o=`foMON`;else if(t.toLowerCase()===n.shmon.toLowerCase())o=`shMON`;else{let e=Y(`token:${t.toLowerCase()}:symbol`);if(e)o=e;else try{o=await r.readContract({address:t,abi:B,functionName:`symbol`})}catch{o=t.slice(0,6)+`...`}}let c=i.toLowerCase()===Mp&&t.toLowerCase()===n.fomon.toLowerCase();s.push({symbol:o,amount:a,isAirdrop:c})}this.transfers=s}_close(){this.open=!1}_breakPascal(e){return e.replace(/([a-z])([A-Z])/g,`$1​$2`)}async _copy(){this.hash&&await navigator.clipboard.writeText(this.hash)}static{this.styles=of`
  ...
  `}render(){if(!this.open)return V``;if(!this.hash)return V`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="dialog error">
          ...
          ${this.errorFull&&this.errorFull!==this.errorMsg?V`
            <div class="err-detail">${this.errorFull}</div>
          `:``}
        </div>
      </div>
    `;let e=A,t=this.hash.slice(0,10)+`…`+this.hash.slice(-6),n=e.explorer?`${e.explorer}/tx/${this.hash}`:null;return V`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="dialog">
          <button class="close" @click=${this._close}>×</button>
          <div class="check">${this.transfers===null?V`<span class="spinner" style="display:inline-block;vertical-align:middle;margin-right:6px"></span>PENDING…`:`✓ CONFIRMED`}</div>
          <div class="action">${this.action}</div>

          <div class="hash-row">
            <span class="hash">${t}</span>
            <button class="copy-btn" @click=${this._copy}>copy</button>
            ${n?V`<a class="explorer-link" href=${n} target="_blank">view ↗</a>`:``}
          </div>

          ${this.transfers===null||this.transfers.length>0?V`
            <hr class="divider"/>
            <div class="transfers-title">token transfers</div>
            ${this.transfers===null?V`<div class="loading"><div class="spinner"></div>awaiting confirmation…</div>`:this.transfers.map(e=>V`
                <div class="transfer">
                  <span class="plus">+</span>
                  <span class="t-amount">${Tp(e.amount)}</span>
                  <span class="t-symbol">${e.symbol}</span>
                  ${e.isAirdrop?V`<span class="airdrop-badge">airdrop</span>`:``}
                </div>
              `)}
          `:``}
        </div>
      </div>
    `}};K([G()],Np.prototype,`open`,void 0),K([G()],Np.prototype,`hash`,void 0),K([G()],Np.prototype,`action`,void 0),K([G()],Np.prototype,`transfers`,void 0),K([G()],Np.prototype,`errorMsg`,void 0),K([G()],Np.prototype,`errorFull`,void 0),K([G()],Np.prototype,`errorName`,void 0),Np=K([U(`tx-confirm-dialog`)],Np);var Pp=null,Fp=!1;async function Ip(){if(!Pp||Fp)return;Fp=!0;let e=await nr(()=>L(e=>e.readContract({address:A.game,abi:z,functionName:`cycle`})),!1,`vibecoin_db_cycle`),t=!1,n=e;for(;n>0n;){let r=await Pp.getFromIndex(`vibecoins`,`by-cycle`,Number(n));if(r!=null){if(!t){let i=await nr(()=>L(e=>e.readContract({address:A.game,abi:z,functionName:`vibecoinCycle`,args:[r.address]})),!1,`vibecoin_db_sanity_check`);if(i!=n){console.warn(`Search database is invalid. Vibecoin `+r.address+` was expected to made by cycle `+n+`, but it was actually in cycle `+i),n=e,await Pp.clear(`vibecoins`);continue}t=!0}n--;continue}let i=await nr(()=>L(e=>e.readContract({address:A.game,abi:z,functionName:`vibecoinByCycle`,args:[n]})),!1,`vibecoin_db_cycle_`+n);if(!i||i==`0x0000000000000000000000000000000000000000`){n--;continue}let[a,o]=await Promise.all([nr(()=>L(e=>e.readContract({address:i,abi:B,functionName:`name`})),!0,`vibecoin_db_cycle_`+n),nr(()=>L(e=>e.readContract({address:i,abi:B,functionName:`symbol`})),!0,`vibecoin_db_cycle_`+n)]);await Pp.put(`vibecoins`,{address:i.toLowerCase(),cycle:Number(n),name:a.toLowerCase(),symbol:o.toLowerCase()}),n--}Fp=!1}window.addEventListener(`lls-updated`,({detail:e})=>{for(let[t,n]of e)t==`game.phase`&&n==2&&Ip().catch(e=>{J(e)})}),(async()=>{Pp=await Vd(`rugpad`,1,{upgrade(e){let t=e.createObjectStore(`vibecoins`,{keyPath:`address`});t.createIndex(`by-cycle`,`cycle`),t.createIndex(`by-symbol`,`symbol`),t.createIndex(`by-name`,`name`)}}),await Ip()})().catch(e=>{J(e)});function Lp(e){if(!e)return IDBKeyRange.lowerBound(``);let t=e.length-1,n=e.substring(0,t)+String.fromCharCode(e.charCodeAt(t)+1);return IDBKeyRange.bound(e,n,!1,!0)}async function Rp(e){let t=Lp(e);if(!Pp)return null;if(e.startsWith(`0x`))return(await Pp.getAll(`vibecoins`,t,10)).map(e=>e.cycle);let n=(await Pp.getAllFromIndex(`vibecoins`,`by-symbol`,t,10)).map(e=>e.cycle);n.length<10&&n.push(...(await Pp.getAllFromIndex(`vibecoins`,`by-name`,t,10-n.length)).map(e=>e.cycle));for(let e=0;e<n.length;e+=1)n.indexOf(n[e],e+1)>=0&&(n.splice(e,1),--e);return n}async function zp(e){if(e=Number(e),Pp)return(await Pp.getFromIndex(`vibecoins`,`by-cycle`,e))?.address}var Bp=300*1e3,Vp=30*1e3,Hp=3333;function Up(){Z(`meta.chainTimeDelta`,[`meta.chainTimeDelta`],async()=>{let e=await L(e=>e.getBlock({blockTag:`latest`})),t=Number(e.timestamp)-Date.now()/1e3;return t=t<60?0:Math.round(t/10)*10,[t]},X.Static),Z(`meta.shmonRate`,[`meta.shmonRate`],async()=>[await L(e=>e.readContract({address:A.shmon,abi:dd,functionName:`convertToAssets`,args:[10n**18n]}))],X.Live,Bp),Z(`game.settlementTimestamp`,[`game.settlementTimestamp`],async()=>[await L(e=>e.readContract({address:A.game,abi:z,functionName:`settlementTimestamp`,args:[]}))],X.Live,Vp),Z(`game.maxRoundDuration`,[`game.maxRoundDuration`],async()=>[await L(e=>e.readContract({address:A.game,abi:z,functionName:`maxRoundDuration`,args:[]}))],X.Live,Vp),Z(`game.state`,[`game.phase`,`game.cycle`,`game.phaseEndTime`,`game.prizePool`,`game.lastBuyer`,`game.vibecoin`,`game.isSettled`],async()=>{let e=await L(e=>e.readContract({address:A.game,abi:z,functionName:`gameState`,args:[!1]})),t=e.phase===3;return[e.phase,e.cycle,e.phaseEndTime,e.prizePool,e.lastBuyer,e.vibecoin.toLowerCase(),t]},X.Live),Z(`game.vibecoinByCycle$`,[`game.vibecoinByCycle$`],async e=>{let t=await zp(Number(e));return t===`0x0000000000000000000000000000000000000000`?[void 0]:[t]},X.StaticWhenKnown),Z(`game.accountInfo$from$`,[`game.seriesADeposit$from$`,`game.seriesBDeposit$from$`,`game.hasClaimedSeriesA$from$`,`game.hasClaimedSeriesB$from$`],async(e,t)=>{let n=await L(n=>n.readContract({address:A.game,abi:z,functionName:`accountInfo`,args:[BigInt(e),t]}));return[n[0],n[1],n[3],n[4]]},X.Live,Hp),Z(`game.seriesADeposit$`,[`game.seriesADeposit$`],async e=>{let t=BigInt(e),n=Y(`game.cycle`),r=Y(`game.phase`);return n==null||r==null?[void 0]:t==n&&r<2?[await L(e=>e.readContract({address:A.game,abi:z,functionName:`totalSeriesA`,args:[]}))]:[await L(e=>e.readContract({address:A.game,abi:z,functionName:`totalSeriesAByCycle`,args:[t]}))]},X.Live,Vp,[`game.cycle`,`game.phase`]),Z(`game.seriesBDeposit$`,[`game.seriesBDeposit$`],async e=>{let t=BigInt(e),n=Y(`game.cycle`),r=Y(`game.phase`);return n==null||r==null?[void 0]:t==n&&r<2?[await L(e=>e.readContract({address:A.game,abi:z,functionName:`totalSeriesB`,args:[]}))]:[await L(e=>e.readContract({address:A.game,abi:z,functionName:`totalSeriesBByCycle`,args:[t]}))]},X.Live,Vp,[`game.cycle`,`game.phase`]);for(let e of[`tokensSeriesAByCycle`,`tokensSeriesBByCycle`,`winnerByCycle`,`prizeByCycle`])Z(`game.`+e+`$`,[`game.`+e+`$`],async t=>{let n=BigInt(t);return[await L(t=>t.readContract({address:A.game,abi:z,functionName:e,args:[n]}))]},X.Live,Vp);Z(`game.referralEarned$`,[`game.referralEarned$`],async e=>[await L(t=>t.readContract({address:A.game,abi:z,functionName:`referralEarned`,args:[e]}))],X.Live,Vp),Z(`game.referralCodeOf$`,[`game.referralCodeOf$`],async e=>[_d(await L(t=>t.readContract({address:A.game,abi:z,functionName:`referralCodeOf`,args:[e]})))||``],X.Live,Vp),Z(`game.settlementTimestamp`,[`game.settlementTimestamp`],async()=>[await L(e=>e.readContract({address:A.game,abi:z,functionName:`settlementTimestamp`,args:[]}))],X.Live,Vp),Z(`game.consts`,[`game.const.seriesADuration`,`game.const.seriesAProtocol`,`game.const.seriesAReferral`,`game.const.seriesAPrize`,`game.const.seriesBProtocol`,`game.const.seriesBReferral`,`game.const.seriesBPrize`,`game.const.pumpProtocol`,`game.const.pumpReferral`,`game.const.pumpPrize`],async()=>{let e=[`SERIES_A_DURATION`,`SERIES_A_PROTOCOL`,`SERIES_A_REFERRAL`,`SERIES_A_PRIZE`,`SERIES_B_PROTOCOL`,`SERIES_B_REFERRAL`,`SERIES_B_PRIZE`,`PUMP_PROTOCOL`,`PUMP_REFERRAL`,`PUMP_PRIZE`],t=[];for(let n of e)try{t.push(await L(e=>e.readContract({address:A.game,abi:z,functionName:n}))),await new Promise(e=>setTimeout(e,95))}catch{t.push(void 0)}return t},X.StaticWhenKnown),Z(`game.const.minBuy`,[`game.const.minBuy`],async()=>[await L(e=>e.readContract({address:A.game,abi:z,functionName:`MIN_BUY`}))],X.StaticWhenKnown),Z(`game.const.seriesAKickoff`,[`game.const.seriesAKickoff`],async()=>[await L(e=>e.readContract({address:A.game,abi:z,functionName:`SERIES_A_KICKOFF`}))],X.StaticWhenKnown),Z(`game.const.initialTimer`,[`game.const.initialTimer`],async()=>[await L(e=>e.readContract({address:A.game,abi:z,functionName:`INITIAL_TIMER`}))],X.StaticWhenKnown),Z(`token$allowance$spender$`,[`token$allowance$spender$`],async(e,t,n)=>[await L(r=>r.readContract({address:e,abi:B,functionName:`allowance`,args:[t,n]}))],X.Live,Hp),Z(`token$balance$`,[`token$balance$`],async(e,t)=>e==`native`?[await L(e=>e.getBalance({address:t}))]:[await L(n=>n.readContract({address:e,abi:B,functionName:`balanceOf`,args:[t]}))],X.Live,Hp),Z(`token$earned$`,[`token$earned$`],async(e,t)=>[await L(n=>n.readContract({address:e,abi:B,functionName:`earned`,args:[t]}))],X.Live,Hp),Z(`token$pendingRewards$`,[`token$pendingRewards$`],async(e,t)=>[await L(n=>n.readContract({address:e,abi:ud,functionName:`pendingRewards`,args:[t]}))],X.Live,Hp),Z(`token$optedIn$`,[`token$optedIn$`],async(e,t)=>[await L(n=>n.readContract({address:e,abi:ud,functionName:`optedIn`,args:[t]}))],X.Live,Hp),Z(`token$seed`,[`token$seed`,`token$palette`],async e=>{let t=await L(t=>t.readContract({address:e,abi:B,functionName:`seed`,args:[]}));return[t,pd(t)]},X.StaticWhenKnown);for(let e of[`name`,`symbol`,`K`,`P0`])Z(`token$`+e,[`token$`+e],async t=>[await L(n=>n.readContract({address:t,abi:B,functionName:e,args:[]}))],X.StaticWhenKnown);Z(`token$logoUri`,[`token$logoUri`],async e=>{let t=await L(t=>t.readContract({address:e,abi:B,functionName:`tokenURI`,args:[]}));return t.startsWith(`data:application/json`)?[JSON.parse(atob(t.split(`,`)[1])).image]:[void 0]},X.StaticWhenKnown);for(let e of[`totalSupply`,`poolBalance`,`athPrice`,`totalVolume`,`volume24h`])Z(`token$`+e,[`token$`+e],async t=>[await L(n=>n.readContract({address:t,abi:B,functionName:e,args:[]}))],X.Live,Vp);for(let e of[`unlocked`,`athPostRug`])Z(`token$`+e,[`token$`+e],async t=>{let n=await L(n=>n.readContract({address:t,abi:B,functionName:e,args:[]}));return n&&ir.remove(`token:`+t+`:`+e),[n]},X.Live,Vp);Z(`fomon.consts`,[`fomon.const.airdropReserve`,`fomon.const.totalSupply`],async()=>{let e=[`AIRDROP_RESERVE`,`TOTAL_SUPPLY`],t=[];for(let n of e)try{t.push(await L(e=>e.readContract({address:A.fomon,abi:ud,functionName:n}))),await new Promise(e=>setTimeout(e,95))}catch{t.push(void 0)}return t},X.StaticWhenKnown),Z(`fomon.minted`,[`fomon.airdropMinted`,`fomon.airdropRemaining`],async()=>{let e=await L(e=>e.readContract({address:A.fomon,abi:ud,functionName:`totalAirdropMinted`})),t=Y(`fomon.const.airdropReserve`);return[e,t==null?void 0:t-e]},X.Live,0,[`fomon.const.airdropReserve`]),Z(`fomon.mintable`,[`fomon.mintable`],async()=>[await L(e=>e.readContract({address:A.fomon,abi:ud,functionName:`remainingCeiling`}))],X.Live),Z(`fomon.mintableThisRound`,[`fomon.mintableThisRound`],async()=>{let e=Y(`game.maxRoundDuration`),t=Y(`game.settlementTimestamp`),n=Y(`game.const.initialTimer`);return!t||!e||!n?[void 0]:[await L(r=>r.readContract({address:A.fomon,abi:ud,functionName:`remainingCeilingAt`,args:[t+n+e]}))]},X.Live,100,[`game.maxRoundDuration`,`game.settlementTimestamp`,`game.const.initialTimer`])}var Wp=new Map([[`token:native:name`,`Monad`],[`token:native:symbol`,`MON`],[`token:${A.shmon.toLowerCase()}:symbol`,`shMON`],[`token:${A.fomon.toLowerCase()}:symbol`,`foMON`]]),Gp=new Map,Kp=new Set,qp=new Map,Jp=new Map,Yp=new Map,Xp=2,Zp=bd();Qn(`rugpad_storage_revision`)!==Xp&&(localStorage.clear(),xd(Zp),$n(`rugpad_storage_revision`,Xp));function Qp(e){let t=[],n=``,r=0,i=e.indexOf(`:`);for(;i!=-1;)n+=e.substring(r,i)+`$`,r=i+1,i=e.indexOf(`:`,r),i==-1&&(i=e.length),t.push(e.substring(r,i)),r=i+1,i=e.indexOf(`:`,r);return n+=e.substring(r),[n,t]}function $p(e,t){t=[...t].reverse();let n=e.replaceAll(`$`,()=>`:`+(t.pop()??console.warn(e,`fed too little arguments`)??``)+`:`);return n.endsWith(`:`)?n.substring(0,n.length-1):n}function Y(e,t,...n){let r=Kp.has(e);if(r){let[t,r]=Qp(e);em($p(Gp.get(t),r),n)}let i=Wp.get(e);if(i!==void 0)return i;if(i=Qn(`lls:143:`+e),i!==void 0)return Wp.set(e,i),i;if(!r){let[r,i]=Qp(e),a=Gp.get(r);if(!a){console.error(e,`has no associated task! This may result in an infinite loading circle.`);return}let o=qp.get(a);if(!o){console.error(a,`was not defined! This may result in an infinite loading circle.`);return}Kp.add(e);let s=$p(a,i);for(let e of Jp.get(a)||[]){Y($p(e,i),t,...n);let a=Gp.get(r);if(!a)continue;let o=$p(a,i),c=Yp.get(s);c||(c=new Set,Yp.set(s,c)),c.add(o)}let c=o.fn,l={name:s,fn:()=>c(...i),repeatMs:o.repeatMs};t?ir.prepend(l):ir.upsert(l),ir.elementRequestedTask(s,...n)}}function em(e,t,n=new Set){if(!n.has(e)){n.add(e),ir.elementRequestedTask(e,...t);for(let r of Yp.get(e)||[])em(r,t,n)}}var X=function(e){return e[e.Static=0]=`Static`,e[e.StaticWhenKnown=1]=`StaticWhenKnown`,e[e.Live=2]=`Live`,e}({});function Z(e,t,n,r,i=0,a=[]){Jp.set(e,a),qp.set(e,{template:e,fn:async(...i)=>{let a=await n(...i),o=!1,s=new Map;for(let e=0;e<t.length;e+=1){let n=t[e],c=$p(n,i),l=Wp.get(c),u=a[e];l!==u&&s.set(c,u),u===void 0?o=!0:(Wp.set(c,u),r<=X.StaticWhenKnown&&$n(`lls:143:`+c,u))}if(r==X.StaticWhenKnown&&!o){let t=$p(e,i);ir.remove(t)}s.size&&window.dispatchEvent(new CustomEvent(`lls-updated`,{detail:s}))},repeatMs:r==X.Static?void 0:i});for(let n of t)Gp.set(n,e)}function tm(){Up()}var nm=25,rm=5,im=1,am=60,om=90,sm=class extends HTMLElement{constructor(...e){super(...e),this._phosphor=null,this._rafId=0,this._marqueeX=0,this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`,this._candles=[],this._loadingVcAddr=``,this._currentVcAddr=``,this._K=0n,this._P0=0n,this._onLiveUpdate=()=>{let e=Y(`game.phase`,!1,this),t=Y(`game.cycle`,!1,this),n=Y(`game.phaseEndTime`,!1,this),r=Y(`meta.chainTimeDelta`,!1,this)??0,i=Number(Y(`game.const.seriesADuration`,!1,this));if(e===void 0||t===void 0||n==null||isNaN(i)){this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`;return}let a=Date.now()/1e3+r,o=e===0&&n>0n?Number(n)-a:1/0,s=e===0&&t>1n&&(n===0n||o>i/2),c=s?t-1n:t,l=s?3:e,u=s||l===2||l===3?Y(`game.vibecoinByCycle:${c}`,!1,this):l===1?Y(`game.vibecoin`,!1,this):void 0;if(u===void 0){this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`;return}let d=u.toLowerCase();if(l===2&&a<Number(n)&&d!==`0x0000000000000000000000000000000000000000`){let e=Y(`token:${d}:name`,!1,this),t=Y(`token:${d}:symbol`,!1,this);e===void 0||t===void 0?(this._marqueeText=`VIBE CHECKING...`,this._marqueeColor=`rgba(255,230,0,0.9)`):(this._marqueeText=`PUMP ${e}! — $${t} TO THE MOON`,this._marqueeColor=`rgba(0,229,255,0.9)`)}else l===3||l===2&&a>=Number(n)?(this._marqueeText=`GET RUGGED`,this._marqueeColor=`rgba(255,45,107,0.9)`):(this._marqueeText=`GENERATING VIBECOIN`,this._marqueeColor=`rgba(255,230,0,0.9)`);let f=Y(`token:${d}:K`,!1,this),p=Y(`token:${d}:P0`,!1,this);if(u!==this._currentVcAddr&&d!==`0x0000000000000000000000000000000000000000`&&p!=null&&f!=null){this._K=f,this._P0=p,this._currentVcAddr=u,this._candles=[];let e=this._buf.width||Math.ceil(window.innerWidth/nm);this._startCandleLoad(u,this._K,this._P0,Math.ceil(e/rm)+2)}let m=Y(`token:${d}:totalSupply`,!1,this);if(m!==void 0&&this._candles[0]&&p!=null&&f!=null){let e=Number(this._P0+this._K*m/10n**18n),t=this._candles[0];this._candles[0]={open:t.open,close:e,high:Math.max(t.high,e),low:Math.min(t.low,e)}}}}connectedCallback(){this.style.cssText=`position:fixed;inset:0;z-index:0;pointer-events:none;overflow:hidden`,this._canvas=document.createElement(`canvas`),this._canvas.style.cssText=`display:block;width:100%;height:100%`,this.appendChild(this._canvas),this._ctx=this._canvas.getContext(`2d`),this._buf=document.createElement(`canvas`),this._bCtx=this._buf.getContext(`2d`),this._bloom=document.createElement(`canvas`),this._blCtx=this._bloom.getContext(`2d`),this._ro=new ResizeObserver(()=>this._onResize()),this._ro.observe(this),window.addEventListener(`lls-updated`,this._onLiveUpdate),this._onLiveUpdate(),this._startAnimation()}disconnectedCallback(){cancelAnimationFrame(this._rafId),this._ro.disconnect(),window.removeEventListener(`lls-updated`,this._onLiveUpdate)}_onResize(){let e=Math.round(this.offsetWidth),t=Math.round(this.offsetHeight);if(!e||!t)return;this._canvas.width=e,this._canvas.height=t,this._bloom.width=e,this._bloom.height=t;let n=Math.ceil(e/nm),r=Math.ceil(t/nm);this._buf.width=n,this._buf.height=r;let i=document.createElement(`canvas`);i.width=nm,i.height=nm;let a=i.getContext(`2d`),o=nm-4,s=Math.floor(o/3);a.fillStyle=`#000`,a.fillRect(0,0,nm,nm),a.fillStyle=`rgb(255,0,0)`,a.fillRect(2,2,s,o),a.fillStyle=`rgb(0,255,0)`,a.fillRect(2+s,2,s,o),a.fillStyle=`rgb(0,0,255)`,a.fillRect(2+s*2,2,s,o),this._phosphor=this._ctx.createPattern(i,`repeat`);let c=Math.ceil(n/rm)+2;c>this._candles.length&&this._currentVcAddr&&this._startCandleLoad(this._currentVcAddr,this._K,this._P0,c)}_startCandleLoad(e,t,n,r){this._loadingVcAddr=e,setTimeout(()=>{this._loadingVcAddr===e&&this._loadCandles(e,t,n,r)},3e3)}async _loadCandles(e,t,n,r){let i=od(),a=10n**18n,o,s;try{let t=await nr(()=>i.getBlock(),!1,`candle:${e}:ref`);o=t.number,s=Number(t.timestamp)}catch{return}if(this._loadingVcAddr!==e)return;let c=150n;try{let t=await nr(()=>i.getBlock({blockNumber:o-50n}),!1,`candle:${e}:probe`),n=s-Number(t.timestamp);n>0&&(c=BigInt(Math.round(50*am/n)))}catch{}if(this._loadingVcAddr!==e)return;let l=[];for(let s=0;s<=r;s++){if(this._loadingVcAddr!==e)return;let r=o-BigInt(s)*c;if(r<0n){l.push(null);continue}try{let o=await nr(()=>i.readContract({address:e,abi:B,functionName:`totalSupply`,blockNumber:r}),!1,`candle:${e}:s${s}`);l.push(Number(n+t*o/a))}catch{l.push(null)}}if(this._loadingVcAddr===e)for(let e=0;e<r;e++){let t=l[e],n=l[e+1];t!==null&&n!==null&&(this._candles[e]={close:t,open:n,high:Math.max(t,n),low:Math.min(t,n)})}}_startAnimation(){let e=performance.now(),t=n=>{let r=(n-e)/1e3;e=n,this._marqueeX-=om*r,this._draw(),this._rafId=requestAnimationFrame(t)};this._rafId=requestAnimationFrame(t)}_draw(){let e=this._canvas.width,t=this._canvas.height,n=this._buf.width,r=this._buf.height;if(!e||!t||!n||!r)return;let i=this._bCtx;i.fillStyle=`rgb(6,6,8)`,i.fillRect(0,0,n,r),this._drawCandlesBuf(i,n,r),this._drawMarqueeBuf(i,n,r);let a=n*nm,o=r*nm,s=this._ctx;s.imageSmoothingEnabled=!1,s.drawImage(this._buf,0,0,a,o),this._phosphor&&(s.globalCompositeOperation=`multiply`,s.fillStyle=this._phosphor,s.fillRect(0,0,a,o),s.globalCompositeOperation=`source-over`);let c=this._blCtx;c.clearRect(0,0,e,t),c.filter=`blur(5px)`,c.drawImage(this._canvas,0,0),c.filter=`none`,s.globalCompositeOperation=`screen`,s.globalAlpha=.35,s.drawImage(this._bloom,0,0),s.globalAlpha=1,s.globalCompositeOperation=`source-over`}_drawCandlesBuf(e,t,n){let r=this._candles.filter(e=>e!==null);if(r.length<2)return;let i=1/0,a=-1/0;for(let e of r)e.low<i&&(i=e.low),e.high>a&&(a=e.high);if(a===i)return;let o=Math.round(n*.1),s=Math.round(n*.1),c=n-o-s,l=e=>n-s-Math.round((e-i)/(a-i)*c);for(let n=0;n<r.length;n++){let i=r[n],a=t-(n+1)*rm;if(a+rm<0)break;e.fillStyle=i.close>=i.open?`rgba(0,229,255,0.2)`:`rgba(255,45,107,0.2)`;let o=a+Math.floor((rm-im)/2),s=l(i.high),c=l(i.low);e.fillRect(o,s,im,Math.max(1,c-s));let u=l(Math.max(i.open,i.close)),d=l(Math.min(i.open,i.close));e.fillRect(a,u,rm,Math.max(1,d-u))}}_drawMarqueeBuf(e,t,n){let r=Math.round(n*.8);e.save(),e.font=`${r}px "Bebas Neue", sans-serif`,e.fillStyle=this._marqueeColor,e.textBaseline=`middle`;let i=this._marqueeText+` — `,a=e.measureText(i).width;if(a<1){e.restore();return}let o=(this._marqueeX/nm%a-a)%a;for(;o<t+a;)e.fillText(i,o,n/2),o+=a;e.restore()}};customElements.get(`app-background`)||customElements.define(`app-background`,sm);var cm=10n**18n,Q=class extends up{constructor(...e){super(...e),this.liveKeys=new Set,this._onLiveUpdate=e=>{for(let t of this.liveKeys)if(e.detail.has(t)){this.requestUpdate();break}},this._onDenomChanged=()=>this.requestUpdate()}static{this.styles=of`
  ...
  `}get spin(){return V`<span class="spin"></span>`}connectedCallback(){super.connectedCallback(),window.addEventListener(`lls-updated`,this._onLiveUpdate),window.addEventListener(`denom-changed`,this._onDenomChanged),this.requestUpdate()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`lls-updated`,this._onLiveUpdate),window.removeEventListener(`denom-changed`,this._onDenomChanged)}willUpdate(e){this.liveKeys.clear()}liveValue(e,t){return this.liveKeys.add(e),Y(e,t,this)}},lm=class extends Q{constructor(...e){super(...e),this.key=``,this.numerator=1,this.denominator=1}_tokenPrefix(){if(this.key.startsWith(`fomon.`))return`token:`+A.fomon.toLocaleLowerCase();let e=this.key.match(/^token:(0x[a-fA-F0-9]+|native)/);return e?`token:${e[1]}`:null}render(){if(!this.key)return V`${this.spin}`;let e=`token:${A.shmon.toLowerCase()}`,t=[`:poolBalance`,`:totalVolume`,`:volume24h`].some(e=>this.key.endsWith(e))||this.key.includes(`:pendingRewards`)?e:this._tokenPrefix()??e,n=gp(),r=n===`mon`?`token:native`:e,i=this.liveValue(`${r}:symbol`);if(i==null)return V`${this.spin}`;let a=this.liveValue(this.key);if(this.liveValue(`meta.shmonRate`)==null||a==null)return V`${this.spin} ${i}`;let o=e=>this.numerator!==1||this.denominator!==1?e*BigInt(this.numerator)/BigInt(this.denominator):e,s;if(t===`token:native`){let e=vp();s=e>0n?o(a)*cm/e:0n}else if(t===e)s=o(a);else{let e=this.liveValue(`${t}:K`),n=this.liveValue(`${t}:P0`),r=this.liveValue(`${t}:totalSupply`);if(e==null||n==null||r==null)return V`${this.spin} ${i}`;s=(n+e*r/cm)*o(a)/cm}let c=t===r?``:`≈`;return n===`mon`?V`${c}${wp(s*vp()/cm,i)}`:V`${c}${wp(s,i)}`}};K([W()],lm.prototype,`key`,void 0),K([W({type:Number})],lm.prototype,`numerator`,void 0),K([W({type:Number})],lm.prototype,`denominator`,void 0),lm=K([U(`live-valuation`)],lm);var um=class extends Q{constructor(...e){super(...e),this.valueKey=``,this.long=!1}_symbolKey(){return this.valueKey.startsWith(`fomon.`)?`token:`+A.fomon.toLocaleLowerCase()+`:symbol`:this.valueKey.replace(/:(?!0x|native).*$/,`:symbol`)}render(){let e=this._symbolKey(),t=this.liveValue(e),n=this.liveValue(this.valueKey);if(!this.valueKey||n===void 0||t===void 0)return V`${this.spin}${t?V` $${t}`:``}`;if(this.long){let e=Number(n/10n**15n)/1e3;return V`${Intl.NumberFormat().format(e)} \$${t}`}return V`${wp(n,t)}`}};K([W({attribute:`value-key`})],um.prototype,`valueKey`,void 0),K([W({type:Boolean})],um.prototype,`long`,void 0),um=K([U(`live-token-amount`)],um);var dm=class extends Q{constructor(...e){super(...e),this.valueKey=``}_symbolKey(){return this.valueKey.startsWith(`fomon.`)?`token:`+A.fomon.toLocaleLowerCase()+`:symbol`:this.valueKey.replace(/:(?!0x|native).*$/,`:symbol`)}render(){let e=this._symbolKey(),t=this.liveValue(e),n=this.liveValue(this.valueKey);this.liveValue(`meta.shmonRate`);let r=gp()===`mon`?`MON`:`shMON`;return!this.valueKey||n===void 0||t===void 0?V`${this.spin} ${r}${t?V`/$${t}`:``}`:V`${Cp(n,t)}`}};K([W({attribute:`value-key`})],dm.prototype,`valueKey`,void 0),dm=K([U(`live-token-price`)],dm);var fm=class extends Q{constructor(...e){super(...e),this.numKey=``,this.demKey=``}render(){let e=this.liveValue(this.numKey),t=this.liveValue(this.demKey);return!this.numKey||!this.demKey||e===void 0||t===void 0?V`${this.spin}%`:V`${Op(e,t)}`}};K([W({attribute:`num-key`})],fm.prototype,`numKey`,void 0),K([W({attribute:`dem-key`})],fm.prototype,`demKey`,void 0),fm=K([U(`live-percent`)],fm);var pm=class extends Q{constructor(...e){super(...e),this.key=``,this.full=!1}render(){let e=this.liveValue(this.key);if(!this.key||e===void 0)return V`0x${this.spin}…${this.spin}`;let t=this.full?e:Ep(e),n=A.explorer;return n?V`<a href="${n}/address/${e}" target="_blank" rel="noopener noreferrer">${t}</a>`:V`${t}`}};K([W()],pm.prototype,`key`,void 0),K([W({type:Boolean})],pm.prototype,`full`,void 0),pm=K([U(`live-address`)],pm);var mm=class extends Q{constructor(...e){super(...e),this.key=``,this.fallback=`--`}render(){let e=this.liveValue(this.key);return!this.key||e===void 0?V`${this.spin}`:V`${e??this.fallback}`}};K([W()],mm.prototype,`key`,void 0),K([W()],mm.prototype,`fallback`,void 0),mm=K([U(`live-string`)],mm);var hm=class extends Q{constructor(...e){super(...e),this.key=``,this.warningMs=300*1e3,this._interval=null}static{this.styles=[Q.styles,of`
      :host(.warning) {
        color: #ff2d6b;
    ...
    `]}connectedCallback(){super.connectedCallback(),this._interval=setInterval(()=>this.requestUpdate(),50)}disconnectedCallback(){super.disconnectedCallback(),this._interval!==null&&(clearInterval(this._interval),this._interval=null)}render(){let e=this.liveValue(this.key);if(!this.key||e===void 0){this.classList.remove(`warning`);let e=this.spin;return V`${e}:${e}:${e}`}let t=this.liveValue(`meta.chainTimeDelta`)??0,n=Number(e)*1e3-(Date.now()+t*1e3);return this.classList.toggle(`warning`,n>0&&n<=this.warningMs),V`${Dp(n)}`}};K([W()],hm.prototype,`key`,void 0),K([W({type:Number,attribute:`warning-ms`})],hm.prototype,`warningMs`,void 0),hm=K([U(`live-countdown`)],hm);var gm=class extends Q{constructor(...e){super(...e),this.prefix=``}render(){let e=gp(),t=e===`mon`?`MON`:`shMON`,n=this.liveValue(`${this.prefix}:K`),r=this.liveValue(`${this.prefix}:P0`),i=this.liveValue(`${this.prefix}:totalSupply`);if(this.liveValue(`meta.shmonRate`),!this.prefix||n===void 0||r===void 0||i===void 0)return V`${this.spin} ${t}`;let a=(r+n*i/cm)*i/cm;if(e===`mon`){let e=vp();return V`≈${wp(e>0n?a*e/cm:0n,`MON`)}`}return V`≈${wp(a,`shMON`)}`}};K([W()],gm.prototype,`prefix`,void 0),gm=K([U(`live-bonding-mcap`)],gm);var _m=class extends Q{constructor(...e){super(...e),this.prefix=``}render(){let e=this.liveValue(`${this.prefix}:symbol`),t=this.liveValue(`${this.prefix}:K`),n=this.liveValue(`${this.prefix}:P0`),r=this.liveValue(`${this.prefix}:totalSupply`);if(!this.prefix||t===void 0||n===void 0||r===void 0)return V`${this.spin}${e?V` shMON/$${e}`:``}`;let i=n+t*r/cm;return V`${(Number(i)/0xde0b6b3a7640000).toFixed(9)} shMON/$${e??`?`}`}};K([W()],_m.prototype,`prefix`,void 0),_m=K([U(`live-bonding-price`)],_m);var vm=class extends Q{constructor(...e){super(...e),this.allocKey=``,this.depositKey=``,this.totalKey=``,this.symbolKey=``}render(){let e=this.liveValue(this.symbolKey),t=this.liveValue(this.allocKey),n=this.liveValue(this.depositKey),r=this.liveValue(this.totalKey);return!this.allocKey||!this.depositKey||!this.totalKey||t===void 0||n===void 0||r===void 0?V`${this.spin}${e?V` $${e}`:``}`:r===0n||t===0n?V`—`:V`${wp(t*n/r,e??`?`)}`}};K([W({attribute:`alloc-key`})],vm.prototype,`allocKey`,void 0),K([W({attribute:`deposit-key`})],vm.prototype,`depositKey`,void 0),K([W({attribute:`total-key`})],vm.prototype,`totalKey`,void 0),K([W({attribute:`symbol-key`})],vm.prototype,`symbolKey`,void 0),vm=K([U(`live-series-tokens`)],vm);var ym=`fomonad-fomon-visited`,$=class extends Q{constructor(...e){super(...e),this.open=!1,this.providerOpen=!1,this.referralCode=``,this.activeRefInput=``,this.registerPending=!1,this.claimPending=!1,this.confirmReg=!1,this.regEligError=!1,this.copiedRef=!1,this.copied=!1,this.denomPref=gp(),this._wrongNetworkClicks=0,this._onWalletChanged=()=>{I?.chainId===143&&(this._wrongNetworkClicks=0),this.requestUpdate()},this._onDocClick=e=>{e.composedPath().includes(this)||(this.open=!1,this.providerOpen=!1)}}static{this.styles=[Q.styles,of`
  ...
  `]}connectedCallback(){super.connectedCallback(),Ju.addEventListener(`wallet-changed`,this._onWalletChanged),document.addEventListener(`click`,this._onDocClick)}disconnectedCallback(){super.disconnectedCallback(),Ju.removeEventListener(`wallet-changed`,this._onWalletChanged),document.removeEventListener(`click`,this._onDocClick)}async _connect(e){await Qu(e),ed(e.info.rdns),await ld(143),this.providerOpen=!1,this.requestUpdate()}_disconnect(){$u(),td(),this.open=!1}async _copyAddress(){I&&(await navigator.clipboard.writeText(I.address),this.copied=!0,setTimeout(()=>{this.copied=!1},1500))}async _claimFoMon(){let e=R();if(!(!e||!I)){this.claimPending=!0;try{q(await e.writeContract({address:A.fomon,abi:ud,functionName:`claim`,account:I.address,chain:e.chain}),`Claimed foMON dividends`)}catch(e){J(e)}finally{this.claimPending=!1}}}async _optIn(){let e=R();if(!(!e||!I))try{q(await e.writeContract({address:A.fomon,abi:ud,functionName:`optIn`,account:I.address,chain:e.chain}),`Opted in to foMON dividends`)}catch(e){J(e)}}async _checkEligAndConfirm(){if(!this.referralCode||!I)return;let e=od();try{await e.simulateContract({address:A.game,abi:z,functionName:`registerReferral`,args:[gd(this.referralCode)],account:I.address}),this.regEligError=!1,this.confirmReg=!0}catch{this.regEligError=!0}}async _registerReferral(){if(!this.referralCode||!this.confirmReg)return;let e=R();if(!(!e||!I)){this.registerPending=!0,this.confirmReg=!1;try{let t=gd(this.referralCode),n=await e.writeContract({address:A.game,abi:z,functionName:`registerReferral`,args:[t],account:I.address,chain:e.chain}),r=this.referralCode;this.referralCode=``,q(n,`Registered referral code "${r}"`)}catch(e){J(e)}finally{this.registerPending=!1}}}async _copyRefUrl(e){await navigator.clipboard.writeText(`https://fomon.rip/${e}`),this.copiedRef=!0,setTimeout(()=>{this.copiedRef=!1},1500)}_setDenom(e){_p(e),this.denomPref=e}render(){if(!I){let e=Yu;return e.length===0?V`<button class="connect-btn" style="cursor:default;opacity:0.4">NO WALLET</button>`:e.length===1?V`
          <button class="connect-btn" @click=${()=>this._connect(e[0])}>
            ${e[0].info.icon?V`<img src=${e[0].info.icon} alt="" style="width:14px;height:14px;vertical-align:middle;margin-right:6px;border-radius:2px" />`:``}
            ...
            <div class="dropdown" style="min-width:220px;padding:12px 16px">
              ...
              ${e.map(e=>V`
                <button class="provider-item" @click=${()=>this._connect(e)}>
                  ${e.info.icon?V`<img src=${e.info.icon} alt="" />`:``}
                  ${e.info.name}
                </button>
              `)}
            </div>
      ...
      `}if(I.chainId!==143)return V`
        <button class="connect-btn" style="background:#ff2d6b22;border-color:#ff2d6b;color:#ff2d6b" @click=${()=>{this._wrongNetworkClicks++,this._wrongNetworkClicks>=2?($u(),td(),this._wrongNetworkClicks=0):ld(143)}}>${`SWITCH TO ${A?.name?.toUpperCase()??`CORRECT NETWORK`}`}</button>
      `;let e=I.address.toLowerCase(),t=A,n=t?.shmon?.toLowerCase()??``,r=t?.fomon?.toLowerCase()??``,i=this.liveValue(`token:${r}:optedIn:${e}`),a=this.liveValue(`token:${r}:pendingRewards:${e}`),o=this.liveValue(`token:${r}:balance:${e}`),s=this.liveValue(`game.referralCodeOf:${e}`),c=this.liveValue(`game.referralEarned:${e}`),l=a!==void 0&&a>0n||o!==void 0&&o>0n&&!localStorage.getItem(ym);return V`
      <div style="position:relative">
        <button
          class="address-btn ${this.open?`active`:``}"
          @click=${()=>{this.open=!this.open,this.open&&localStorage.setItem(ym,`1`)}}
        >
          ${l?V`<span class="notif-dot"></span>`:``}
          ${I.icon?V`<img src=${I.icon} alt="" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;border-radius:2px" />`:``}
          ${this.copied?V`<span class="copied-flash">COPIED!</span>`:V`<span>${Ep(I.address)}</span>`}
        ...
        ${this.open?V`
          <div class="dropdown">
            ...
            <div class="row">
              <span class="lbl">shMON${t.shmonUrl?V`&nbsp;<a href="${t.shmonUrl}" target="_blank" rel="noopener" title="redeem shMON for MON">Redeem ↗</a>`:``}</span>
    ...
    `}};K([G()],$.prototype,`open`,void 0),K([G()],$.prototype,`providerOpen`,void 0),K([G()],$.prototype,`referralCode`,void 0),K([G()],$.prototype,`activeRefInput`,void 0),K([G()],$.prototype,`registerPending`,void 0),K([G()],$.prototype,`claimPending`,void 0),K([G()],$.prototype,`confirmReg`,void 0),K([G()],$.prototype,`regEligError`,void 0),K([G()],$.prototype,`copiedRef`,void 0),K([G()],$.prototype,`copied`,void 0),K([G()],$.prototype,`denomPref`,void 0),K([G()],$.prototype,`_wrongNetworkClicks`,void 0),$=K([U(`wallet-widget`)],$);var bm=class extends up{constructor(...e){super(...e),this.currentHash=window.location.hash||`#pump`,this._onHash=()=>{this.currentHash=window.location.hash||`#pump`}}static{this.styles=of`
    :host {
      display: block;
    }
  ...
  `}connectedCallback(){super.connectedCallback(),window.addEventListener(`hashchange`,this._onHash)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._onHash)}render(){return V`
      <header>
    ...
    `}};K([G()],bm.prototype,`currentHash`,void 0),bm=K([U(`header-bar`)],bm);var xm=class extends Q{constructor(...e){super(...e),this.claimPendingA=!1,this.claimPendingB=!1,this._timerInterval=null,this._onWalletChanged=()=>{this.requestUpdate()}}static{this.styles=[Q.styles,of`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),Ju.addEventListener(`wallet-changed`,this._onWalletChanged),this._timerInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),Ju.removeEventListener(`wallet-changed`,this._onWalletChanged),this._timerInterval&&=(clearInterval(this._timerInterval),null)}async _claimA(){let e=Y(`game.cycle`),t=R();if(!(!t||!I||e===void 0)){this.claimPendingA=!0;try{q(await t.writeContract({address:A.game,abi:z,functionName:`claimSeriesATokens`,args:[e],account:I.address,chain:t.chain}),`Claimed Insider Vibecoins`)}catch(e){console.error(`Claim A error:`,e)}finally{this.claimPendingA=!1}}}async _claimB(){let e=Y(`game.cycle`),t=R();if(!(!t||!I||e===void 0)){this.claimPendingB=!0;try{q(await t.writeContract({address:A.game,abi:z,functionName:`claimSeriesBTokens`,args:[e],account:I.address,chain:t.chain}),`Claimed "Insider" Vibecoins`)}catch(e){console.error(`Claim B error:`,e)}finally{this.claimPendingB=!1}}}_phaseStatus(e,t,n,r,i){if(e<t)return`ENDED`;if(n===0n&&t===0)return e===0?`DEPOSIT TO KICK OFF (${i===void 0?`…`:Sp(i,0)} needed)`:`PENDING`;if(e<t)return`ENDED`;let a=Math.max(Number(n)-(Date.now()/1e3+r),0),o=Math.floor(a/3600),s=Math.floor(a%3600/60),c=Math.floor(a%60);return`${e>t?`OPENS`:`ENDS`} IN ${o}h ${s}m ${c}s`}render(){let e=I?.address.toLowerCase()??``,t=this.liveValue(`game.cycle`),n=this.liveValue(`game.phase`),r=this.liveValue(`game.phaseEndTime`),i=this.liveValue(`meta.chainTimeDelta`)??0;if(t===void 0||n===void 0)return V`<div class="cards">${this.spin}</div>`;let a=n===0,o=n===1,s=n===2||n===3,c=s?(this.liveValue(`game.vibecoinByCycle:${t}`)??``).toLowerCase():``,l=e?this.liveValue(`game.seriesADeposit:${t}:from:${e}`):void 0,u=e?this.liveValue(`game.seriesBDeposit:${t}:from:${e}`):void 0,d=e?l===void 0?void 0:l>0n:!1,f=e?u===void 0?void 0:u>0n:!1,p=e?this.liveValue(`game.hasClaimedSeriesA:${t}:from:${e}`):!1,m=e?this.liveValue(`game.hasClaimedSeriesB:${t}:from:${e}`):!1,h=this.liveValue(`game.const.seriesAKickoff`),g=r===void 0?void 0:this._phaseStatus(0,n,r,i,h),_=r===void 0?void 0:this._phaseStatus(1,n,r,i);return V`
      <div class="cards">
        <!-- Series A -->
    ...
    `}_openBuy(e){window.dispatchEvent(new CustomEvent(`open-buy-dialog`,{detail:{target:e}}))}};K([G()],xm.prototype,`claimPendingA`,void 0),K([G()],xm.prototype,`claimPendingB`,void 0),xm=K([U(`seed-section`)],xm);var Sm=class extends Q{constructor(...e){super(...e),this.cycle=0,this._claimPending=!1,this._optPending=!1,this._onWalletChanged=()=>{this.requestUpdate()}}static{this.styles=[Q.styles,of`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),Ju.addEventListener(`wallet-changed`,this._onWalletChanged)}disconnectedCallback(){super.disconnectedCallback(),Ju.removeEventListener(`wallet-changed`,this._onWalletChanged)}async _optToggle(e,t){let n=R();if(!(!n||!I)){this._optPending=!0,this.requestUpdate();try{let r=t?`optOut`:`optIn`;q(await n.writeContract({address:e,abi:B,functionName:r,account:I.address,chain:n.chain}),t?`Vibes undesired`:`Vibes desired`)}catch(e){J(e)}finally{this._optPending=!1,this.requestUpdate()}}}async _claimDividends(e){let t=R();if(!(!t||!I)){this._claimPending=!0,this.requestUpdate();try{q(await t.writeContract({address:e,abi:B,functionName:`claimDividends`,args:[I.address],account:I.address,chain:t.chain}),`Claim Vibes`)}catch(e){console.error(`Claim dividends error:`,e)}finally{this._claimPending=!1,this.requestUpdate()}}}_renderChart(e,t,n,r,i,a,o){let s=n*3n/2n||10n**18n,c=10n**18n,l=[],u=[];for(let n=0;n<=60;n++){let r=s*BigInt(n)/BigInt(60),i=e+t*r/c;l.push(Number(r)/0xde0b6b3a7640000),u.push(Number(i)/1e9)}let d=l[l.length-1]||1,f=Math.max(...u)||1,p=e=>38+e/d*262,m=e=>66-e/f*66,h=l.map((e,t)=>`${t===0?`M`:`L`}${p(e).toFixed(1)},${m(u[t]).toFixed(1)}`).join(` `),g=Math.round(p(Number(n)/0xde0b6b3a7640000)),_=a===void 0?null:Math.round(p(Number(a))),v=r!==void 0&&r>0n?p(Number(r)/0xde0b6b3a7640000):null,y=r!==void 0&&i!==void 0&&r>0n&&i>0n?p(Number(r+i)/0xde0b6b3a7640000):null,b=e=>String(e).split(``).map(e=>`₀₁₂₃₄₅₆₇₈₉`[+e]).join(``),x=e=>{let t=e/1e9;if(t>=.001)return t.toPrecision(3);let n=t.toFixed(30).split(`.`)[1]??``,r=0;for(let e of n)if(e===`0`)r++;else break;let i=n.slice(r,r+3);return`0.0${b(r-1)}${i}`};return V`
    ...
    `}render(){let e=this.liveValue(`game.vibecoinByCycle:${this.cycle}`),t=this.liveValue(`game.phase`);if(e===void 0)return V`<div class="panel"><div class="chart-area chart-loading">${this.spin}</div></div>`;let n=e.toLowerCase(),r=I?.address.toLowerCase()??``,i=this.liveValue(`token:${n}:K`),a=this.liveValue(`token:${n}:P0`),o=this.liveValue(`token:${n}:totalSupply`),s=this.liveValue(`token:${n}:unlocked`),c=this.liveValue(`token:${n}:athPrice`),l=this.liveValue(`token:${n}:athPostRug`),u=this.liveValue(`game.tokensSeriesAByCycle:${this.cycle}`),d=this.liveValue(`game.tokensSeriesBByCycle:${this.cycle}`),f=r?this.liveValue(`token:${n}:balance:${r}`):void 0,p=r?this.liveValue(`token:${n}:earned:${r}`):void 0,m=r?this.liveValue(`token:${n}:optedIn:${r}`):void 0,h=10n**18n,g=(()=>{if(!(c===void 0||i===void 0||a===void 0))return c===0n||i===0n||c<=a?0n:(c-a)*h/i})(),_=i!==void 0&&a!==void 0&&o!==void 0;return V`
      <div class="panel">
        <div class="row">
        ...
        ${_?this._renderChart(a,i,o,u,d,g,l):V`<div class="chart-area chart-loading">LOADING CHART…</div>`}
        ...
        <div class="dividends">
          ...
          ${r&&m===void 0?this.spin:m?V`
                <span class="div-val">
                  ${r?V`<live-token-amount long value-key="token:${n}:earned:${r}"></live-token-amount>`:`—`}
                </span>
                <button
                  class="claim-btn"
    ...
    `}};K([W({type:Number})],Sm.prototype,`cycle`,void 0),Sm=K([U(`pump-section`)],Sm);var Cm=class extends Q{constructor(...e){super(...e),this.cycle=0,this.triggerPending=!1,this._timerInterval=null}static{this.styles=[Q.styles,of`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),this._timerInterval=setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),this._timerInterval&&=(clearInterval(this._timerInterval),null)}async _triggerSettlement(){let e=R();if(!(!e||!I)){this.triggerPending=!0;try{q(await e.writeContract({address:A.game,abi:z,functionName:`triggerSettlement`,account:I.address,chain:e.chain}),`Rugpull triggered — settlement in progress`)}catch(e){J(e)}finally{this.triggerPending=!1}}}_renderBuyerBlock(e,t,n){if(e===void 0)return V`<div class="no-buyer">${this.spin}</div>`;let r=e===`0x0000000000000000000000000000000000000000`?void 0:this.liveValue(`game.referralCodeOf:${e.toLowerCase()}`),i=A.explorer;return e===`0x0000000000000000000000000000000000000000`?V`
      <div class="no-buyer">no rugpuller yet</div>
    `:V`
      <div class="rugpuller-block">
        <div class="rugpuller-label">${n?`Rugpuller`:`Current Rugpuller`}${r?V` · <span style="color:#ffe600">${r}</span>`:``}</div>
        ${i?V`
    ...
    `}render(){let e=this.liveValue(`game.cycle`);if(e===void 0)return V`<div class="panel">${this.spin}</div>`;if(this.cycle!==Number(e)){let e=this.liveValue(`game.winnerByCycle:${this.cycle}`),t=`game.prizeByCycle:${this.cycle}`;return V`
        <div class="panel">
      ...
      `}let t=this.liveValue(`game.phase`),n=this.liveValue(`game.phaseEndTime`),r=this.liveValue(`meta.chainTimeDelta`)??0,i=this.liveValue(`game.lastBuyer`);if(t===void 0||n===void 0)return V`<div class="panel">${this.spin}</div>`;let a=t===2&&n>0n?Math.max(0,Number(n)*1e3-(Date.now()+r*1e3)):0,o=t===3||t===2&&a===0;return V`
      <div class="panel">
  ...
  `]}connectedCallback(){super.connectedCallback(),this._genInterval=setInterval(()=>this._tickGen(),150),this._tickInterval=setInterval(()=>this.requestUpdate(),1e3),this._taglineInterval=setInterval(()=>{let e=this._taglineIdx;for(;e===this._taglineIdx;)e=Math.floor(Math.random()*Dm.length);this._taglineIdx=e},5e3)}disconnectedCallback(){super.disconnectedCallback(),this._genInterval&&=(clearInterval(this._genInterval),null),this._tickInterval&&=(clearInterval(this._tickInterval),null),this._taglineInterval&&=(clearInterval(this._taglineInterval),null)}async _settle(){let e=R();if(!(!e||!I)){this.settlePending=!0;try{q(await e.writeContract({address:A.game,abi:z,functionName:`settleSeriesB`,account:I.address,chain:e.chain}),`Vibecoin generated — insider rounds settled`)}catch(e){J(e)}finally{this.settlePending=!1}}}_tickGen(){this._scrambledTicker=Tm(),this._tickCount++;let e=this._genStats;if(e.tokensConsumed+=Math.floor(Math.random()*14)+4,e.exitLiquidity+=Math.floor(Math.random()*8)+2,e.hopium+=+(Math.random()<.3),e.bagHolders+=Math.floor(Math.random()*6)+1,e.rugThreads=Math.min(99.7,e.rugThreads+Math.random()*.02),Math.random()<.02&&(e.grayAreas+=1),Math.random()<.01&&(e.marketMakers+=1),this._tickCount%20==0){let e=this._headingIdx;for(;e===this._headingIdx;)e=Math.floor(Math.random()*Em.length);this._headingIdx=e}if(this._tickCount%27==0){let e=this._statIdx;for(;e===this._statIdx;)e=Math.floor(Math.random()*Om.length);this._statIdx=e,(e===9||e===10)&&(this._genStats={...this._genStats,grayAreas:Math.floor(Math.random()*3)+1,marketMakers:Math.floor(Math.random()*2)+1})}}render(){let e=A,t=this.liveValue(`game.phase`),n=this.liveValue(`game.cycle`),r=this.liveValue(`game.phaseEndTime`),i=this.liveValue(`meta.chainTimeDelta`)??0,a=this.liveValue(`game.const.seriesADuration`);if(t===void 0||n===void 0||r===void 0)return V`
        ...
        >
          ${l?V`
            ...
            <div class="vc-ticker-row">
              <span class="vc-ticker">${w===void 0?this.spin:V`\$${w}`}</span>
          ...
          `:v?V`
            <div class="vc-pending">
              ...
              <div class="vc-pending-phase">${ne?`Vibecoin Ready`:Em[this._headingIdx]}</div>
              ...
              <br>
              ${ne?`Insider rounds have concluded. Vibecoin may now be generated.`:`Vibecoin will be generated after insider rounds conclude.`}<br>
              </div>
              ${ne?V`
          ...
          `:V`
            ...
            `}
            ${e.explorer&&g?V`
              <a class="vc-name" href="${e.explorer}/address/${p}" target="_blank">${te??this.spin}</a>
            `:V`<div class="vc-name">${te??this.spin}</div>`}
            <div class="vc-ticker-row">
              <span class="vc-ticker">${T===void 0?this.spin:V`\$${T}`}</span>
              <span class="round-badge">#${Number(n)}</span>
            </div>
            ${g?V`
              <div class="hero-stats">
                <div class="hero-stat">
                  <span class="hero-stat-val"><live-valuation key="token:${ee}:poolBalance"></live-valuation></span>
                  <span class="hero-stat-label">Locked In</span>
                </div>
                <div class="hero-stat">
                  <span class="hero-stat-val"><live-bonding-mcap prefix="token:${ee}"></live-bonding-mcap></span>
                  <span class="hero-stat-label">Market Cap</span>
                </div>
                <div class="hero-stat">
    ...
    `}};K([G()],Am.prototype,`_scrambledTicker`,void 0),K([G()],Am.prototype,`_headingIdx`,void 0),K([G()],Am.prototype,`_statIdx`,void 0),K([G()],Am.prototype,`_genStats`,void 0),K([G()],Am.prototype,`settlePending`,void 0),K([G()],Am.prototype,`_taglineIdx`,void 0),Am=K([U(`pump-tab`)],Am);var jm=class extends Q{constructor(...e){super(...e),this.cycle=0,this.claimAPending=!1,this.claimBPending=!1,this.claimDivPending=!1,this.optInPending=!1,this._onWalletChanged=()=>{this.requestUpdate()}}static{this.styles=[Q.styles,of`
    :host { display: block; }
    ...
    .btn:disabled { opacity: 0.25; cursor: not-allowed; }
  `]}connectedCallback(){super.connectedCallback(),Ju.addEventListener(`wallet-changed`,this._onWalletChanged)}disconnectedCallback(){super.disconnectedCallback(),Ju.removeEventListener(`wallet-changed`,this._onWalletChanged)}async _claimDividends(){let e=R();if(!e||!I)return;let t=Y(`game.vibecoinByCycle:${this.cycle}`);if(t){this.claimDivPending=!0;try{q(await e.writeContract({address:t,abi:B,functionName:`claimDividends`,args:[I.address],account:I.address,chain:e.chain}),`Claim Vibes`)}catch(e){J(e)}finally{this.claimDivPending=!1}}}async _claimA(){let e=R();if(!(!e||!I)){this.claimAPending=!0;try{q(await e.writeContract({address:A.game,abi:z,functionName:`claimSeriesATokens`,args:[BigInt(this.cycle)],account:I.address,chain:e.chain}),`Claimed Insider Vibecoins`)}catch(e){J(e)}finally{this.claimAPending=!1}}}async _claimB(){let e=R();if(!(!e||!I)){this.claimBPending=!0;try{q(await e.writeContract({address:A.game,abi:z,functionName:`claimSeriesBTokens`,args:[BigInt(this.cycle)],account:I.address,chain:e.chain}),`Claimed "Insider" Vibecoins`)}catch(e){J(e)}finally{this.claimBPending=!1}}}async _optIn(){let e=R();if(!e||!I)return;let t=Y(`game.vibecoinByCycle:${this.cycle}`);if(t){this.optInPending=!0;try{q(await e.writeContract({address:t,abi:B,functionName:`optIn`,account:I.address,chain:e.chain}),`Vibes desired`)}catch(e){J(e)}finally{this.optInPending=!1}}}render(){if(this.cycle<=0)return V`
        <div class="card" style="border-color:#1a1a30">
          <div class="icon"></div>
          <div class="info"><div class="name-row">${this.spin}</div></div>
        </div>
      `;let e=this.liveValue(`game.vibecoinByCycle:${this.cycle}`);if(e===void 0)return V`
        <div class="card" style="border-color:#1a1a30">
          <div class="icon"></div>
          <div class="info"><div class="name-row">${this.spin}</div></div>
        </div>
      `;let t=e.toLowerCase(),n=this.liveValue(`token:${t}:name`),r=this.liveValue(`token:${t}:unlocked`),i=this.liveValue(`token:${t}:athPrice`),a=this.liveValue(`token:${t}:athPostRug`),o=this.liveValue(`token:${t}:palette`),s=this.liveValue(`token:${t}:logoUri`),c=this.liveValue(`game.winnerByCycle:${this.cycle}`),l=c?this.liveValue(`game.referralCodeOf:${c.toLowerCase()}`):void 0,u=I?.address.toLowerCase(),d=u?this.liveValue(`token:${t}:balance:${u}`):void 0,f=u?this.liveValue(`token:${t}:earned:${u}`):void 0,p=u?this.liveValue(`token:${t}:optedIn:${u}`):void 0,m=u?this.liveValue(`game.seriesADeposit:${this.cycle}:from:${u}`):void 0,h=u?this.liveValue(`game.seriesBDeposit:${this.cycle}:from:${u}`):void 0,g=u?this.liveValue(`game.hasClaimedSeriesA:${this.cycle}:from:${u}`):void 0,_=u?this.liveValue(`game.hasClaimedSeriesB:${this.cycle}:from:${u}`):void 0,v=o?.[0]??`#00e5ff`,y=o?.[1]??`#0066ff`,b=v+`14`,x=v+`2e`,S=v+`40`,C=v+`4d`,w=v+`66`,ee=v+`99`,te=v+`2a`,T=A.explorer,ne=c!==void 0&&c!==`0x0000000000000000000000000000000000000000`,re=I&&(d!==void 0&&d>0n||ne);return V`
      ...
      ">
        ...
        <div class="info">
          <div class="name-row">
            ${T?V`
              <a class="vc-name" href="${T}/address/${e}" target="_blank">${n??this.spin}</a>
            `:V`
          ...
          <div class="stats">
            ...
            ${ne?V`
              ...
              <span class="stat-val">
                ...
                ${l?V`<span style="color:#ffe600">${l}</span> `:``}
                ${T?V`
                  <a style="color:#ff2d6b;text-decoration:none" href="${T}/address/${c}" target="_blank">${Ep(c)}</a>
                `:V`<span style="color:#ff2d6b">${Ep(c)}</span>`}
              </span>
            `:``}
          </div>
        </div>
    ...
    `}};K([W({type:Number})],jm.prototype,`cycle`,void 0),K([G()],jm.prototype,`claimAPending`,void 0),K([G()],jm.prototype,`claimBPending`,void 0),K([G()],jm.prototype,`claimDivPending`,void 0),K([G()],jm.prototype,`optInPending`,void 0),jm=K([U(`vibecoin-card`)],jm);var Mm=20,Nm=class extends Q{constructor(...e){super(...e),this.search=``,this.searchResults=null,this.visibleCount=Mm,this.observer=null,this.sentinelEl=null,this._searchDebounce=null,this._onHashChange=()=>this._applyHash()}static{this.styles=[Q.styles,of`
    :host { display: block; }
  ...
  `]}connectedCallback(){super.connectedCallback(),window.addEventListener(`hashchange`,this._onHashChange),this._applyHash()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._onHashChange),this.observer?.disconnect(),this._searchDebounce&&clearTimeout(this._searchDebounce)}_applyHash(){let e=window.location.hash.split(`?`)[0].match(/^#vibecoins\/(0x[0-9a-fA-F]{40})$/);e&&this._setSearch(e[1])}_setSearch(e){if(this.search=e,this.visibleCount=Mm,this._searchDebounce&&clearTimeout(this._searchDebounce),!e.trim()){this.searchResults=null;return}this.searchResults=null,this._searchDebounce=setTimeout(()=>void this._doSearch(e.trim().toLowerCase()),200)}async _doSearch(e){let t=await Rp(e);this.search.trim().toLowerCase()===e&&(this.searchResults=t??[])}updated(){let e=this.renderRoot?.querySelector(`.sentinel`);e&&e!==this.sentinelEl&&(this.observer?.disconnect(),this.sentinelEl=e,this.observer=new IntersectionObserver(e=>{e[0].isIntersecting&&(this.visibleCount+=Mm)},{rootMargin:`200px`}),this.observer.observe(e))}render(){let e=this.liveValue(`game.cycle`),t=this.liveValue(`game.phase`),n=this.search.trim()!==``,r,i;if(n)r=this.searchResults??[],i=this.searchResults===null;else if(e===void 0||t===void 0)r=[],i=!0;else{let n=t>=2?Number(e):Number(e)-1;r=[];for(let e=n;e>=1;e--)r.push(e);i=!1}let a=r.slice(0,this.visibleCount);return V`
      <div class="container">
        ...
        `:V`
          <div class="list">
            ${a.map(e=>V`<vibecoin-card .cycle=${e}></vibecoin-card>`)}
            ${a.length<r.length?V`<div class="sentinel"></div>`:``}
          </div>
        `}
      </div>
    `}};K([G()],Nm.prototype,`search`,void 0),K([G()],Nm.prototype,`searchResults`,void 0),K([G()],Nm.prototype,`visibleCount`,void 0),Nm=K([U(`vibecoins-tab`)],Nm);function Pm(e){if(e<0n)throw Error(`sqrt of negative`);if(e===0n)return 0n;let t=e,n=(t+1n)/2n;for(;n<t;)t=n,n=(t+e/t)/2n;return t}function Fm(e,t,n,r){let i=10n**18n,a=n*t/i+r*t*t/(2n*i*i)+e,o=Pm(n*n+2n*r*a);if(r===0n)return 0n;let s=(o-n)*i/r;return s>t?s-t:0n}var Im=class extends Q{constructor(...e){super(...e),this.visible=!1,this.target=`publicBuy`,this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.error=``,this._sellShMonPreview=0n,this._buyPreview=null,this._tokenBalance=0n,this._refCode=null}static{this.styles=[Q.styles,of`
    :host { display: block; }
  ...
  `]}open(e){this.target=e.target,this.vibecoinAddress=e.vibecoinAddress,this._tokenBalance=e.tokenBalance??0n,this._refCode=bd(),this.amountStr=``,this.useShMon=!1,this.pending=!1,this.approvePending=!1,this.error=``,this._sellShMonPreview=0n,this._buyPreview=null,this.visible=!0}async _updateSellPreview(){let e=this._amountWei;if(e===0n||!this.vibecoinAddress){this._sellShMonPreview=0n;return}try{this._sellShMonPreview=await od().readContract({address:this.vibecoinAddress,abi:B,functionName:`shMonForTokens`,args:[e]})}catch{this._sellShMonPreview=0n}}_close(){this.visible=!1}get _amountWei(){let[e,t,n]=this.amountStr.match(/^([0-9]+)\.?([0-9]+)?$/)??[];return t?(n=n?n.slice(0,18).padEnd(18,`0`):`0`,BigInt(t)*10n**18n+BigInt(n)):0n}_effectiveVcAddrLow(){return this.vibecoinAddress?this.vibecoinAddress.toLowerCase():(this.liveValue(`game.vibecoin`)??``).toLowerCase()}_getTitle(){let e=this._effectiveVcAddrLow(),t=e?this.liveValue(`token:${e}:name`):void 0;switch(this.target){case`seriesA`:return`INSIDER ROUND`;case`seriesB`:return`"INSIDER" ROUND`;case`publicBuy`:return t?`PUMP ${t.toUpperCase()}`:`PUMP`;case`vibecoin`:return t?`BUY ${t.toUpperCase()}`:`BUY`;case`vibecoinSell`:return t?`SELL ${t.toUpperCase()}`:`SELL`}}_getFeeBreakdown(){if(this.target===`vibecoin`)return[[`Dividend pool`,1000n],[`To buyer`,9000n]];switch(this.target){case`seriesA`:{let e=this.liveValue(`game.const.seriesAProtocol`)??400n,t=this.liveValue(`game.const.seriesAReferral`)??600n,n=this.liveValue(`game.const.seriesAPrize`)??2000n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoins minted`,10000n-e-t-n]]}case`seriesB`:{let e=this.liveValue(`game.const.seriesBProtocol`)??400n,t=this.liveValue(`game.const.seriesBReferral`)??600n,n=this.liveValue(`game.const.seriesBPrize`)??1000n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoins minted`,10000n-e-t-n]]}default:{let e=this.liveValue(`game.const.pumpProtocol`)??400n,t=this.liveValue(`game.const.pumpReferral`)??600n,n=this.liveValue(`game.const.pumpPrize`)??2400n;return[[`Protocol`,e],[`Referral`,t],[`Rugpull Fund`,n],[`Vibecoin mint`,10000n-e-t-n]]}}}_computeTokenPreview(){let e=this._amountWei;if(e===0n)return 0n;let t=vp();if(!this.useShMon&&t===0n)return;let n=this.useShMon?e:e*10n**18n/t;if(this.target===`publicBuy`)return this._buyPreview?.tokensToUser;if(this.target===`vibecoin`){let e=this.vibecoinAddress?.toLowerCase();if(!e)return;let t=this.liveValue(`token:${e}:totalSupply`),r=this.liveValue(`token:${e}:P0`),i=this.liveValue(`token:${e}:K`);return t===void 0||r===void 0||i===void 0?void 0:Fm(n,t,r,i)}return 0n}async _updateBuyPreview(){if(this.target!==`publicBuy`)return;let e=this._amountWei;if(e===0n){this._buyPreview=null;return}let t=vp();if(!this.useShMon&&t===0n)return;let n=this.useShMon?e:e*10n**18n/t;try{let e=await od().readContract({address:A.game,abi:z,functionName:`previewBuy`,args:[n]});this._buyPreview={tokensToUser:e.tokensToUser,timerExtensionMs:e.timerExtensionMs}}catch{}}_formatTimerExtension(){let e=this._buyPreview?.timerExtensionMs;if(e===void 0)return`-`;if(e===0n)return`0s`;let t=Number(e);if(t<1e3)return`+0.${String(t).padStart(3,`0`)}s`;let n=Math.floor(t/1e3);if(n<60)return`+${n}.${String(t%1e3).padStart(3,`0`)}s`;let r=Math.floor(n/60);if(r<60)return`+${r}m ${n%60}s`;let i=Math.floor(n/3600);return i<36?`+${i}h ${r%60}m`:`+${Math.floor(i/24)}d ${i%24}h`}async _approve(){let e=A,t=R();if(!t||!I)return;let n=this._amountWei;if(n!==0n){this.approvePending=!0,this.error=``;try{let r=this.target===`vibecoin`?this.vibecoinAddress:e.game;q(await t.writeContract({address:e.shmon,abi:dd,functionName:`approve`,args:[r,n],account:I.address,chain:t.chain}),`Approved shMON Spending`)}catch(e){J(e)}finally{this.approvePending=!1}}}async _buy(){let e=A,t=R();if(!t||!I)return;let n=this._amountWei;if(n!==0n){if(this.target===`seriesA`||this.target===`seriesB`){let e=this.target===`seriesA`?0:1,t=Y(`game.phase`);if(t===void 0||t!==e){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}let n=Y(`game.phaseEndTime`)??0n,r=Y(`meta.chainTimeDelta`)??0;if(n>0n&&Date.now()/1e3+r>=Number(n)){this.error=`${this.target===`seriesA`?`Insider`:`"Insider"`} round has already ended.`;return}}this.pending=!0,this.error=``;try{let r=wd(bd()??``),i;i=this.target===`vibecoinSell`?await t.writeContract({address:this.vibecoinAddress,abi:B,functionName:`sell`,args:[n,I.address],account:I.address,chain:t.chain}):this.useShMon?this.target===`seriesA`?await t.writeContract({address:e.game,abi:z,functionName:`depositSeriesAWithShMon`,args:[r,n],account:I.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:z,functionName:`depositSeriesBWithShMon`,args:[r,n],account:I.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:z,functionName:`buyWithShMon`,args:[r,n],account:I.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:B,functionName:`buyWithShMon`,args:[I.address,n,!0],account:I.address,chain:t.chain}):this.target===`seriesA`?await t.writeContract({address:e.game,abi:z,functionName:`depositSeriesA`,args:[r],value:n,account:I.address,chain:t.chain}):this.target===`seriesB`?await t.writeContract({address:e.game,abi:z,functionName:`depositSeriesB`,args:[r],value:n,account:I.address,chain:t.chain}):this.target===`publicBuy`?await t.writeContract({address:e.game,abi:z,functionName:`buy`,args:[r],value:n,account:I.address,chain:t.chain}):await t.writeContract({address:this.vibecoinAddress,abi:B,functionName:`buy`,args:[I.address,!0],value:n,account:I.address,chain:t.chain}),this._close(),q(i,this._actionLabel())}catch(e){J(e)}finally{this.pending=!1}}}_actionLabel(){let e=this._effectiveVcAddrLowCached(),t=e?Y(`token:${e}:symbol`)??`token`:`token`,n=e?Y(`token:${e}:name`)??``:``,r=this.useShMon?bp(this._amountWei):xp(this._amountWei);switch(this.target){case`seriesA`:return`${r} insider deposit`;case`seriesB`:return`${r} "insider" deposit`;case`publicBuy`:return`Spend ${r} pumping ${n}`;case`vibecoin`:return`Spend ${r} on ${t}`;case`vibecoinSell`:return`Sell ${wp(this._amountWei,t)} for shMON`}}_effectiveVcAddrLowCached(){return this.vibecoinAddress?this.vibecoinAddress.toLowerCase():(Y(`game.vibecoin`)??``).toLowerCase()}_renderMinBuyWarning(){if(this.target!==`publicBuy`&&this.target!==`seriesA`&&this.target!==`seriesB`||this._amountWei===0n)return V``;let e=this.liveValue(`game.const.minBuy`);if(e===void 0)return V``;let t=vp();if((this.useShMon||t===0n?this._amountWei:this._amountWei*10n**18n/t)>=e)return V``;let n;if(this.useShMon)n=bp(e);else if(t===0n)n=bp(e)+` shMON equivalent`;else{let r=(e*t/10n**18n+10n**17n-1n)/10n**17n;n=`${(Number(r)/10).toFixed(1)} MON`}return V`<div class="min-buy-warning">⚠ Minimum buy: ${n}</div>`}_renderPreview(){let e=this._amountWei,t=this._effectiveVcAddrLow(),n=t?this.liveValue(`token:${t}:symbol`)??`???`:`???`;if(this.target===`vibecoinSell`)return V`
        <div class="preview">
      ...
      <div class="preview">
        ...
        <div class="fee-section">
          ${r.map(([t,r])=>{let a=this.target===`vibecoin`&&i!==void 0&&i>0n?` · ${wp(i*r/10000n,n)}`:this.target!==`vibecoin`&&e>0n?` · ${this.useShMon?Sp(e*r/10000n):xp(e*r/10000n)}`:``;return V`
              <div class="preview-row">
      ...
      `:``}
    `}render(){if(!this.visible)return V``;let e=this.target===`vibecoinSell`,t=!e&&this.useShMon,n=!!I,r=this.target===`seriesA`||this.target===`seriesB`,i=this.target===`seriesA`&&n?this.liveValue(`token:${A.fomon.toLowerCase()}:balance:${I.address.toLowerCase()}`):void 0,a=this.target===`seriesA`&&n&&i===0n,o=e?this._tokenBalance:0n,s=this.target===`vibecoin`?this.vibecoinAddress:A.game,c=t&&n?`token:${A.shmon.toLowerCase()}:allowance:${I.address.toLowerCase()}:spender:${s.toLowerCase()}`:void 0,l=c?this.liveValue(c):void 0,u=!t||l!==void 0&&l>=this._amountWei,d=this._effectiveVcAddrLow(),f=d?this.liveValue(`token:${d}:symbol`)??`???`:`???`;return V`
      <div class="overlay" @click=${e=>{e.target===e.currentTarget&&this._close()}}>
        <div class="modal">
          ...
          <div class="field">
            <label style="display:flex;justify-content:space-between">
              ...
              ${e&&o>0n?V`<span style="color:#d8d8e8">BAL: ${wp(o,f)}</span>`:``}
            </label>
    ...
    `}};K([G()],Im.prototype,`visible`,void 0),K([G()],Im.prototype,`target`,void 0),K([G()],Im.prototype,`vibecoinAddress`,void 0),K([G()],Im.prototype,`amountStr`,void 0),K([G()],Im.prototype,`useShMon`,void 0),K([G()],Im.prototype,`pending`,void 0),K([G()],Im.prototype,`approvePending`,void 0),K([G()],Im.prototype,`error`,void 0),K([G()],Im.prototype,`_sellShMonPreview`,void 0),K([G()],Im.prototype,`_buyPreview`,void 0),K([G()],Im.prototype,`_tokenBalance`,void 0),Im=K([U(`buy-dialog`)],Im);var Lm=`disclaimer-accepted-v1`,Rm=class extends up{constructor(...e){super(...e),this.open=!localStorage.getItem(Lm)}_accept(){localStorage.setItem(Lm,`1`),this.open=!1}static{this.styles=of`
    .overlay {
  ...
  `}_renderContent(){return V`
      ...
      `}render(){if(!this.open)return V``;let e={10143:`TESTNET — HEADS UP`,143:`DISCLAIMER`}[143]??`YOU FOUND THE DEV BUILD`,t={10143:`GOT IT — LET ME PLAY`,143:`I ACCEPT — LET ME PLAY`}[143]??`LETS GO`;return V`
      <div class="overlay">
        <div class="dialog">
  ...
  `}connectedCallback(){super.connectedCallback(),this._handleHash(),window.addEventListener(`hashchange`,this._handleHash),Ju.addEventListener(`wallet-changed`,this._onWalletChanged),window.addEventListener(`open-buy-dialog`,this._onOpenBuyDialog)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener(`hashchange`,this._handleHash),Ju.removeEventListener(`wallet-changed`,this._onWalletChanged),window.removeEventListener(`open-buy-dialog`,this._onOpenBuyDialog)}render(){return V`
      ...
      <main>
        ${this.tab===`pump`?V`<pump-tab></pump-tab>`:``}
        ${this.tab===`vibecoins`?V`<vibecoins-tab></vibecoins-tab>`:``}
      </main>
      <buy-dialog></buy-dialog>
      <tx-confirm-dialog></tx-confirm-dialog>
      <disclaimer-modal></disclaimer-modal>
    `}};K([G()],zm.prototype,`tab`,void 0),zm=K([U(`app-root`)],zm);function Bm(e,t,n,r=new Set){let i=e+t;switch(typeof n){case`object`:if(n===null)return i+`null
...
`}}function Vm(e){let t=e instanceof Error?e.name+`: `+e.message:`Unknown error type (?!)`,n=e instanceof Error?e.stack??e.message:Bm(``,``,e);window.dispatchEvent(new CustomEvent(`app-error-show`,{detail:{short:t,full:n,name:`UNCAUGHT ERROR`}}))}window.addEventListener(`error`,e=>Vm(e.error??e.message)),window.addEventListener(`unhandledrejection`,e=>Vm(e.reason)),tm();var Hm=document.getElementById(`loading-spinner`);Hm&&Hm.remove(),Y(`meta.shmonRate`,!0,document.documentElement);var Um=document.body,Wm=document.createElement(`app-background`);Um.insertBefore(Wm,Um.firstChild);var Gm=document.createElement(`div`);Gm.id=`app`;var Km=document.createElement(`app-root`);Gm.appendChild(Km),Um.appendChild(Gm);